...
async function make(i, n) {
  var e = path_1.default.join(i, "dist", n.name + ".cpk");
  ...
  if (n.packages["cocos-play"].resourceURL) {
    t.push("remote");
  }

  await _generate_zip(i, e, t);
}
...
function _generate_game_config_json(e, t, i) {
  ...
  if (i.bundles.length) {
    ...
    for (const o of i.bundles) {
      if (o.isSubpackage) {
        a.push({
          name: subpackagePrefix + o.name,
          root: "" + subpackagePrefix + o.name + "/",
        });
      }
    }
    n.subpackages = a;
