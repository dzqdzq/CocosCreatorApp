...
class IOSCompileCMD extends PlatformCompileCmd {
  async compile() {
    ...
    var t = /Apple/.test(t) ? "-arch arm64" : "-arch x86_64";
    const a = new RegExp(e + ".xcworkspace$");
    var e = fs.readdirSync(i).find((i) => a.test(i));

    if (e) {
      e =
        `-workspace ${i}/${e} -scheme ALL_BUILD ` +
      ...
      this.plugin.args.ios.iphoneos &&
        (await this.plugin.runCmake([e, "-sdk", "iphoneos", "-arch arm64"]));

      this.plugin.args.ios.simulator &&
        (await this.plugin.runCmake([e, "-sdk", "iphonesimulator", t]));
    }
