...
export const resolveFileNameConflict = (targetFolder: string, fileName: string): string => {
    ...
    while (existsSync(join(targetFolder, `${fileBase}${fileExt}`))) {
        if ((/(\d+)$/.test(fileBase) )) {
            if (/^\d+$/.test(fileBase)) {
                // 纯数字处理
                const num = parseInt(fileBase, 10) + 1;
                fileBase = num.toString().padStart(fileBase.length, '0');
            } else {
                // 混合字符处理（如 file123）
                fileBase = fileBase.replace(/^(.*?)(\d+)$/, ($: string, $1: string, $2: string) => {
                    const num = parseInt($2, 10) + 1;
