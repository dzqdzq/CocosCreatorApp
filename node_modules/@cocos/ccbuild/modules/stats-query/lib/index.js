...
class StatsQuery {
  ...
  getOverriddenConstantsOfFeatures(featureIds) {
    var _a;
    const constants = {};
    for (const featureId of featureIds) {
      const featureConstants = (_a = this._features[featureId]) === null || _a === void 0 ? void 0 : _a.overrideConstants;
      if (featureConstants) {
...
(function (StatsQuery) {
  class ConstantManager {
    constructor(engineRoot) {
      this._ccConfigJsonStr = '';
      this._engineRoot = engineRoot;
    }
    //#region export string
    exportDynamicConstants(options) {
    ...
    _exportConstants(options, dynamic) {
      const {
        mode,
        platform,
        flags
    ...
    _getConstantConfig() {
      ...
      if (!this._ccConfigJsonStr) {
        this._ccConfigJsonStr = fs_extra_1.default.readFileSync(path_1.default.join(this._engineRoot, './cc.config.json').replace(/\\/g, '/'), {
          encoding: 'utf-8'
        });
      }
      const engineConfig = JSON.parse(this._ccConfigJsonStr);
