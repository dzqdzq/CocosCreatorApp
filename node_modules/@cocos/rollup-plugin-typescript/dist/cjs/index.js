...
var typescript$1 = require('@cocos/typescript');
var url = require('url');
...
const getTsLibPath = () => {
    ...
    try {
        ...
        return resolveId(process.env.__TSLIB_TEST_PATH__ || 'tslib/tslib.es6.js', {
            ...
            basedir: url.fileURLToPath(new URL('.', (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index.js', document.baseURI).href))))
        });
    }
    catch (_) {
        return null;
...
function normalizeCompilerOptions(ts, compilerOptions) {
    ...
    switch (compilerOptions.module) {
        ...
        case ts.ModuleKind.System: {
            ...
            throw new Error(`@cocos/rollup-plugin-typescript: The module kind should be 'ES2015', 'ESNext', 'node16' or 'nodenext', found: '${moduleType}'`);
        }
        default:
...
function validateSourceMap(context, compilerOptions, outputOptions, autoSetSourceMap) {
    if (compilerOptions.sourceMap && !outputOptions.sourcemap && !autoSetSourceMap) {
        context.warn(`@cocos/rollup-plugin-typescript: Rollup 'sourcemap' option must be set to generate source maps.`);
    }
    else if (!compilerOptions.sourceMap && outputOptions.sourcemap) {
        context.warn(`@cocos/rollup-plugin-typescript: Typescript 'sourceMap' compiler option must be set to generate source maps.`);
    }
}
/**
...
function validatePaths(context, compilerOptions, outputOptions) {
    if (compilerOptions.out) {
        context.error(`@cocos/rollup-plugin-typescript: Deprecated Typescript compiler option 'out' is not supported. Use 'outDir' instead.`);
    }
    else if (compilerOptions.outFile) {
        context.error(`@cocos/rollup-plugin-typescript: Typescript compiler option 'outFile' is not supported. Use 'outDir' instead.`);
    }
        ...
        if (compilerOptions[dirProperty] && outputDir) {
            ...
            if (outputOptions.dir) {
                ...
                if (fromRollupDirToTs.startsWith('..')) {
                    context.error(`@cocos/rollup-plugin-typescript: Path of Typescript compiler option '${dirProperty}' must be located inside Rollup 'dir' option.`);
                }
            }
                ...
                if (fromTsDirToRollup.startsWith('..')) {
                    context.error(`@cocos/rollup-plugin-typescript: Path of Typescript compiler option '${dirProperty}' must be located inside the same directory as the Rollup 'file' option.`);
                }
            }
            else {
    ...
    if (compilerOptions.declaration || compilerOptions.declarationMap || compilerOptions.composite) {
        if (DIRECTORY_PROPS.every((dirProperty) => !compilerOptions[dirProperty])) {
            context.error(`@cocos/rollup-plugin-typescript: You are using one of Typescript's compiler options 'declaration', 'declarationMap' or 'composite'. ` +
