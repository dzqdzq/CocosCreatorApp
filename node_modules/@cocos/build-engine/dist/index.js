...
async function doBuild({ options, }) {
    ...
    const vmInternalConstants = statsQuery.evaluateEnvModuleSourceFromRecord(Object.assign({ EXPORT_TO_GLOBAL: true }, buildTimeConstants));
...
export async function initialize(isWasm) {
    let ammo;
    if (isWasm) {
        ammo = await import('${asset_ref_1.pathToAssetRefURL(wasmBinaryPath)}');
    } else {
        ammo = await import('${utils_1.filePathToModuleRequest(bulletAsmJsModule)}');
    }
    return ammo.default;
}
...
export default initialize;
        ...
        rpVirtualOptions['@cocos/bullet'] = `
import Bullet from '${utils_1.filePathToModuleRequest(bulletAsmJsModule)}';
