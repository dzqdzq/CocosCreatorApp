...
exports.isSourceChanged = exports.build = exports.parseModuleOption = exports.enumerateModuleOptionReps = exports.ModuleOption = void 0;
...
async function doBuild({ options, }) {
    ...
    const vmInternalConstants = statsQuery.evaluateEnvModuleSourceFromRecord(Object.assign({ EXPORT_TO_GLOBAL: true }, options.buildTimeConstants));
    ...
    if (split) {
        ...
        rpVirtualOptions.cc = statsQuery.evaluateIndexModuleSource(selectedFeatureUnits, (featureUnit) => utils_1.filePathToModuleRequest(statsQuery.getFeatureUnitFile(featureUnit)));
        rollupEntries.cc = 'cc';
    ...
    if (rollupFormat === 'system' || rollupFormat === 'systemjs') {
        ...
        rollupPlugins.push(code_asset_1.codeAsset({
    ...
    if (options.noDeprecatedFeatures) {
        rollupPlugins.push(remove_deprecated_features_1.default(typeof options.noDeprecatedFeatures === 'string' ? options.noDeprecatedFeatures : undefined));
    }
    rollupPlugins.push(asset_ref_1.assetRef({
        ...
        load(id) {
            ...
            return `export * from '${utils_1.filePathToModuleRequest(replacement)}';`;
        },
    }, plugin_virtual_1.default(rpVirtualOptions), ts_paths_1.default({
        ...
        sourceMap: false,
    }), plugin_babel_1.default(Object.assign({ skipPreflightCheck: true }, babelOptions)));
    if (options.progress) {
        rollupPlugins.unshift(rollup_plugin_progress_1.default());
    }
    ...
    if (visualizeOptions) {
        ...
        if (rpVisualizer) {
            const visualizeFile = (_d = visualizeOptions.file) !== null && _d !== void 0 ? _d : path_1.default.join(options.out, 'visualize.html');
    ...
    const wasmBinaryPath = path_1.default.join(ammoJsWasmModule, '..', 'ammo.wasm.wasm');
    if (ammoJsWasm === 'fallback') {
        rpVirtualOptions['@cocos/ammo'] = `
import wasmBinaryURL from '${asset_ref_1.pathToAssetRefURL(wasmBinaryPath)}';
let ammo;
let isWasm = false;
if (typeof WebAssembly === 'undefined') {
    ammo = await import('${utils_1.filePathToModuleRequest(ammoJsAsmJsModule)}');
} else {
    ammo = await import('${utils_1.filePathToModuleRequest(ammoJsWasmModule)}');
    isWasm = true;
}
...
`;
        ...
        rpVirtualOptions['@cocos/ammo'] = `
import wasmBinaryURL from '${asset_ref_1.pathToAssetRefURL(wasmBinaryPath)}';
import Ammo from '${utils_1.filePathToModuleRequest(ammoJsWasmModule)}';
export default Ammo;
const isWasm = true;
export { isWasm, wasmBinaryURL };
`;
        ...
        rpVirtualOptions['@cocos/ammo'] = `
import Ammo from '${utils_1.filePathToModuleRequest(ammoJsAsmJsModule)}';
export default Ammo;
const isWasm = false;
const wasmBinaryURL = '';
export { isWasm, wasmBinaryURL };
