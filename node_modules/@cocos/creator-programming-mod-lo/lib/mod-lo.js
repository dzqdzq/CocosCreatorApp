...
class ModLo {
    ...
    async load(url) {
        ...
        if (moduleType === 'esm') {
            ...
            asserts_1.asserts(moduleType === 'commonjs', 'Unknown module system.');
    ...
    async resolve(specifier, from, fromType) {
        ...
        asserts_1.asserts(fromType !== 'json');
    ...
    constructor(url, detail) {
        ...
        if (detail) {
            if (detail.type === 'unsupported-node-builtins') {
                detailString = i18n_1.i18nTranslate('modLo_access_denied_error_unsupported_node_builtins');
            }
            else if (detail.type === 'unopened-db') {
                detailString = i18n_1.i18nTranslate('modLo_access_denied_error_db_not_mounted', { domain: detail.domain });
            }
        }
        if (!detailString) {
            detailString = i18n_1.i18nTranslate('modLo_access_denied_error_default_reason');
        }
        super(i18n_1.i18nTranslate('modLo_access_denied_error', { url }) + detailString);
    }
}
