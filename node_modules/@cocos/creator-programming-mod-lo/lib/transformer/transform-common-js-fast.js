...
class FastCommonJsMod {
    constructor(source, id) {
        ...
        this._source = source;
        this._id = id;
        this._cjsAnalyzeResult = detect_exports_1.analyzeCommonJs(source);
    }
    source() {
        return {
            code: this._source,
...
class IdAlloc {
    constructor(source, requestCount, namedExports) {
        ...
        const ast = acorn_1.parse(source, {
            ecmaVersion: 'latest',
            sourceType: 'script',
        });
        const myIds = new Set();
...
function getModLoCommonJsWrap(code, id, analyzeResult, cjsModuleLoaderModuleSpecifier, esm) {
    ...
    console_1.assert(iCommonJsPlaceHolder >= 0);
    const iLastNewLine = wrapper.lastIndexOf('\n', iCommonJsPlaceHolder);
    console_1.assert(iLastNewLine <= iCommonJsPlaceHolder);
    return {
