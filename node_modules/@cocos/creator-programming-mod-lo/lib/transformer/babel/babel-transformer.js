...
class BabelTransformer {
    ...
    async transformJson(url, source, inputSourceMap) {
        const jsonEsmSource = transform_json_1.transformJson(url, source);
...
class BabelModBase {
    ...
    async systemjs(resolver) {
        ...
        if (resolver) {
            systemJsPlugins.unshift([detect_imports_1.createBabelPluginDetectAndRewriteImports({
                    specifiers,
                    rewrite: resolver,
                })]);
        }
    ...
    async _transform(plugins, presets) {
        ...
        const babelResult = await (syncBabelTransform
            ...
            plugins }));
        asserts_1.assertsNonNullable(babelResult);
        asserts_1.assertsNonNullable(babelResult.code);
        return {
    ...
    async _transformMultiplePasses(passes) {
        asserts_1.asserts(passes.length > 0);
        const { _url: url, _source: source, _sourceType: sourceType, _map: inputSourceMap, } = this;
        ...
        asserts_1.assertsNonNullable(firstPassResult);
        asserts_1.assertsNonNullable(firstPassResult.ast);
        ...
        for (let iPass = 1; iPass < passes.length; ++iPass) {
            ...
            asserts_1.assertsNonNullable(passResult);
            asserts_1.assertsNonNullable(passResult.ast);
            ast = passResult.ast;
        }
