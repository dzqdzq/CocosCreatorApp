...
async function cjsResolve(specifier, parentURL) {
    ...
    const resolved = await new Promise((done, rejected) => {
        resolve_1.default(specifier, resolveOpts, (err, resolved, _pkg) => {
            if (err) {
                if (node_builtins_1.isNodeJsBuiltinModule(specifier)) {
                    done(new url_1.URL(node_builtins_1.toNodeProtocolUrl(specifier)));
                }
                rejected(new resolve_error_1.ModuleNotFoundError(specifier, parentURL));
            }
            else {
                asserts_1.asserts(resolved);
                const resolvedUrl = url_1.pathToFileURL(resolved);
                done(resolvedUrl);
            }
        });
    });
