...
const initializeFieldBuilder = (0, core_1.template)(`function %%NAME%% (target, property, initializer) {
    if (!initializer) return;
    target[property]=initializer();
}`);
const applyDecoratedInitializerBuilder = (0, core_1.template)(`function %%NAME%% (target, property, decorators, initializer) {
...
function generateHelperModuleSource() {
    ...
    for (let helperName in helpers) {
        ...
        const { code } = (0, generator_1.default)(t.exportNamedDeclaration(helper));
        result += code + '\n';
    }
    return result;
}
