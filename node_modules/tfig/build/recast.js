...
function recastTopLevelModule({ program, typeChecker, rModule, nameResolver, exportPrivates, resolveEntity, registerNonExportedSymbol, }) {
    ...
    function recastRModule(rModule) {
        ...
        Object.entries(rModule.imports).map(([specifier, detail]) => {
            ...
            statements.push(nodeFactor.createImportDeclaration(undefined, // decorators
            undefined, // modifiers
        ...
        rModule.interopRecord.forEach((interop) => {
            if (interop.exports.length !== 0) {
                statements.push(nodeFactor.createExportDeclaration(undefined, // decorators
                undefined, // modifiers
                false, // isTypeOnly
                nodeFactor.createNamedExports(interop.exports.map(({ importName, asName }) => asName === importName ?
                    ...
                    nodeFactor.createExportSpecifier(nodeFactor.createIdentifier(importName), nodeFactor.createIdentifier(asName)))), nodeFactor.createStringLiteral(interop.specifier)));
            }
            if (interop.imports.length !== 0) {
                ...
                nodeFactor.createNamedImports(interop.imports.map(({ importName, asName }) => asName === importName ?
                    ...
                    nodeFactor.createImportSpecifier(nodeFactor.createIdentifier(importName), nodeFactor.createIdentifier(asName))))), nodeFactor.createStringLiteral(interop.specifier)));
            }
        });
        ...
        const moduleDeclaration = nodeFactor.createModuleDeclaration(undefined, // decorators
        [nodeFactor.createModifier(typescript_1.default.SyntaxKind.DeclareKeyword)], nodeFactor.createStringLiteral(rModule.entity.name), nodeFactor.createModuleBlock(statements));
        return moduleDeclaration;
    }
    function recastREntity(rEntity) {
        ...
        if (namespaceTraits) {
            ...
            const namespaceDeclaration = nodeFactor.createModuleDeclaration(undefined, // decorators
            [nodeFactor.createModifier(typescript_1.default.SyntaxKind.ExportKeyword)], // TODO: recastModifiers(moduleDeclaration.modifiers),
    ...
    function recastNamespaceTraits(namespaceTraits) {
        ...
        if (namespaceTraits.selfExports.length !== 0) {
            ...
            nodeFactor.createNamedExports(namespaceTraits.selfExports.map(({ importName, asName }) => asName === importName ?
                nodeFactor.createExportSpecifier(undefined, nodeFactor.createIdentifier(importName)) :
                nodeFactor.createExportSpecifier(nodeFactor.createIdentifier(importName), nodeFactor.createIdentifier(asName)))), undefined));
        }
        for (const { where, importName, asName } of namespaceTraits.selfExportsFromNamespaces) {
            statements.push(nodeFactor.createImportEqualsDeclaration(undefined, // decorators
            [nodeFactor.createModifier(typescript_1.default.SyntaxKind.ExportKeyword)], // modifiers
        ...
        if (namespaceTraits.neNamespace) {
            const neNsDeclaration = nodeFactor.createModuleDeclaration(undefined, // decorators,
            [nodeFactor.createModifier(typescript_1.default.SyntaxKind.ExportKeyword)], nodeFactor.createIdentifier(namespaceTraits.neNamespace.trait.entity.name), nodeFactor.createModuleBlock(namespaceTraits.neNamespace.statements), typescript_1.default.NodeFlags.Namespace);
    ...
    function recastStatement(statement) {
        if (typescript_1.default.isClassDeclaration(statement)) {
            ...
            return nodeFactor.createVariableStatement(recastDeclarationModifiers(statement, false), nodeFactor.createVariableDeclarationList(statement.declarationList.declarations.map((declaration) => recastVariableDeclaration(declaration, declaration.name.getText(), false)), statement.declarationList.flags));
        }
    ...
    function recastDeclarationNoComment(declaration, newName, forceExport) {
        if (typescript_1.default.isClassDeclaration(declaration)) {
            ...
            return nodeFactor.createVariableStatement(recastDeclarationModifiers(declaration, forceExport), nodeFactor.createVariableDeclarationList([recastVariableDeclaration(declaration, newName, forceExport)], declaration.parent.flags));
        }
    ...
    function recastSourceFileDeclarationAsNamespaceDeclaration(sourceFile, newName) {
        ...
        return nodeFactor.createModuleDeclaration(undefined, // decorators
        undefined, nodeFactor.createIdentifier(newName), newBody, typescript_1.default.NodeFlags.Namespace);
    }
    function recastModuleDeclarationAsNamespaceDeclaration(moduleDeclaration, newName) {
        ...
        return nodeFactor.createModuleDeclaration(undefined, // decorators
        recastDeclarationModifiers(moduleDeclaration, true), nodeFactor.createIdentifier(newName), newBody, typescript_1.default.NodeFlags.Namespace);
    }
    function recastFunctionDeclaration(functionDeclaration, newName, forceExport) {
        return nodeFactor.createFunctionDeclaration(undefined, recastModifiers(functionDeclaration.modifiers), functionDeclaration.asteriskToken, newName, recastTypeParameterArray(functionDeclaration.typeParameters), recastParameterArray(functionDeclaration.parameters), // parameters
    ...
    function recastPropertyDeclaration(propertyDeclaration) {
        return copyComments(propertyDeclaration, nodeFactor.createPropertyDeclaration(undefined, recastModifiers(propertyDeclaration.modifiers), recastPropertyName(propertyDeclaration.name), recastToken(propertyDeclaration.questionToken), recastTypeNode(propertyDeclaration.type), recastExpression(propertyDeclaration.initializer)));
    }
    function recastMethodDeclaration(methodDeclaration) {
        return copyComments(methodDeclaration, (nodeFactor.createMethodDeclaration(undefined, recastModifiers(methodDeclaration.modifiers), recastToken(methodDeclaration.asteriskToken), recastPropertyName(methodDeclaration.name), recastToken(methodDeclaration.questionToken), recastTypeParameterArray(methodDeclaration.typeParameters), recastParameterArray(methodDeclaration.parameters), // parameters
        recastTypeNode(methodDeclaration.type), undefined)));
    }
    function recastConstructorDeclaration(constructorDeclaration) {
        return copyComments(constructorDeclaration, (nodeFactor.createConstructorDeclaration(undefined, recastModifiers(constructorDeclaration.modifiers), recastParameterArray(constructorDeclaration.parameters), // parameters
        undefined)));
    }
    function recastParameter(parameter) {
        return nodeFactor.createParameterDeclaration(undefined, recastModifiers(parameter.modifiers), recastToken(parameter.dotDotDotToken), parameter.name.getText(), recastToken(parameter.questionToken), recastTypeNode(parameter.type));
    }
    ...
    function recastTypeParameter(typeParameter) {
        return nodeFactor.createTypeParameterDeclaration(typeParameter.name.getText(), recastTypeNode(typeParameter.constraint), recastTypeNode(typeParameter.default));
    }
    ...
    function recastClassDeclaration(classDeclaration, newName, forceExport) {
        ...
        for (const element of classDeclaration.members) {
            ...
            if (typescript_1.default.isMethodDeclaration(element)) {
                ...
                // Since TS 3.7
                classElements.push(copyComments(element, nodeFactor.createGetAccessorDeclaration(undefined, // decorators
                ...
                // Since TS 3.7
                classElements.push(copyComments(element, nodeFactor.createSetAccessorDeclaration(undefined, // decorators
                recastModifiers(element.modifiers), // modifiers
                recastPropertyName(element.name), // name
                recastParameterArray(element.parameters), // parameters
                undefined)));
        ...
        return nodeFactor.createClassDeclaration(undefined, recastDeclarationModifiers(classDeclaration, forceExport), newName, recastTypeParameterArray(classDeclaration.typeParameters), recastHeritageClauses(classDeclaration.heritageClauses), classElements);
    }
    ...
    function recastInterfaceDeclaration(interfaceDeclaration, newName, forceExport) {
        return nodeFactor.createInterfaceDeclaration(undefined, recastDeclarationModifiers(interfaceDeclaration, forceExport), newName, recastTypeParameterArray(interfaceDeclaration.typeParameters), recastHeritageClauses(interfaceDeclaration.heritageClauses), recastTypeElements(interfaceDeclaration.members));
    }
    ...
    function recastEnumDeclaration(enumDeclaration, newName, forceExport) {
        return nodeFactor.createEnumDeclaration(undefined, recastDeclarationModifiers(enumDeclaration, forceExport), newName, enumDeclaration.members.map((enumerator) => {
    ...
    function recastTypeAliasDeclaration(typeAliasDeclaration, newName, forceExport) {
        return nodeFactor.createTypeAliasDeclaration(undefined, recastDeclarationModifiers(typeAliasDeclaration, forceExport), newName, recastTypeParameterArray(typeAliasDeclaration.typeParameters), recastTypeNode(typeAliasDeclaration.type));
    }
    ...
    function recastTypeNode(type) {
        ...
        if (typescript_1.default.isTypeReferenceNode(type)) {
            ...
            return nodeFactor.createConstructorTypeNode(recastTypeParameterArray(type.typeParameters), recastParameterArray(type.parameters), // parameters
            recastTypeNode(type.type));
        }
            ...
            return nodeFactor.createMappedTypeNode(type.readonlyToken && type.readonlyToken.kind === typescript_1.default.SyntaxKind.ReadonlyKeyword ?
                ...
                recastToken(type.readonlyToken), recastTypeParameter(type.typeParameter), recastTypeNode(type.nameType), recastToken(type.questionToken), recastTypeNode(type.type));
        }
