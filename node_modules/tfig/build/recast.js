...
function recastTopLevelModule({ program, typeChecker, rModule, nameResolver, exportPrivates, resolveEntity, registerNonExportedSymbol, }) {
    ...
    function recastDeclarationNoComment(declaration, newName, forceExport) {
        if (typescript_1.default.isClassDeclaration(declaration)) {
            ...
            return nodeFactor.createVariableStatement(typescript_1.default.isVariableStatement(declaration.parent.parent) ?
                recastDeclarationModifiers(declaration.parent.parent, forceExport) :
                undefined, nodeFactor.createVariableDeclarationList([recastVariableDeclaration(declaration, newName, forceExport)], declaration.parent.flags));
        }
    ...
    function recastFunctionDeclaration(functionDeclaration, newName, forceExport) {
        return nodeFactor.createFunctionDeclaration(recastModifiers(functionDeclaration.modifiers), functionDeclaration.asteriskToken, newName, recastTypeParameterArray(functionDeclaration.typeParameters), recastParameterArray(functionDeclaration.parameters), // parameters
    ...
    function recastClassDeclaration(classDeclaration, newName, forceExport) {
        ...
        return nodeFactor.createClassDeclaration(recastDeclarationModifiers(classDeclaration, forceExport), newName, recastTypeParameterArray(classDeclaration.typeParameters), recastHeritageClauses(classDeclaration.heritageClauses), classElements);
    }
    function isPrivateMember(classElement) {
        const modifiers = typescript_1.default.canHaveModifiers(classElement) ? typescript_1.default.getModifiers(classElement) : undefined;
        if (!modifiers) {
            return false;
        }
    ...
    function recastInterfaceDeclaration(interfaceDeclaration, newName, forceExport) {
        return nodeFactor.createInterfaceDeclaration(recastDeclarationModifiers(interfaceDeclaration, forceExport), newName, recastTypeParameterArray(interfaceDeclaration.typeParameters), recastHeritageClauses(interfaceDeclaration.heritageClauses), recastTypeElements(interfaceDeclaration.members));
    }
    ...
    function recastEnumDeclaration(enumDeclaration, newName, forceExport) {
        return nodeFactor.createEnumDeclaration(recastDeclarationModifiers(enumDeclaration, forceExport), newName, enumDeclaration.members.map((enumerator) => {
    ...
    function recastTypeAliasDeclaration(typeAliasDeclaration, newName, forceExport) {
        return nodeFactor.createTypeAliasDeclaration(recastDeclarationModifiers(typeAliasDeclaration, forceExport), newName, recastTypeParameterArray(typeAliasDeclaration.typeParameters), recastTypeNode(typeAliasDeclaration.type));
    }
    ...
    function recastDeclarationModifiers(declaration, forceExport) {
        if (!typescript_1.default.canHaveModifiers(declaration)) {
            return [];
        }
