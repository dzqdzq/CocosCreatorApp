...
function rollupTypes(options) {
    ...
    function createTscOptions() {
        var _a;
        return {
            rootDir: (_a = options.rootDir) !== null && _a !== void 0 ? _a : path_1.default.dirname(inputs[0]),
        };
    }
    ...
    function bundle() {
        ...
        const exportDistribution = (0, distribute_exports_1.distributeExports)(entryModules.map((eM) => eM.symbol), typeChecker, options.priority);
        const distributionMap = new Map();
        const neNamespaceMap = new Map();
        ...
        function getEnclosingModuleName(symbol) {
            ...
            while (true) {
                ...
                if (typescript_1.default.isModuleDeclaration(currentNode) && !(currentNode.flags & typescript_1.default.NodeFlags.Namespace)) {
                    return (_b = (_a = typeChecker.getSymbolAtLocation(currentNode.name)) === null || _a === void 0 ? void 0 : _a.getName()) !== null && _b !== void 0 ? _b : '';
                }
                currentNode = currentNode.parent;
            }
        }
        function generateUniqueName(symbol, parentModule, referenceNamespaceTraits) {
            var _a;
            ...
            while (true) {
                ...
                else if (typescript_1.default.isModuleDeclaration(current) && typescript_1.default.isSourceFile(current.parent)) {
                    namespaces.push(createIdFromModuleName(current.name));
                }
                current = current.parent;
            }
            if (!(typescript_1.default.isModuleDeclaration(declaration0) && typescript_1.default.isSourceFile(declaration0.parent))) {
                namespaces.push(symbol.getName());
            }
            return namespaces.join('_');
        }
        function createIdFromModuleName(name) {
    ...
    function emit(groupSource) {
        ...
        const lines = [];
        const statementsArray = typescript_1.default.createNodeArray(groupSource.statements);
