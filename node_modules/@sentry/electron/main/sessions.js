...
async function endSession() {
    ...
    if (session) {
        if (session.status === 'ok') {
            core.logger.log('Ending session');
            core.endSession();
        }
        else {
            core.logger.log('Session was already ended');
        }
    }
    else {
        core.logger.log('No session');
    }
...
async function previousSessionWasAbnormal() {
    ...
    if (previous && client) {
        ...
        core.logger.log(`Found previous abnormal session`);
...
async function checkPreviousSession(crashed) {
    ...
    if (previous && client) {
        ...
        core.logger.log(`Found previous ${status} session`);
        const sesh = core.makeSession(previous);
        core.updateSession(sesh, {
            status,
            errors: (sesh.errors || 0) + 1,
...
function sessionCrashed() {
    ...
    core.logger.log('Session Crashed');
    const session = core.getIsolationScope().getSession();
    if (!session) {
        core.logger.log('No session to update');
        return;
    }
    if (session.status === 'ok') {
        core.logger.log('Setting session as crashed');
        const errors = session.errors + 1;
        core.updateSession(session, { status: 'crashed', errors });
        core.captureSession();
    }
    else {
        core.logger.log('Session already ended');
    }
}
...
function sessionAnr() {
    ...
    if (session.status === 'ok') {
        core.logger.log('Setting session as abnormal ANR');
...
const exitHandler = async (event) => {
    ...
    core.logger.log('[Session] Exit Handler');
    ...
    try {
        ...
        core.logger.warn('[Session] Error ending session:', e);
    }
    electron.app.exit();
};
