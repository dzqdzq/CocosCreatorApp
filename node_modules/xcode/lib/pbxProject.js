...
const { assert, debug } = require('console');
 ...
 * @returns {Object} file; see pbxFile
 */
pbxProject.prototype.addSourceFileForce = function (path, opt, group) {
    var file;
    if (group) {
        file = this.addFileForce(path, group, opt);
    }
    else {
        file = this.addPluginFile(path, opt);
    }
 ...
 * @returns {Object} file; see pbxFile
 */
pbxProject.prototype.addHeaderFileForce = function (path, opt, group) {
    if (group) {
        return this.addFileForce(path, group, opt);
    }
    else {
        return this.addPluginFile(path, opt);
    }
 ...
 * @returns {Object} file; see pbxFile
 */
pbxProject.prototype.addResourceFileForce = function(path, opt, group) {
    opt = opt || {};
...
pbxProject.prototype.addToPbxBuildFileSection = function(file) {
    ...
    assert(file.uuid, `uuid is not defined in file object`);
    assert(file.fileRef, `fileRef is not defined in file object`);
    // if(!file.fileRef && file.basename.indexOf("libcocos2d") > -1) throw new Error(`bad argument: ${JSON.stringify(file)}`);
...
pbxProject.prototype.addTargetDependency = function(target, dependencyTargets) {
    ...
    let pbxTargetDependencySection = this.pbxTargetDependencySection();
    let pbxContainerItemProxySection = this.pbxContainerItemProxySection();
    for (var index = 0; index < dependencyTargets.length; index++) {
        var dependencyTarget = dependencyTargets[index];
        if ( !(dependencyTarget in pbxContainerItemProxySection) && typeof nativeTargets[dependencyTarget] == "undefined"){
    ...
    for (var index = 0; index < dependencyTargets.length; index++) {
        ...
        let targetDependencyUuid = this.generateUuid();
            targetDependencyCommentKey = f("%s_comment", targetDependencyUuid);
        if(!pbxContainerItemProxySection[dependencyTargetUuid]) {
            let itemProxy = {
            ...
            let proxyItem = pbxContainerItemProxySection[dependencyTargetUuid];

            let targetDependency = {
                isa: PBXTargetDependency,
                name: proxyItem.remoteInfo,
                targetProxy: dependencyTargetUuid,
                targetProxy_comment: PBXContainerItemProxy
            };
            if(pbxTargetDependencySection && pbxContainerItemProxySection) {
                pbxTargetDependencySection[targetDependencyUuid] = targetDependency;
                pbxTargetDependencySection[targetDependencyCommentKey] = PBXTargetDependency;
                nativeTargets[target].dependencies.push({ value: targetDependencyUuid, comment: PBXTargetDependency });
            }
...
pbxProject.prototype.loadAnotherXcodeProj = function(xcodeprojPath, groupHash, opt) {
    let refProjFile = this.addFile(xcodeprojPath, groupHash, opt);
    if(!refProjFile) {
        // already add
        throw new Error(`${xcodeprojPath} is already loaded`);
    }
    let containerItemProxy = this.pbxContainerItemProxySection();
    let referenceProxy = this.pbxReferenceProxySection();
    let productGroup = this.pbxCreateGroup("Products");
    let refProj = pbxProject(path.join(xcodeprojPath, "project.pbxproj")).parseSync();
    let refTargets = refProj.pbxNativeTargetSection();

    for(let targetUUID in refTargets) {
        if(targetUUID.endsWith("_comment")) continue;
        let refTarget = refTargets[targetUUID];
        let itemProxyUUID = this.generateUuid();
        let referencProxyUUID = this.generateUuid();
        let itemProxy = {
            isa: "PBXContainerItemProxy",
            containerPortal: refProjFile.fileRef,
            containerPortal_comment: refProjFile.basename,
            proxyType: 2, //TODO: dont know the meaning
            // remoteGlobalIDString: targetUUID,
            remoteGlobalIDString: refTarget.productReference,
            remoteInfo: refTarget.name
        };
        containerItemProxy[itemProxyUUID]= itemProxy;
        containerItemProxy[itemProxyUUID + '_comment']= 'PBXContainerItemProxy';

        referenceProxy[referencProxyUUID] = {
            isa: 'PBXReferenceProxy',
			fileType : 'archive.ar',
			path : `"${refTarget.productReference_comment}"`,
            remoteRef : itemProxyUUID , /* PBXContainerItemProxy */
            remoteRef_comment : 'PBXContainerItemProxy',
			sourceTree : 'BUILT_PRODUCTS_DIR'
        };
        referenceProxy[referencProxyUUID+'_comment'] = refTarget.productReference_comment;

        this.addToPbxGroup({fileRef: referencProxyUUID, basename: refTarget.productReference_comment}, productGroup);
    }

    let root = this.hash.project.rootObject;
    let rootProj = this.pbxProjectSection()[root];
    if (rootProj) {
        let basename = path.basename(xcodeprojPath);
        let arr = rootProj.projectReferences = rootProj.projectReferences || [];
        arr.push({ 
            ProductGroup: productGroup,
            ProductGroup_comment : 'Products',
            ProjectRef : refProjFile.fileRef,
            ProjectRef_comment : basename,
        });
    }
...
pbxProject.prototype.addContainerItemProxy = function(file, remoteGlobalIDString, name) {
    let pbxContainerItemProxy = 'PBXContainerItemProxy';
    let pbxTargetDependencySection = this.pbxContainerItemProxySection();
    let remoteContainer = file.fileRef;

    for(let key in pbxTargetDependencySection) {
        let item = pbxTargetDependencySection[key];
        if(item && item.containerPortal == remoteContainer && item.remoteGlobalIDString) {
            return {uuid: key};
        }
    }

    let key = this.generateUuid();
    pbxTargetDependencySection[key] = {
        isa: "PBXContainerItemProxy",
        containerPortal: remoteContainer,
        containerPortal_comment: file.basename,
        proxyType: 2, //TODO: dont know the meaning
        remoteGlobalIDString: remoteGlobalIDString,
        remoteInfo: name
    };
    pbxTargetDependencySection[key+'_comment'] = 'PBXContainerItemProxy';
...
pbxProject.prototype.pbxBuildFileSection = function() {
    if(!this.hash.project.objects['PBXBuildFile']){
        this.hash.project.objects['PBXBuildFile'] = {};
    }
    return this.hash.project.objects['PBXBuildFile'];
}

pbxProject.prototype.pbxXCBuildConfigurationSection = function() {
    if(!this.hash.project.objects['XCBuildConfiguration']){
        this.hash.project.objects['XCBuildConfiguration'] = {};
    }
    return this.hash.project.objects['XCBuildConfiguration'];
}

pbxProject.prototype.pbxFileReferenceSection = function() {
    if(!this.hash.project.objects['PBXFileReference']){
        this.hash.project.objects['PBXFileReference'] = {};
    }
    return this.hash.project.objects['PBXFileReference'];
}

pbxProject.prototype.pbxNativeTargetSection = function() {
    if(!this.hash.project.objects['PBXNativeTarget']){
        this.hash.project.objects['PBXNativeTarget'] = {};
    }
...
pbxProject.prototype.pbxXCConfigurationList = function() {
    if(!this.hash.project.objects['XCConfigurationList']){
        this.hash.project.objects['XCConfigurationList'] = {};
    }
    return this.hash.project.objects['XCConfigurationList'];
}

pbxProject.prototype.pbxFrameworksBuildPhaseSection = function() {
    if(!this.hash.project.objects['PBXFrameworksBuildPhase']){
        this.hash.project.objects['PBXFrameworksBuildPhase'] = {};
    }
    return this.hash.project.objects['PBXFrameworksBuildPhase'];
}

pbxProject.prototype.pbxSourcesBuildPhaseSection = function() {
    if(!this.hash.project.objects['PBXSourcesBuildPhase']){
        this.hash.project.objects['PBXSourcesBuildPhase'] = {};
    }
    return this.hash.project.objects['PBXSourcesBuildPhase'];
}

pbxProject.prototype.pbxResourcesBuildPhaseSection = function() {
    if(!this.hash.project.objects['PBXResourcesBuildPhase']){
        this.hash.project.objects['PBXResourcesBuildPhase'] = {};
    }
    return this.hash.project.objects['PBXResourcesBuildPhase'];
}

pbxProject.prototype.pbxReferenceProxySection = function() {
    if(!this.hash.project.objects['PBXReferenceProxy']){
        this.hash.project.objects['PBXReferenceProxy'] = {};
    }
    return this.hash.project.objects['PBXReferenceProxy'];
}

pbxProject.prototype.pbxContainerItemProxySection = function() {
    if(!this.hash.project.objects['PBXContainerItemProxy']) {
...
pbxProject.prototype.pbxTargetDependencySection = function() {
    if(!this.hash.project.objects['PBXTargetDependency']) {
        this.hash.project.objects['PBXTargetDependency'] = {};
    }
    return this.hash.project.objects['PBXTargetDependency'];
}

pbxProject.prototype.pbxCopyFilesBuildPhaseSection = function() {
    if(!this.hash.project.objects['PBXCopyFilesBuildPhase']) {
        this.hash.project.objects['PBXCopyFilesBuildPhase'] = {};
    }
    return this.hash.project.objects['PBXCopyFilesBuildPhase'];
}
...
pbxProject.prototype.pbxEmbedAppClipsBuildPhaseObj = function (target) {
    return this.buildPhaseObject('PBXCopyFilesBuildPhase', 'Embed App Clips', target);
};
...
pbxProject.prototype.buildPhaseObject = function(name, group, target) {
    var section = this.hash.project.objects[name] = this.hash.project.objects[name] || {},
        obj, sectionKey, key;
    var buildPhase = this.buildPhase(group, target);

    if(!buildPhase) {
        let phaseUUID = this.generateUuid();
        let shortName = name.match(/PBX(\w+)BuildPhase/)[1];
        let item = {
            isa: name,
            buildActionMask:2147483647,
            files: [],
        ...
        section[phaseUUID] = item;
        section[phaseUUID+'_comment'] = shortName;
        let targetObj = this.pbxNativeTargetSection()[target];
...
function filetypeForProducttype (productType) {
    ...
    FILETYPE_BY_PRODUCTTYPE = {
            ...
            'com.apple.product-type.application.on-demand-install-capable': '"wrapper.application"',
...
pbxProject.prototype.findPBXGroupKeyAndType = function(criteria, groupType) {
    ...
    for (var key in groups) {
        ...
        if (criteria && criteria.path && criteria.name) {
            ...
            if(group.name && criteria.name instanceof RegExp) {
                if(group.name.match(criteria.name)) {
                    target = key;
                    break;
                }
            } 
...
pbxProject.prototype.addFileForce = function (path, group, opt) {
    var file = new pbxFile(path, opt);
