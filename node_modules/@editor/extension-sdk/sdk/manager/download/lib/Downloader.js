...
class Downloader {
    ...
    async storeZipFileFromBuffer(buffer) {
        const tempPath = await this.getTempZipStorePath();
        await (0, fs_extra_1.writeFile)(tempPath, Buffer.from(buffer));
        await CacheManager_1.cacheManage.save(tempPath, {
    ...
    async downloadFile(cachePath) {
        try {
            ...
            if (cachePath) {
                ...
                const fileBuffer = await (0, download_helper_1.downloadByUrl)({
                ...
                zipPath = await this.storeZipFileFromBuffer(fileBuffer);
            }
    ...
    async download() {
        ...
        const cache = await CacheManager_1.cacheManage.get({
            name: this.downloadInfo.name,
            version: this.downloadInfo.version,
        });
        await this.downloadFile(cache);
