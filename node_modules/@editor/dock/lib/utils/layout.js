...
class Snapshot {
    ...
    move (point) {
        ...
        array.forEach((list, index) => {
            for (let i = 0; i < list.length; i++) {
                ...
                // 如果是隐藏面板只做数值计算，不设置到面板上，等鼠标释放以后再作用到面板上
                const isDisplayPanel = !item.minimize;
                ...
                if (num < item.min) {
                    ...
                    if (isDisplayPanel) {
                        ...
                        requestAnimationFrame(() => {
                            ...
                            item.element.dispatchEvent(moveEvent);
                        });
                    }
                } else {
                    _offset = 0;
                ...
                if (isDisplayPanel && item._num !== num && !item.element.lock) {
                    item.element.lock = true;
                    requestAnimationFrame(() => {
                        item.element.lock = false;
                        ...
                        item.element.dispatchEvent(resizeEvent);
                    });
                }
    ...
    expandLayout () {
        const $temp = this.nextSiblingList.find(item => item.minimize) || this.prevSiblingList.find(item => item.minimize);
        if (!$temp) {
            return;
        }
        const $tabs = $temp.element.firstChild.$tabs;

        if (!$tabs || !$tabs.$nav) return;

        // get active element
        let $activeItem = $tabs.$nav.firstChild;
        for (const item of $tabs.$nav.children) {
            if (item.data.active) {
                $activeItem = item;
                break;
            }
        ...
        // 先激活面板
        $tabs.$groups.active = $activeItem.data.name;
        // 在展开
        $activeItem.dispatchEvent(new MouseEvent('dblclick', {
            bubbles: true,
            cancelable: true,
        }));
    }
