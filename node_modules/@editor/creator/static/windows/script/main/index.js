...
function _startup() {
    ...
    const ENV = Editor.Startup._builtinJSON.env || {};
    if (ENV.WINDOW_STYLE) {
        try {
    ...
    function initLayout() {
        Editor.Startup.removeListener('package-scanned-ready', initLayout);
        Editor.Startup.removeListener('package-ready', initLayout);
        ...
        if (Editor.Startup.ready.package) {
            ...
            Editor.Startup.on('package-ready', () => {
                startupCustom(topbar, footer);
            });
        }
    }
    ...
    // 初始化布局系统
    const ready = Editor.Startup.ready;
    if (ready.package) {
        initLayout();
    } else {
        Editor.Startup.on('package-scanned-ready', initLayout);
        Editor.Startup.on('package-ready', initLayout);
    }
...
function startupCustom(topbar, footer) {
    ...
    let mainHeader = ENV.MAIN_WINDOW_HEADER;
    ...
    list.forEach((pkg) => {
        if (pkg.info && pkg.info.creator && pkg.info.creator.replaceGlobal) {
            ...
            if (replaceGlobal.header) {
                try {
                    mainHeader = join(pkg.path, replaceGlobal.header);
                } catch (error) {
                    console.warn(error);
                }
            }
    ...
    if (mainHeader) {
        try {
            topbar.showCustomHeader(mainHeader);
        } catch (error) {
            console.warn(error);
        }
    }
