...
function eventEmit(event) {
    ...
    //todo:hack 的方式，应该用 MessageChannel 去适配的，但是看着好像目前不太好改，先用 remote 模块
    let webContent = remote.webContents.fromId(current.webContentID);
    webContent.send('editor-lib-panel:kit-panel-emit', current.kitID, event.type, event.detail);
}
