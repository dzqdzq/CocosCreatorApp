 ...
 * @param options
 */
async function quickCompile(options, bundlers) {
    var _a;
    ...
    const getStem = (file) => path_1.default.basename(file, path_1.default.extname(file));
    ...
    const statsQuery = await stats_query_1.StatsQuery.create(options.rootDir);
    await main();
    async function main() {
    ...
    async function targetMain(targetOptions, targetRecord, bundler, targetIndex) {
        ...
        if (targetOptions.includeIndex) {
            ...
            const vmIndex = file_uid_1.encodeVirtualFileUid('cc');
            virtualModules.set('cc', indexModuleSource);
            entryAliases['cc'] = vmIndex;
        }
        const changedFiles = await target_1.detectChangedFiles({
            rootDir,
            entries: Object.values(entryAliases),
            targetRecord,
            bundler,
            fsCache,
        });
    ...
    async function getTargetPubEntries(targetOptions) {
        ...
        for (const [k, v] of Object.entries(entryAliases)) {
            const uid = file_uid_1.encodeFilePath(v, rootDir);
            if (!uid) {
    ...
    async function writeTargetImportMap(targetOptions, targetRecord, entryAliases, bundler, chunkDir) {
        ...
        const importMapFile = path_1.default.join(targetOptions.dir, 'import-map.json');
        for (const [alias, entry] of Object.entries(entryAliases)) {
            ...
            if (typeof finalUrl === 'string') {
                ...
                const relUrl = path_url_interop_1.urlRelative(path_url_interop_1.pathToFileURL(importMapFile), path_url_interop_1.pathToFileURL(finalUrl.path));
                importMap.imports[alias] = relUrl;
            }
        }
        const chunkBaseURL = path_url_interop_1.urlRelative(path_url_interop_1.pathToFileURL(importMapFile), path_url_interop_1.pathToFileURL(chunkDir));
    ...
    async function readDirFiles(dir) {
        ...
        return (await Promise.all(files.map(async (file) => {
            const stat = await fs_extra_1.default.stat(path_1.default.join(dir, file));
...
class QuickCompiler {
    ...
    async build() {
        await quickCompile(this._options, this._bundlers);
    }
}
