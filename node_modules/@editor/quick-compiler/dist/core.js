...
async function core({ rootDir, statsQuery, entries, targetOptions, targetRecord, bundler, chunkDir, fsCache, vm, logger, onProgress, }) {
    ...
    const moduleOverrides = setupModuleOverrides();
    // Do audition.
    ...
    async function audit(fileUid) {
        ...
        await store;
        await seq_1.sequenceDiscarding(Object.entries(newFileRecord.dependencies).reduce((result, [_moduleSpecifier, resolveRecord]) => {
    ...
    async function compileFile(source, fileUid, record, bundler) {
        ...
        const readDepRecord = (moduleSpecifier) => {
            ...
            if (resolveRecord.resolved && !resolveRecord.external) {
                const replacement = module_resolve_1.replaceWithOutputExtension(getModuleRequestTo(resolveRecord.resolved));
                return replacement;
            }
            else {
                return undefined;
        ...
        const replaceModuleSpecifier = (moduleSpecifier) => {
            ...
            let resolvedModule;
            const overridePath = moduleOverrides === null || moduleOverrides === void 0 ? void 0 : moduleOverrides[moduleSpecifier];
            if (overridePath) {
                resolvedModule = {
                    file: overridePath,
                    isExternal: false,
            ...
            else if (virtualModules.has(moduleSpecifier)) {
                resolvedModule = {
            ...
            if (!resolvedModule) {
                logger.error(`Failed to resolve ${moduleSpecifier} from ${fileName}`);
            }
