...
class CacheTransformed {
    ...
    getOutFileUrl(fileUid) {
        return url_1.pathToFileURL(this.getOutFilePath(fileUid)).href;
    }
    async serialize() {
        return;
    }
    ...
    async forEachModule(visitor) {
        ...
        await visitRecursive(this._cacheDir, '', async (file, prefix) => {
            if (file.toLowerCase().endsWith('.js')) {
                ...
                await visitor(prefix, code, map, map ? url_1.pathToFileURL(mapPath) : undefined);
            }
        });
    }
    getRelativeOutputPath(fileUid) {
        ...
        if (file_uid_1.isVirtualFileUid(fileUid)) {
            const encodedName = utilts_1.encodeUrlAsFilePath(file_uid_1.decodeVirtualFileUid(fileUid));
            relative = path_1.default.join('virtual', encodedName.startsWith('.js') ? encodedName : `${encodedName}.js`);
        }
        else {
            relative = path_1.default.join('fs', transformOutputExtension(file_uid_1.decodeRegularFilePath(fileUid)));
        }
        return relative;
    }
