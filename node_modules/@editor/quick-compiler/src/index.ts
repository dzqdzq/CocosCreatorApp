...
import type { TargetOptions } from './core';
...
import { StatsQuery } from '@cocos/build-engine/dist/stats-query';
 ...
 * @param options
 */
async function quickCompile(options: QuickCompileOptions, bundlers: IBundler[]) {
    ...
    const getStem = (file: string) => ps.basename(file, ps.extname(file));
    ...
    const statsQuery = await StatsQuery.create(options.rootDir);
    ...
    async function writeTargetImportMap(
        ...
        const importMapFile = ps.join(targetOptions.dir, 'import-map.json');
    ...
    async function readDirFiles(dir: string) {
        ...
        return (await Promise.all(files.map(async (file) => {
            const stat = await fs.stat(ps.join(dir, file));
