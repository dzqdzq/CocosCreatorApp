...
var image = require('../image-47066dad.js');
...
class LFX_Baker extends EventEmitter__default['default'] {
    ...
    Upload(asset_path) {
        ...
        // Save
        fs__default['default'].writeFileSync(immediatePath + '/lfx.i', buff.Buffer);
        // Copy Textures
    ...
    Download() {
        ...
        const filename = this._path + '/output/lfx.o';
...
class LFX_App {
    ...
    static Reset(node) {
        ...
        for (const model of models) {
            ...
            if (model.node) {
                ...
                nodeMgr.emit('change', model.node, { type: NodeOperationType.SET_PROPERTY, propPath: `__comps__.${compIndex}.lightmapSettings` });
            }
        }
    ...
    static DirtyBaked(node) {
        ...
        const lights = node.getComponents(cc.Light);
        for (const light of lights) {
            if (light.staticSettings.bakeable) {
                light.baked = true;
            }
        }
        for (const child of node.children) {
    ...
    Init(data = {}) {
        ...
        // 全局光照缩放
        this.Baker.World.Settings.GIScale = data.giScale || 0; // 0: 关闭
        // 全局光照采样数
        this.Baker.World.Settings.GISamples = data.giSamples || 10;
    ...
    async _exportModel(model) {
        ...
        if (!model.lightmapSettings.bakeable && !model.lightmapSettings.castShadow) {
            return false;
        }
        const fxmesh = new LFX_Mesh();
        fxmesh.CastShadow = model.lightmapSettings.castShadow;
        fxmesh.ReceiveShadow = model.lightmapSettings.receiveShadow;
        if (model.lightmapSettings.bakeable) {
            fxmesh.LightMapSize = model.lightmapSettings.lightmapSize;
        }
        ...
        if (model.materials.length > 0) {
            ...
            for (let m = 0; m < model.materials.length; ++m) {
                ...
                if (mmtl !== null) {
                    ...
                    if (tex !== null && tex.getPixelFormat() === PixelFormat.RGBA8888) {
                    ...
                    // 启动烘焙程序 (调试模式从c++启动)
                    {
                        ...
                        this._process = exec__default['default'].execFile(LFX_SERVER, [LFX_URL], { cwd: this._uploadPath }, (err, stdout, stderr) => {
                            ...
                            if (err && err.signal !== 'SIGKILL') {
                                ...
                                console.error(err);
                                console.error('If it\'s aborted with an exception, try to lower the number of Resolution or MSAA.');
                                this.Baker.Stop();
                                clearInterval(this._timer);
                                this.Baker.Close();
                                this._timer = null;
                        ...
                        if (!this._process) {
                            console.log('Launching lightfx failed.');
                            clearInterval(this._timer);
                        }
                    }
                    lfx_launching = true;
