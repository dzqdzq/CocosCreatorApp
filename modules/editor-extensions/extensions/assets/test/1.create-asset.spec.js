...
describe('测试 Assets 面板上的 UI 元素', () => {
    [
        ...
        describe(`选中第一个数据库并创建新的 ${item.en}`, async () => {
            ...
            assetsSelector.find('.content').find('ui-drag-area').find('ui-drag-item').eq(0);
            ...
            it('在创建菜单里点击文件夹', async () => {
                ...
                addSelector.find('.header').find('.i-add').eq(0);
                ...
                inputSelector.find('.content').find('ui-drag-area').eq(0).find('input');
            ...
            it(`创建一个资源，然后改名成 ${testName}`, async function() {
                ...
                testSelector.find('.content').find('ui-drag-area').find('ui-drag-item').eq(1);
