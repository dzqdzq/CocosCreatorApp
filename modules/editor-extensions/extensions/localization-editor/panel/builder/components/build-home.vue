<template>
    <div class="home">
        <!-- <ui-prop>
            <ui-label slot="label" i18n :value="pkgName +'.build.use_polyfill'"></ui-label>
            <div slot="content">
        ...
        <ui-prop>
            <ui-label
                slot="label"
                i18n
                :value="pkgName + '.build.use_language'"></ui-label>
            <div slot="content">
                <div class="language">
                    <div id="fix-box">
                        <ui-input
                        ...
                        <div id="selectAll" class="selectItem">
                            ...
                            <ui-label i18n :value="pkgName + '.build.select_all'"></ui-label>
                        </div>
                    </div>
        ...
        <ui-prop>
            <ui-label
                slot="label"
                i18n
                :value="pkgName + '.build.default_language'"></ui-label>
            <ui-select
                slot="content"
        ...
        <ui-prop>
            <ui-label
                slot="label"
                i18n
                :value="pkgName + '.build.fallback_language'"></ui-label>
            <ui-select
                slot="content"
...
import type ILanguageInfo from '../../../src/builder/ILanguageInfo';
import type ILocalizationEditorOptions from '../../../src/builder/ILocalizationEditorOptions';
import type IPolyFillInfo from '../../../src/builder/IPolyfillInfo';
...
import { builder, wrapper, eventBus } from '../index';
type UUID = string;
...
async function updateLanguageMapAndLocalLocale(options: ITaskOptions, key?: string) {
    ...
    if (!defaultLanguageFromOptions) {
        eventBus.emit('changeBuilderOptions', `packages.${pkgName}.${defaultLanguageKey}`, defaultLanguage.value, !defaultLanguage.value);
    ...
    if (!fallbackLanguageFromOptions) {
        eventBus.emit('changeBuilderOptions', `packages.${pkgName}.${fallbackLanguageKey}`, fallbackLanguage.value, false);
...
function onPolyfillMapConfirm(key: string, value: boolean) {
    ...
    if (item) {
        item.value = value;
        eventBus.emit('changeBuilderOptions', `packages.${pkgName}.${polyfillKey}.${key}`, item, false);
    }
}
...
function onLanguageMapConfirm(key: Intl.BCP47LanguageTag, value: boolean) {
    ...
    if (item) {
        item.value = value;
        eventBus.emit('changeBuilderOptions', `packages.${pkgName}.${targetLanguageKey}.${key}`, {
            locale: item.locale,
            value,
        }, false);
    }
...
async function onDefaultLanguageConfirm(displayName: string) {
    ...
    eventBus.emit('changeBuilderOptions', `packages.${pkgName}.${defaultLanguageKey}`, value, !displayName);
...
function onFallbackLanguageConfirm(displayName: Intl.BCP47LanguageTag) {
    ...
    eventBus.emit('changeBuilderOptions', `packages.${pkgName}.${fallbackLanguageKey}`, value, false);
    onLanguageMapConfirm(value, true);
}
