...
export default class SceneThread implements ISceneThread {
    ...
    async uninstall(scanOptions: IScanOption[], indexData: TranslateDataObject): Promise<void> {
        const dirty = await this.editorMessageService.queryDirty();
        if (dirty) {
            throw new Error(`${CustomError.NAME}:${MessageCode.EDITOR_DIRTY}`);
        }
        ...
        await this.l10nManagerWrapper.releaseAsset(indexData);
    ...
    addResourceBundle(language: Intl.BCP47LanguageTag, resourceData: ResourceData): Promise<void> {
        return this.l10nManagerWrapper.addResourceBundle(language, resourceData);
    }

    preview(locale: Intl.BCP47LanguageTag, indexData: TranslateDataObject): Promise<void> {
        return this.l10nManagerWrapper.preview(locale, indexData);
    }
    ...
    removeResourceBundle(language: Intl.BCP47LanguageTag, indexData: TranslateDataObject): Promise<void> {
        return this.l10nManagerWrapper.removeResourceBundle(language, indexData);
    }
}
