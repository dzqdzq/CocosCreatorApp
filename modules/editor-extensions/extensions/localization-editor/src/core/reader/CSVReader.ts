    ...
    protected generateTranslateData(csvTranslateItems: ICSVTranslateItem[]): TranslateData {
        ...
        for (const csvItem of csvTranslateItems) {
            ...
            if (this.checkVariant(plurals, csvItem)) {
                ...
                originItem.addVariant(new TranslateItem(csvItem.key, csvItem.targetValue, TranslateItemType.External));
            } else {
                originItem = translateData.items.get(csvItem.key) ?? new TranslateItem(csvItem.key, csvItem.targetValue, TranslateItemType.External);
                originItem.value = csvItem.targetValue;
            }
