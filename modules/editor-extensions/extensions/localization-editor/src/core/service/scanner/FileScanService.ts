...
export default class FileScanService {
    ...
    async scan(
        ...
        for (const [i, item] of shouldTranslateItems.entries()) {
            ...
            const targetUrl = assetInfo.url.replaceAll(fromPattern ?? from.locale, toPattern ?? to.locale);
            if (!targetUrl.startsWith('db://assets/')) {
                console.warn(`FileScanService.scan the resources: ${targetUrl} not into db://assets/`);
                continue;
            }
