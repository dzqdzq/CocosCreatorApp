...
export default class SceneProcessor implements IProcessor {
    ...
    async process(assetInfo: AssetInfo, translateItemValueMap: Map<TranslateItemValue, TranslateItem>): Promise<TranslateItemArray> {
        ...
        if (assetInfo.type.includes('Scene')) {
            ...
            const prefabAsset: cc.Prefab = this.assetSerializeService.deserializeFile<cc.Prefab>(assetInfo.file);
    ...
    scan(baseNode: cc.BaseNode, assetInfo: AssetInfo): TranslateItem[] {
        const result: TranslateItem[] = [];
        baseNode.walk((node) => {
            if (node instanceof cc.Node) {
                if (node.getComponent(SceneProcessor.ICUComponentName)) {
                    return;
                }
                const label = node.getComponent(cc.Label);
                if (label) {
                    const sameValueItem = this.translateItemValueMap.get(label.string);
                    const addComponentResult = this.addComponent(node, sameValueItem?.key);
    ...
    private addComponent(node: cc.Node, key?: TranslateItemKey): AddComponentResult {
