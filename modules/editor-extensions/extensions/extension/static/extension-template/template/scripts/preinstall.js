...
const PATH = { packageJSON: join(__dirname, "../package.json") };
function checkCreatorTypesVersion(e) {
  var o = process.platform === "win32" ? "npm.cmd" : "npm";
  let n = spawnSync(o, ["view", "@cocos/creator-types", "versions"], {
    shell: true,
  }).stdout.toString();
  try {
    n = JSON.parse(n);
...
try {
  ...
  const g = f.devDependencies["@cocos/creator-types"].replace(/^[^\d]+/, "");

  if (!checkCreatorTypesVersion(g)) {
