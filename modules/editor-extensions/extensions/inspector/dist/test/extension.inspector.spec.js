...
const { serialize } = require("../extension/element");

const { decode } = require("../extension/adapter/xml/parser");
        ...
        async handle() {
          ...
          elem.queryTemplate = () => {
            const vElem = decode(`
                        <div></div>
                        `);
            vElem.children[0].addEventListener("change", () => {});
            return serialize(vElem);
          };
          elem.uuids = ["1"];
          let result = false;
          // @ts-ignore 覆盖方法，劫持数据用于测试
          elem.emitEvent = (vElem, name) => {
            result = true;
          };
          // 等待一个宏任务，因为 inspector-component 内有一些异步处理
          await new Promise((resolve) => {
