...
const { operation, query } = require("./shared/scene-message");
...
const Component = Vue.extend({
  ...
  computed: {
    ...
    progress() {
      return this.total ? (100 * this.finished.length) / this.total : 0;
    },
    total() {
      return (
        this.finished.length +
        this.remaining.length +
        (this.currentInfo ? 1 : 0)
      );
    },
    isFirstBuild() {
  ...
  methods: {
    ...
    stopBake() {
      ...
      if (this.currentInfo) {
        this.log(
          Editor.I18n.t("reflection-probe.panel.log.start_cancel", {
            nodeName: this.currentInfo.nodeName,
          })
        );
      }

      operation("cancel-bake");
    },
    onBakeStart(e) {
      this.currentInfo = e;
    ...
    onBakeEnd(e, t) {
      if (e === "cancel") {
        ...
        this.error(
          Editor.I18n.t("reflection-probe.panel.log.bake_error", {
            nodeName: t.nodeName,
            err: e.message,
          })
        );
        ...
        console.debug(
          `Failed to bake reflection probe on the node ${t.nodeName} with error :  ` +
            e.message
        );

        e.stack &&
          console.debug(
            `Failed to bake reflection probe on the node ${t.nodeName} with stack trace : ` +
              e.stack
          );
      } else if (
        !e &&
        !(t &&
          this.log(
            Editor.I18n.t("reflection-probe.panel.log.bake_finished", {
              nodeName: t.nodeName,
              currentNum: this.currentNum.toString(),
              totalNum: this.total.toString(),
            })
          ),
        t && this.remaining.length != 0)
      ) {
        this.log(
          Editor.I18n.t("reflection-probe.panel.log.bake_all_finished", {
            num: this.total.toString(),
          })
        );
      }
    ...
    async clearResults() {
      ...
      await operation("clear-results");
    },
    onBakeUpdate(e) {
      this.remaining = e.remaining;
      this.finished = e.finished;
      this.currentInfo = e.currentInfo ?? null;
    },
    log(e) {
      this.logInfos.push({
        text: e,
...
const methods = {
  onBakeStart(...e) {
    panelDataMap.get(this)?.component.onBakeStart(...e);
  },
  onBakeEnd(...e) {
    panelDataMap.get(this)?.component.onBakeEnd(...e);
  },
  onBakeUpdate(...e) {
    panelDataMap.get(this)?.component.onBakeUpdate(...e);
  },
  onClearUpdated(...e) {
    panelDataMap.get(this)?.component.onClearUpdated(...e);
  },
  async init() {
    if (!panelDataMap.get(this)) {
      var { remaining, finished, currentInfo } = await query("query-bake-info");

      var i = !!(await query("query-is-clearing"));
