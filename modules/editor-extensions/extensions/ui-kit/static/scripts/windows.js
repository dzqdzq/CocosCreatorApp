...
const { registerTranslator } = require('@editor/creator-ui-kit/dist/renderer');

const step = profile.get('num-input.step');
...
exports.load = function() {
    ...
    function register(item) {
        if (
            item.info.contributions
            && item.info.contributions['ui-kit']
        ) {
    ...
    Editor.UI.Curve.registerOpenHandle(async function(e) {
        ...
        Editor.Panel._kitControl.open({
            ...
            events: {
                confirm: (value) => {
                    $elem.value = value;
                    $elem.dispatch('confirm', value);
                },
                change: (value) => {
                    $elem.value = value;
                    $elem.dispatch('change', value);
                },
                cancel: (value) => {
                    $elem.value = value;
                    $elem.dispatch('cancel', value);
                },
            },
        });
    });
...
const Image = {
    ...
    async image(asset, size) {
        let fileExt = extname(asset.name).toLowerCase();

        if (!size) {
            if (['.tga', '.hdr', '.bmp'].includes(fileExt)) {
                fileExt = '.png';
            }

            return `${asset.library[fileExt]}?${Date.now()}`;
        }
