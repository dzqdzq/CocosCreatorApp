...
/**
 * 获取 NumInput 组件的配置：【步长、启用滚轮数值调节】
 * @returns {{step: (*|number), wheel_enable: (*|boolean)}}
 */
function updateNumInputConfig() {
    if (typeof Editor.UI.NumInput.changeConfig === 'function') {
        ...
        console.warn('Editor.UI.NumInput.changeConfig is not defined, need to update creator-ui-kit');
    }
}
...
exports.load = function() {
    ...
    Profile.on('change', (protocol, file, key, value) => {
        if (protocol === 'global' && file === 'editor/ui-kit.json') {
            // 更新 num-input tooltip step、wheel_enable
            if (key === 'num-input.step' || key === 'num-input.wheel_enable') {
                updateNumInputConfig();
            }
        }
    });
