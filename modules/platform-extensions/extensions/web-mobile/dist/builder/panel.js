Object.defineProperty(exports, "__esModule", { value: true });
exports.$ = undefined;
exports.data = undefined;
exports.template = undefined;
exports.update = update;
exports.ready = ready;

const { readFileSync } = require("fs");

const { join } = require("path");

const { getPreviewUrl } = require("../utils");

const qrcode = require("qrcode");
let panel;
exports.template = readFileSync(
  join(__dirname, "../../static/view.html"),
  "utf8"
);
const component = {
  data() {
    return {
      preview_url: "",
      webGPUTips: "",
      webGPULink:
        "https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts",
    };
  },
  methods: {
    t(e, t = "") {
      return Editor.I18n.t("web-mobile." + t + e);
    },
    async preview() {
      var panel_options = panel.options;
      var t = Editor.UI.__protected__.File.resolveToRaw(
        panel_options.buildPath
      );
      var t = join(t, panel_options.outputName);
      return Editor.Message.request("web-mobile", "preview", t);
    },
    async init() {
      var e = panel.options.packages["web-mobile"].useWebGPU;

      this.preview_url = await getPreviewUrl(panel.options.outputName, e);

      if (e && !this.preview_url.startsWith("https")) {
        this.webGPUTips = "i18n:web-mobile.tips.webGPUServer";
      } else {
        this.webGPUTips = "";

        this.$nextTick(() => {
          qrcode.toCanvas(this.$refs.canvas, this.preview_url, {
            errorCorrectionLevel: "H",
            maskPattern: 2,
            margin: 1,
          });
        });
      }
    },
  },
  mounted() {
    this.init();
  },
};
function update(e, t) {
  panel = this;

  if (!t || t.startsWith("packages." + panel.pkgName)) {
    panel.options = e;
    panel.vm.init();
  }
}
function ready(e, t, r) {
  panel = this;
  var i = require("vue/dist/vue.js");
  panel.options = e;
  panel.pkgName = r;
  panel.vm = new i({ el: panel.$.root, ...component });
}
exports.data = { vm: null, options: {} };
exports.$ = { root: ".web-mobile" };
