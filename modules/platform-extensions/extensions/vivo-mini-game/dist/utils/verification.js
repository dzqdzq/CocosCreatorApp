Object.defineProperty(exports, "__esModule", { value: true });
exports.defaultOptions = undefined;
exports.getVerifyFuncMap = getVerifyFuncMap;
exports.verificationFunc = verificationFunc;

const { extname } = require("path");

const { existsSync } = require("fs-extra");

function getVerifyFuncMap() {
  const e = {};

  [
    "versionName",
    "versionCode",
    "minPlatformVersion",
    "package",
    "privatePemPath",
    "certificatePemPath",
    "icon",
  ].forEach((i) => {
    e[i] = (e, t) => verificationFunc(i, e, t);
  });

  return e;
}
function verificationFunc(e, t, i) {
  var r = { error: "", newValue: t };
  let o = "";
  switch (e) {
    case "versionName":
    case "versionCode":
    case "minPlatformVersion": {
      if (!t) {
        r.error = e + " " + Editor.I18n.t("vivo-mini-game.tips.not_empty");
        r.newValue = null;
      }

      break;
    }
    case "package": {
      if (t) {
        if (!/^[a-zA-Z]+[0-9a-zA-Z_]*(\.[a-zA-Z]+[0-9a-zA-Z_]*)*$/.test(t)) {
          r.error =
            `Game package(${t})` +
            Editor.I18n.t("vivo-mini-game.tips.project_name_not_legal");

          r.newValue = null;
        }
      } else {
        r.error =
          `Game package(${t}) ` +
          Editor.I18n.t("vivo-mini-game.tips.package_name_error");

        r.newValue = null;
      }

      break;
    }
    case "privatePemPath": {
      if (t) {
        o = Editor.UI.__protected__.File.resolveToRaw(t);

        (extname(o) === ".pem" && existsSync(o)) ||
          (r.error = Editor.I18n.t(
            "vivo-mini-game.tips.private_pem_path_error"
          ));
      } else {
        r.error = Editor.I18n.t("vivo-mini-game.options.private_pem_path_hint");
      }

      break;
    }
    case "certificatePemPath": {
      if (t) {
        o = Editor.UI.__protected__.File.resolveToRaw(t);

        (extname(o) === ".pem" && existsSync(o)) ||
          (r.error = Editor.I18n.t(
            "vivo-mini-game.tips.certificate_pem_path_error"
          ));
      } else {
        r.error = Editor.I18n.t(
          "vivo-mini-game.options.certificate_pem_path_hint"
        );
      }

      break;
    }
    case "icon": {
      if (typeof t != "string") {
        r.error =
          "Icon Path" + Editor.I18n.t("vivo-mini-game.tips.icon_not_exist");
      } else if (!existsSync(t)) {
        r.error =
          "Icon Path" + Editor.I18n.t("vivo-mini-game.tips.icon_not_exist");
      }
    }
  }
  return r;
}
exports.defaultOptions = {
  package: "com.vivo.cocos",
  icon: Editor.App.icon,
  versionName: Editor.App.version,
  versionCode: Editor.App.version,
  minPlatformVersion: "1056",
  deviceOrientation: "portrait",
  useDebugKey: true,
  privatePemPath: "",
  certificatePemPath: "",
  logLevel: "log",
};
