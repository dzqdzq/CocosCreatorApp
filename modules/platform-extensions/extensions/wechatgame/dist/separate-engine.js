...
async function buildCocos(e = false) {
  ...
  if (!e || !existsSync(a)) {
    ...
    if (existsSync(s)) {
      var o = readJSONSync(s);
      if (require("lodash").isEqual(o, r)) {
        return a;
      }
    }
    emptyDirSync(n);
    const i = await buildEngine(r);
    const c = {};

    await Promise.all(
      Object.keys(i.exports).map(async (e) => {
        var t = calcCodeMd5(join(a, i.exports[e]));
        c[i.exports[e]] = t;
      })
    );

    await writeJSONSync(join(n, "meta.json"), Object.assign(i, { md5Map: c }), {
      spaces: 2,
    });

    await writeJSONSync(s, r, { spaces: 2 });
    o = getWechatPluginFeatures(t);
    s = join(n, "plugin");
    r = e.getUnitsOfFeatures(o);
    await generateCocos(
      ccbuild_1.buildEngine.enumerateDependentChunks(i, r),
      s
    );
  }
  return a;
...
async function generateCocos(e, r) {
  ...
  await Promise.all(
    e.map(async (e, t) => {
      ...
      if (o.md5Map[e]) {
        s.signature.push({ md5: o.md5Map[e], path: e });
      } else {
...
exports.outDir = join(engineRoot, "bin/.cache/editor-cache/wechat-game");
