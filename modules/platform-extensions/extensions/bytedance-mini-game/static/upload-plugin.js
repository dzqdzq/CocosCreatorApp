const axios = require("axios");
const fs = require("fs");
const FormData = require("form-data");
const crypto = require("crypto");
const promisify = require("util").promisify;
const Buffer = require("buffer").Buffer;
const { outputJSON, readJSON } = require("fs-extra");
const join = require("path").join;

const privateKey =
  "-----BEGIN RSA PRIVATE KEY-----\ \n-----END RSA PRIVATE KEY-----";

function generateSignature(e, a, r, t) {
  e = [e, a, t, r];
  const o = crypto.createHash("sha256");
  e.forEach((e) => {
    o.update(
      e +
        `
`
    );
  });
  a = o.digest("base64");
  t = crypto.createSign("RSA-SHA256");
  t.update(Buffer.from(a));

  r = t.sign(
    { key: privateKey, padding: crypto.constants.RSA_PKCS1_PADDING },
    "base64"
  );

  return r;
}
async function calculateMD5(e) {
  const a = crypto.createHash("md5");
  e = fs.createReadStream(e);

  e.on("data", (e) => {
    a.update(e);
  });

  await promisify(require("stream").pipeline)(e, async () => {});
  return a.digest("hex");
}
async function uploadZipFile(e, a, r, t) {
  if (!fs.existsSync(e)) {
    throw new Error("文件不存在: " + e);
  }
  var o = await calculateMD5(e);
  var n = Date.now();
  var i = generateSignature(a, r, n.toString(), o);
  var s = new FormData();
  s.append("app_id", a);
  s.append("version", r);
  s.append("source", fs.createReadStream(e));
  s.append("source_md5", o);
  try {
    var p = await axios.post(t, s, {
      headers: {
        ...s.getHeaders(),
        "x-signature": i,
        "x-timestamp": n.toString(),
      },
    });
    if (p.data.error === 0) {
      console.log("上传成功: " + new Date().toLocaleString());

      await outputJSON(join(__dirname, "../dist/uploadInfo.json"), {
        appID: a,
        version: r,
        signature: i,
        timestamp: n,
      });

      return { appID: a, version: r, signature: i, timestamp: n };
    }
    console.error("上传失败:", p.data);
  } catch (e) {
    console.error("上传失败:", e.response ? e.response.data : e.message);
  }
}
async function checkPluginCompileState(e) {
  var {
    appID: e,
    version,
    timestamp,
    signature,
  } = (e = e || (await readJSON(join(__dirname, "../dist/uploadInfo.json"))));

  var o = new FormData();
  o.append("app_id", e);
  o.append("version", version);
  try {
    var n = await axios.get(
      "https://developer.toutiao.com/v1/game_plugin_compile_progress",
      o,
      {
        headers: {
          ...o.getHeaders(),
          "x-signature": signature,
          "x-timestamp": timestamp.toString(),
        },
      }
    );
    if (n.data.error === 0) {
      console.log("编译成功: " + new Date().toLocaleString());
      return true;
    }
    console.error("编译失败:", n.data);
  } catch (e) {
    console.error("编译失败:", e.response ? e.response.data : e.message);
  }
}
module.exports = {
  uploadZipFile,
  checkPluginCompileState,
};
