Object.defineProperty(exports, "__esModule", { value: true });
exports.configs = undefined;
const options_1 = require("./options");
const optionConfig = {};

Object.keys(options_1.defaultOptions).forEach((e) => {
  optionConfig[e] = { default: options_1.defaultOptions[e] };
});

const cfg = {
  platformName: "i18n:android.title",
  platformType: "ANDROID",
  doc: "editor/publish/android/build-example-android.html",
  panel: "./panel",
  hooks: "./hooks",
  commonOptions: {
    polyfills: { hidden: true },
    useBuiltinServer: { hidden: false },
    nativeCodeBundleMode: { default: "wasm" },
  },
  options: Object.assign(optionConfig, {
    packageName: {
      default: options_1.defaultOptions.packageName,
      verifyRules: ["required"],
    },
    apiLevel: {
      default: options_1.defaultOptions.apiLevel,
      verifyRules: ["required"],
    },
    swappy: {
      default: false,
      label: "i18n:android.options.swappy",
      description: "i18n:android.options.swappy_tips",
      render: { ui: "ui-checkbox" },
    },
  }),
  textureCompressConfig: {
    platformType: "android",
    support: {
      rgb: [
        "etc2_rgb",
        "etc1_rgb",
        "astc_4x4",
        "astc_5x5",
        "astc_6x6",
        "astc_8x8",
        "astc_10x5",
        "astc_10x10",
        "astc_12x12",
      ],
      rgba: [
        "etc2_rgba",
        "etc1_rgb_a",
        "astc_4x4",
        "astc_5x5",
        "astc_6x6",
        "astc_8x8",
        "astc_10x5",
        "astc_10x10",
        "astc_12x12",
      ],
    },
  },
  assetBundleConfig: {
    supportedCompressionTypes: ["none", "merge_dep", "merge_all_json"],
    platformType: "native",
  },
  customIconConfigs: [
    {
      type: "icon",
      value: "android-studio",
      description: "i18n:android.customIcon.label",
      executeType: "hook",
      hook: "openWithIDE",
    },
  ],
};

exports.configs = { android: cfg, "huawei-agc": cfg };
