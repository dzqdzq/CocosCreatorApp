Object.defineProperty(exports, "__esModule", { value: true });
exports.defaultOptions = undefined;
exports.getVerifyFuncMap = getVerifyFuncMap;
exports.verificationFunc = verificationFunc;

const { extname } = require("path");

const { existsSync } = require("fs-extra");

function getVerifyFuncMap() {
  const e = {};

  [
    "versionName",
    "versionCode",
    "minPlatformVersion",
    "package",
    "privatePemPath",
    "certificatePemPath",
    "icon",
  ].forEach((r) => {
    e[r] = (e, t) => verificationFunc(r, e, t);
  });

  return e;
}
function verificationFunc(e, t, r) {
  var i = { error: "", newValue: t };
  let o = "";
  switch (e) {
    case "versionName":
    case "versionCode":
    case "minPlatformVersion": {
      if (!t) {
        i.error = e + " " + Editor.I18n.t("oppo-mini-game.tips.not_empty");
        i.newValue = null;
      }

      break;
    }
    case "package": {
      if (t) {
        if (!/^[a-zA-Z]+[0-9a-zA-Z_]*(\.[a-zA-Z]+[0-9a-zA-Z_]*)*$/.test(t)) {
          i.error =
            `Game package(${t})` +
            Editor.I18n.t("oppo-mini-game.tips.package_name_error");

          i.newValue = null;
        }
      } else {
        i.error =
          `Game package(${t}) ` +
          Editor.I18n.t("oppo-mini-game.tips.package_name_error");

        i.newValue = null;
      }

      break;
    }
    case "privatePemPath": {
      if (t) {
        o = Editor.UI.__protected__.File.resolveToRaw(t);

        (extname(o) === ".pem" && existsSync(o)) ||
          (i.error =
            "" +
            t +
            Editor.I18n.t("oppo-mini-game.tips.private_pem_path_error"));
      } else {
        i.error = Editor.I18n.t("oppo-mini-game.options.private_pem_path_hint");
      }

      break;
    }
    case "certificatePemPath": {
      if (t) {
        o = Editor.UI.__protected__.File.resolveToRaw(t);

        (extname(o) === ".pem" && existsSync(o)) ||
          (i.error = Editor.I18n.t(
            "oppo-mini-game.tips.certificate_pem_path_error"
          ));
      } else {
        i.error = Editor.I18n.t(
          "oppo-mini-game.options.certificate_pem_path_hint"
        );
      }

      break;
    }
    case "icon": {
      if (typeof t != "string") {
        i.error =
          "Icon Path" + Editor.I18n.t("oppo-mini-game.tips.icon_not_exist");
      } else if (!existsSync(t)) {
        i.error =
          "Icon Path" + Editor.I18n.t("oppo-mini-game.tips.icon_not_exist");
      }
    }
  }
  return i;
}
exports.defaultOptions = {
  package: "com.oppo.coco",
  icon: Editor.App.icon,
  versionName: Editor.App.version,
  versionCode: "1056",
  minPlatformVersion: "1056",
  deviceOrientation: "portrait",
  tinyPackageMode: false,
  useDebugKey: true,
  privatePemPath: "",
  certificatePemPath: "",
  logLevel: "log",
};
