...
返回 GPU 对象，该对象是 WebGPU API 的入口。

##### 返回值

|属性|说明|
|:--|:--|
|undefined|当前运行环境不支持 WebGPU 功能|
|GPU Object|WebGPU API 入口对象|
...
```JS
Promise<ResultObject> WebAssembly.instantiateFromFile(path, importObject)
```
...
- Object importObject [可选]

一个将被导入到新创建实例中的对象，它允许包含：函数、WebAssembly.Memory、WebAssembly.Table、WebAssembly.Global 等，在不同的运行环境中支持的情况不同，需要上层做相应的判断
...
|属性|键值类型|是否必填|说明|
|:--|:--|:--|:--|
|module|WebAssembly.Module|是|一个被编译好的 WebAssembly.Module 对象，该属性在不同的运行环境支持情况不同，需要上层做相应的判断|
|instance|WebAssembly.Instance|是|一个包含所有导出函数的 WebAssembly.Instance 对象|
