...
test('pp-geometry:normalized-integer', () => {
    ...
    uTest(Uint8Array, new Float32Array([0.0, 0.4980392156, 0.5019607843, 1.0]));
    /* eslint-disable-next-line */
    uTest(Uint16Array, new Float32Array([0.0, 0.49999237060546875, 0.5000076293945312, 1.0]));
    ...
    iTest(Int8Array, new Float32Array([-1.0, -0.5039370059967041, -0.4960629940032959, 0.0, 0.4960629940032959, 0.5039370059967041, 1.0]));
    iTest(Int16Array, new Float32Array([-1.0, -0.5000152587890625, -0.4999847412109375, 0.0, 0.4999847412109375, 0.5000152587890625, 1.0]));
});
...
test('pp-geometry:reduce-joint-influences', () => {
    ...
    const merge = (
        ...
        expect(attributes[PPGeometry.StdSemantics.set(PPGeometry.StdSemantics.joints, 0)].data).toBeExactArrayBufferView(expected.joints);
        expect(attributes[PPGeometry.StdSemantics.set(PPGeometry.StdSemantics.weights, 0)].data).toBeExactArrayBufferView(expected.weights);
    };
