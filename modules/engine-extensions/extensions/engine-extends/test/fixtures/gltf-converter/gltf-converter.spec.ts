import { GltfAssetFinderKind, GltfConverter, IGltfAssetFinder } from '../../../source/handler/assets/utils/gltf-converter';
import * as cc from 'cc';
import * as glTF from '../../../../engine-extends/@types/glTF';
...
} from '../../../source/handler/assets/utils/glTF.constants';
import { gltfConverterSilentLogger as glTFConverterSilentLogger } from '../../importers/glTF/util';
...
describe('glTF converter: prefab', () => {
    const singleRootNodeGlTf: glTF.GlTf = {
        ...dummyGlTFAsset,
        nodes: [{ name: 'SingleRootNode', children: [1, 2] }, {}, {}],
        scenes: [{ nodes: [0] }],
    };

    const multiRootNodeGlTf: glTF.GlTf = {
        ...dummyGlTFAsset,
        nodes: [{ name: 'MultiRootNode1' }, { name: 'MultiRootNode2' }],
        scenes: [{ nodes: [0, 1] }],
    };
...
describe('glTF converter/Mesh/Morph Normals', () => {
    ...
    const buffers = [positions, normals, morphPositions, morphNormals];
    ...
    test('Optional', () => {
        const glTFConverter = new GltfConverter(glTF, createBuffersFromArrayViews(buffers), dummyGlTFPath, {
            logger: glTFConverterSilentLogger,
            userData: {
...
function readMeshBufferView(data: cc.Mesh['data'], bufferView: cc.Mesh.IBufferView, constructor: typeof Float32Array) {
    return new constructor(data.buffer, data.byteOffset + bufferView.offset, bufferView.count);
}

function createBuffersFromArrayViews(arrayViews: Float32Array[]) {
    return arrayViews.map((arrayView) => Buffer.from(new Uint8Array(arrayView.buffer, arrayView.byteOffset, arrayView.byteLength)));
}

function createAccessorStuffsFromArrayViews(
    arrayViewAccessors: Array<{
        data: Float32Array;
