...
const {
  margeBundleConfig,
} = require("../../dist/importer/importers/migrates/migrate-bundle-config");
const {
  transformPlatformSettings,
} = require("../../../../../../builtin/builder/dist/share/bundle-utils");
...
describe("Migrate bundle", async () => {
  const platformConfigs = await Editor.Message.request(
    "builder",
    "query-bundle-config"
  );
  ...
  describe("Migrate resources bundle", () => {
    ...
    const { displayName, configs } = margeBundleConfig(
      compressionType,
      isRemoteBundle,
      "test1"
    );
    ...
    Object.keys(configs).forEach((platformType) => {
      const res = transformPlatformSettings(
        configs[platformType],
        platformConfigs[platformType].platformConfigs
      );
      describe(`check res with ${platformType}`, () => {
        Object.keys(res).forEach((platform) => {
          it(platform, () => {
            expect(res[platform].compressionType).to.equal(
              compressionType[platform] ?? defaultConfig.compressionType
            );
            expect(res[platform].isRemote).to.equal(
              isRemoteBundle[platform] ?? defaultConfig.isRemote
            );
          });
        });
      });
