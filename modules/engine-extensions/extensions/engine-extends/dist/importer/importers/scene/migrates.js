...
exports.migrateMaskImageStencil = undefined;
exports.migrateCSMData = undefined;
exports.migratePunctualLightLuminance = undefined;
exports.migrateShadowsData = undefined;
exports.migrateSkyLightingTypeData = undefined;
exports.migrateFogData = undefined;
exports.migrateHDRData = undefined;
exports.migrateShadowAutoAdapt = undefined;
exports.migratePrivateNode = undefined;
exports.migrateShadowDepthBias = undefined;
exports.migrateShadowInfo = undefined;
exports.migratePrefabCompPrefabInfo = undefined;
exports.migrateUILayout = undefined;
exports._renameMap = undefined;
exports.migrateShadow = undefined;
exports.migrateScrollAndPageViewComponenetModule = undefined;
exports.migrateParticleComponentModule = undefined;
exports.migrateParticleModule = undefined;
exports.migrateSkybox = undefined;
exports.migrateUIPriority = undefined;
exports.migrateWidgetComponent = undefined;
exports.migrateSavePrefabInfoAgain = undefined;
exports.migrateSavePrefabInfo = undefined;
exports.migrateDefaultLayer = undefined;
exports.migrateNameToId = undefined;
exports.migrateAnimationName = undefined;
exports.migrateImageUuid = undefined;
...
const prefab_1 = require("../migrates/prefab");
const migration_utils_1 = require("../utils/migration-utils");
const migrate_curve_range_3_3_0_1 = require("../migrates/migrate-curve-range-3-3-0");
const migrate_geometry_curve_3_3_0_1 = require("../migrates/migrate-geometry-curve-3-3-0");
const migrate_prefab_1_1_34_1 = require("./migrate-prefab-1-1-34");
  ...
  utils_2.walk(e, (a) => {
    if ("__uuid__" in a) {
      ...
      if (/@/.test(a_uuid)) {
        var i = a_uuid.split("@");
        let e;
        a_uuid = asset_db_1.queryAsset(i[0]);
...
exports.migrationHook = {
  ...
  async post(e, a) {
    ...
    if (a > 0) {
      a = JSON.stringify(t.json, null, 2);
      await fs_extra_1.writeFile(e.source, a);
    }
...
exports.migrations = [
  ...
  { version: "1.1.26", migrate: migrateCanvasCamera },
  {
    ...
    migrate: async (e) => {
      ...
      var t = a.json || (await fs_extra_1.readJSON(e.source));
      await migratePrefabCompPrefabInfo(e);
      await prefab_1.beforeMigratePrefab(e);
      await prefab_1.migratePrefab(e);
  ...
  },
  {
    version: "1.1.34",
    migrate: async (e) => {
      await migrate_prefab_1_1_34_1.migratePrefabInstanceRoots(e);
    },
  },
  {
    version: "1.1.35",
...
async function migrateSkinningRoot(e) {
  ...
  for (const r of a) {
    if (skinningRE.test(r.__type__)) {
      ...
      if (!utils_2.getComponent(a, t, animationRE)) {
        let e = r.node.__id__;

        while (
          e === r.node.__id__ ||
          !utils_2.getComponent(a, e, animationRE)
        ) {
          var i = a[e]._parent;
          e = (i && i.__id__) || null;
        }
  ...
  fs_extra_1.writeJSONSync(e.source, a, { spaces: 2 });
}
async function migrateAnimationName(r) {
  var e = r.getSwapSpace().json || (await fs_extra_1.readJSON(r.source));

  await utils_2.walkAsync(e, async (a) => {
    if ("__uuid__" in a) {
      var a_uuid = a.__uuid__;
      if (/@/.test(a_uuid)) {
  ...
  fs_extra_1.writeJSONSync(r.source, e, { spaces: 2 });
}
...
async function migrateNormalizeScene(e) {
  ...
  if (e.extname === ".scene") {
    a = e.getSwapSpace().json || (await fs_extra_1.readJSON(e.source));
    ...
    fs_extra_1.writeFileSync(e.source, EditorExtends.serialize(a));
    MissingClass.reset();
  }
}
...
async function migrateVisibility(e) {
  var a;
  var t = e.getSwapSpace().json || (await fs_extra_1.readJSON(e.source));
  for (const i of t) {
    ...
    if (/@/.test(e_uuid)) {
      ...
      if (!(t.length <= 1)) {
        for (let e = 1; e < t.length; e++) {
          t[e] = utils_1.nameToId(t[e]);
        }
        e_uuid = t.join("@");

        if (i === true || asset_db_1.queryAsset(e_uuid)) {
          e.__uuid__ = e_uuid;
        }
      }
    }
...
async function migrateSavePrefabInfo(t) {
  if (t.extname === ".prefab") {
    ...
    var t = r.json || (await fs_extra_1.readJSON(t.source));
    let e = false;
    let a = false;
    for (const o of t) {
...
async function migrateCameraVisibility(e) {
  (e.getSwapSpace().json || (await fs_extra_1.readJSON(e.source))).forEach(
    (e) => {
...
async function migrateWidgetComponent(e) {
  for (const a of e.getSwapSpace().json ||
    (await fs_extra_1.readJSON(e.source))) {
...
async function migrateSkinningMaterial(e) {
  var a;
  var t;
  var i = e.getSwapSpace().json || (await fs_extra_1.readJSON(e.source));
  for (const c of i) {
    if (c._materials && c.__type__ === "cc.SkinningModelComponent") {
      for (const l of c._materials) {
        var r;
        var o = l && l.__uuid__;
        var n = o && asset_db_1.queryAsset(o);
        if (n && !skinningMaterials.has(o)) {
          e._assetDB.taskManager.pause(e.task);
          await n.waitInit();
          e._assetDB.taskManager.resume(e.task);

          if (fs_extra_1.existsSync(n.source)) {
            skinningMaterials.add(o);
            var _ = await fs_extra_1.readJSON(n.source);
            var s = _._defines.length || 1;
            for (let a = 0; a < s; a++) {
              let e = _._defines[a];

              if (!(e = e || (_._defines[0] = {})).USE_SKINNING) {
                e.USE_SKINNING = true;
              }
            }
            fs_extra_1.writeJSONSync(n.source, _, { spaces: 2 });
          } else {
            if (o[o.length - 6] === "@" && !n._name.includes("-skinning")) {
              r = utils_1.nameToId(
                r.slice(0, r.indexOf(".")) + "-skinning.material"
              );

              n.parent &&
                n.parent.subAssets[r] &&
...
async function migrateBackSkinningMaterial(e) {
  for (const _ of e.getSwapSpace().json ||
    (await fs_extra_1.readJSON(e.source))) {
    if (_._materials && _.__type__ === "cc.SkinningModelComponent") {
      for (const s of _._materials) {
        var a = s && s.__uuid__;
        if (a[a.length - 6] === "@") {
          var t = a.slice(0, a.length - 6);
          var i = asset_db_1.queryAsset(t);
          if (i) {
            e._assetDB.taskManager.pause(e.task);
            await i.waitInit();
            e._assetDB.taskManager.resume(e.task);
            ...
            for (const c in i.subAssets) {
              ...
              if (
                utils_1.nameToId(
                  n.slice(0, n.indexOf(".")) + "-skinning.material"
                ) === r
              ) {
                s.__uuid__ = o.uuid;
                break;
...
async function migrateSkinningMaterialForMeshSplit(e) {
  for (const _ of e.getSwapSpace().json ||
    (await fs_extra_1.readJSON(e.source))) {
    if (_.__type__ === "cc.SkinningModelComponent") {
      var _materials = _._materials;
      if (_._mesh) {
        var t = _._mesh.__uuid__;
        var i = t.split("@")[0];
        var i = asset_db_1.queryAsset(i);
        if (i) {
          e._assetDB.taskManager.pause(e.task);
          await i.waitInit();
          e._assetDB.taskManager.resume(e.task);
          i = asset_db_1.queryAsset(t);
          if (i) {
            t = i.library + ".json";
            if (fs_extra_1.existsSync(t)) {
              const s = fs_extra_1.readJsonSync(t)._struct;
              var r = s.primitives.length;
              const _materials_length = _materials.length;
              if (_materials_length < r) {
                t = await reader_manager_1.glTfReaderManager.getOrCreate(
                  i.parent
                );
                let e = t.processedMeshes.find((i) => {
                  var e;
                  return null == (e = s.jointMaps)
                    ? undefined
                    : e.every((e, t) =>
                        e.every(
                          (e, a) => i.jointMaps && i.jointMaps[t][a] === e
                        )
                      );
                });
                ...
                var o =
                  ...
                  Array(r)
                    .fill(0)
                    .map((e, a) => Math.min(a, _materials_length - 1));

                var n = _materials.slice();
                for (let e = 0; e < r; e++) {
                  _materials[e] = n[o[e]];
                }
              }
...
async function migrateParticleComponentModule(e) {
  for (const t of e.getSwapSpace().json ||
    (await fs_extra_1.readJSON(e.source))) {
...
async function migrateComponentNames(r_type) {
  var i =
    r_type.getSwapSpace().json || (await fs_extra_1.readJSON(r_type.source));
  ...
  try {
    for (const r of i) {
      if (
        ...
        !utils_2.getComponent(i, r.node.__id__, widgetRE) &&
...
async function migrateCanvasCamera(e) {
  ...
  if (_times === 0) {
    ...
    var a = path_1.join(
      __dirname,
      "../../../../static/migrate-scene-canvas.ts"
    );

    var t = path_1.join(Editor.Project.path, "./assets/migrate-canvas.ts");
    await fs_extra_1.copyFile(a, t);
  ...
  try {
    for (const v of i) {
      if (v.__type__ === "cc.Camera") {
        for (let e = 20; e > 0; e--) {
          var o = 1 << e;

          if (!_maskMap.has(o) && v._visibility & o) {
            v._visibility ^= o;
          }
        }
      }
    ...
    let t = 0;
    for (const h of i) {
      if (h.__type__ === "cc.Canvas") {
        var _ = 20 - ++a >= _maskMap.size;
        ...
        var s = i[h.node.__id__];

        for (let e = 20 - a; e >= 0; e--) {
          if (!_maskMap.has(1 << e) || !_) {
            t = 1 << e;
            n = "canvas_" + e;
            s._layer = t;

            if (!r.find((e) => e.value === t)) {
              r.push({ name: n, value: t });
        ...
        if (h.node) {
          preChildSet(i, h.node, t);
          var i_length = i.length;
          var l = JSON.parse(getNodeStr());
          l[0]._name = "UICamera_" + i[h.node.__id__]._name;
          l[0]._parent = h.node;
          l[0]._layer = t;
          l[0]._id = Editor.Utils.UUID.generate();
          i[h.node.__id__]._children.push({ __id__: i_length });
          var d;
          ...
          y[0]._priority = getViewPriority(h, h._priority);
          y[0]._targetTexture = h._targetTexture;
          y[0]._clearFlags = h._clearFlag !== undefined ? h._clearFlag : 6;

          y[0]._color =
            h._color !== undefined
              ? JSON.parse(JSON.stringify(h._color))
          ...
          h._cameraComponent = {};
          h._cameraComponent.__id__ = p ? i_length + 2 : i_length + 1;
          h._alignCanvasWithScreen = true;
          i.push(l[0]);
...
async function migrateGeometryCurveCurveRange330(e) {
  ...
  var a =
    (await migrate_geometry_curve_3_3_0_1.migrateGeometryCurve330(a),
    await migrate_curve_range_3_3_0_1.migrateCurveRange330(a),
    a.get());
...
async function migrateShadowInfo(e) {
  let a = 4096;
  let t = 8;
  for (const n of e.getSwapSpace().json ||
    (await fs_extra_1.readJSON(e.source))) {
    if (
      n.__type__ === "cc.ShadowsInfo" &&
      (n._shadowColor && (n._saturation = n._shadowColor.a / 255), n._size)
    ) {
      var n_size = n._size;
      var r = cc.math.absMax(n_size.x, n_size.y);
      for (let e = 8; e < 12; e++) {
      ...
      n._size = { __type__: "cc.Vec2", x: 1 << t, y: 1 << t };
    }
  }
}
...
async function migrateShadowAutoAdapt(e) {
  var a;
  var t;
  for (const i of e.getSwapSpace().json ||
    (await fs_extra_1.readJSON(e.source))) {
...
async function migrateHDRData(e) {
  ...
  var s;
  var c = 1 / 38400;
  for (const l of e.getSwapSpace().json ||
    (await fs_extra_1.readJSON(e.source))) {
...
async function migrateSkyLightingTypeData(e) {
  for (const r of e.getSwapSpace().json ||
    (await fs_extra_1.readJSON(e.source))) {
...
async function migrateMaskImageStencil(e) {
  ...
  var i;
  var r;
  var o = e.getSwapSpace().json || (await fs_extra_1.readJSON(e.source));
  for (const n of o) {
    if (n.__type__ === "cc.Mask") {
      (a = n)._type === 3 &&
        a._spriteFrame !== null &&
        ((t = o[a.node.__id__]),
