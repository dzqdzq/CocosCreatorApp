...
function compress(e, r, s) {
  for (const a of s) {
    switch (a.type) {
      case "remove-linear-keys":
        {
          ({ keys: e, values: r } = removeLinearKeys(e, r, a.maxDiff));
          break;
        }
      case "remove-trivial-keys":
        {
          ({ keys: e, values: r } = removeTrivialKeys(e, r, a.maxDiff));
        }
    }
  }
  return { times: e, values: r };
}
function removeLinearKeys(r, s, a = 0.001) {
  var e = r.length;
  if (e < 3) {
    return { keys: r.slice(), values: s.slice() };
  }
  ...
  return filterFromRemoveFlags(r, s, o);
}
function removeTrivialKeys(e, r, s = 0.001) {
  var a = e.length;
  if (a < 2) {
    return { keys: e.slice(), values: r.slice() };
  }
  var o = new Array(a).fill(!1);
  for (let e = 1; e < a; ++e) {
    var i = e - 1;
    var { [i]: i, [e]: l } = r;
    approx(i, l, s) && (o[e] = !0);
...
function filterFromRemoveFlags(s, a, o) {
  var i = s.length;
