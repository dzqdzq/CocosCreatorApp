...
function getDependUUIDList(t, i) {
  if (typeof t != "string") {
  ...
  }
  {
    ...
    return (
      (e =
        e &&
        JSON.parse(
          JSON.stringify(Array.from(new Set(e)).filter((e) => e !== i))
        )) || []
    );
  }
}
function getDependList(e) {
  ...
  return typeof e == "string"
    ...
    : {
        uuids: (i = getDeserializeResult(e)).uuids,
        dependScriptUuids: i.dependScriptUuids,
      };
}
...
function getDeserializeResult(e) {
  ...
  t.reset();
  const i = EditorExtends.MissingReporter.classInstance;
  i.reset();
  i.hasMissingClass = false;
  const r = new Set();
  e = cc.deserialize(e, t, {
    classFinder(e) {
      if (Editor.Utils.UUID.isUUID(e)) {
        r.add(Editor.Utils.UUID.decompressUUID(e));
      }

      return i.classFinder(e);
    },
  });
  ...
  return {
    instance: e,
    uuids: t.uuidList,
    dependScriptUuids: Array.from(r),
    classFinder: i.classFinder,
  };
}
function i18nTranslate(e, ...t) {
  let i = electron_i18n_1.default.translation(e);
  if (typeof t[0] == "object") {
    var [r] = t;
    var e = i.match(/{(\w+)}/g);
    if (e) {
      for (const s of e) {
        var n = s.substr(1, s.length - 2);
        i = i.replace(s, r[n]);
      }
    }
  }
...
function getPixiel(e, t, i, r) {
  t = 4 * t + i * r * 4;
  return { r: e[t], g: e[1 + t], b: e[2 + t], a: e[3 + t] };
}
function getTrimRect(e, t, i, r) {
  var n = r;
  let s = t;
  let o = i;
  let a = 0;
...
function removeNull(e, t) {
  let i = false;
  for (const s of e) {
    if (s._children && s._children.length) {
      for (let e = 0; e < s._children.length; e++) {
        var r = s._children[e];

        if (!r) {
          s._children.splice(e, 1);
          i = true;
    ...
    if (s._components) {
      for (let e = 0; e < s._components.length; e++) {
        var n = s._components[e];

        if (!n) {
          s._components.splice(e, 1);
