...
const textureFuncRemap = new Map([
    ['ExternalOES', '2D'],
]);
...
const options = {
    throwOnError: true, throwOnWarning: false, noSource: false, skipParserTest: false,
...
const convertType = (t) => { const tp = mappings.typeMap[t]; return tp === undefined ? t : tp; };
        ...
        while (defineCapture !== null) {
            ...
            if (new RegExp('\\b' + fnName + '\\b').test(fnBody)) { warn(`EFX2002: recursive macro processor '${fnName}'`); }
                ...
                for (let macroCapture = macroRE.exec(code); macroCapture !== null; macroCapture = macroRE.exec(code)) {
                    ...
                    if (!definePrefixRE.test(prefix)) { // for top level invocations
...
const expandInputStatement = (statements) => {
    ...
    for (const statement of statements) {
        ...
        let macroDepthStencilIn = 
        `\n` +
        `#pragma rate ${inputPrefix}${name} pass\n` +
        `#if CC_DEVICE_CAN_BENEFIT_FROM_INPUT_ATTACHMENT\n` +
        `  #if __VERSION__ >= 450\n` +
        `    layout(input_attachment_index = ${gl4Index}) uniform ${varType[1]} ${inputPrefix}${name};\n` +
        ...
        `#if CC_DEVICE_CAN_BENEFIT_FROM_INPUT_ATTACHMENT\n` +
        `  #if __VERSION__ >= 450\n` +
        `    layout(input_attachment_index = ${gl4Index}) uniform subpassInput ${inputPrefix}${name};\n` +
        `    #define subpassLoad_${name} subpassLoad(${inputPrefix}${name})\n` +
        `  #elif __VERSION__ >= 300\n` +
...
const expandSubpassInout = (code) => {
    const inputStatements = new Array();
    ...
    while (defineCapture !== null) {
        let signed = defineCapture[1] ? defineCapture[1] : 'f';
        let input = defineCapture[2];
        let inout = defineCapture[3];
        let precision = defineCapture[4];
        let name = defineCapture[5];

        let filter = FilterMap[input];
    ...
    let subpassReplace = subpassReplaceRE.exec(code);
    if (subpassReplace) {
    ...
    return (code, entry, functions) => {
        ...
        for (const cur of getAllCaptures(code, scopeRE)) {
            ...
            if (c === '}' && --depth === 0) {
                ...
                state = 0;
                if (name) { functionsFull.push({ name, beg, end, paramListEnd, deps: [] }); }
            }
        }
        // inspection
...
const parseCustomLabels = (arr, out = {}) => {
    ...
    while (labelCap) {
        try { out[labelCap[1]] = yaml.load(labelCap[2] || 'true'); }
...
const pushDefines = (defines, existingDefines, newDefine) => {
    if (existingDefines.has(newDefine.name)) { return; }
...
const extractDefines = (tokens, defines, cache) => {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (str[0] === '#endif') { // pop one level up
            ...
            def && def.forEach((d, i) => def[i] = (d[0] === '!' ? d.slice(1) : ('!' + d))); save(t.line);
            if (str[0] === '#else') { continue; } elifClauses++;
        } else if (str[0] === '#pragma') { // pragmas
            if (str.length <= 1) { continue; }
            if (str[1] === 'define-meta') { // define specifications
                ...
                if (!def) { pushDefines(defines, cache.existingDefines, def = { name: str[2], type: 'boolean', defines: d, dummyDependency: true }); }
                ...
                for (const key in prop) {
                    if (key === 'range') { // number range
                        ...
                        switch (prop.default) {
                            case true: def.default = 1; break;
                            case false: def.default = 0; break;
                            default: def.type = 'constant'; def.default = prop.default; def.fixedType = true; break;
                        }
                    } else if (key === 'editor') {
                        def.editor = prop.editor;
                    } else {
                        warn(`EFX2105: define pragma: illegal label '${key}'`, t.line); continue;
                    }
                }
        ...
        str.splice(1).some(s => {
            ...
            if (id) { // is identifier
                if (id[0] === 'defined' // skip macros that can be undefined
                    ...
                    || id[0].startsWith('GL_') || id[0] === 'VULKAN') { return false; }
                ...
                else { pushDefines(defines, cache.existingDefines, df = { name: id[0], type: 'boolean', defines: d }); }
                defs.push((s[0] === '!' ? '!' : '') + id[0]);
...
const extractParams = (() => {
    ...
    const nextWord = (tokens, i) => { do { ++i; } while (exMap[tokens[i].type]); return i; };
    ...
    return (tokens, cache, shaderInfo, stage, functions) => {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            if (str === 'uniform') {
                ...
                if (stage === 'compute') { // compute shader local_size definition, skipped
            ...
            if (tokens[idx].data !== '{') {
                ...
                if (dest === shaderInfo.blocks) { // samplerTextures
                    const uType = tokens[i + (param.precision ? 4 : 2)].data;
                    const uTypeCap = nonBlockUniforms.exec(uType);
                    if (!uTypeCap) { error('EFX2201: vector uniforms must be declared in blocks.', t.line); }
                    else if (uType === 'sampler') { dest = shaderInfo.samplers; type = 'samplers'; }
                    else if (uTypeCap[0] === 'sampler') { dest = shaderInfo.samplerTextures; type = 'samplerTextures'; }
                    else if (uTypeCap[0] === 'texture') { dest = shaderInfo.textures; type = 'textures'; }
                    else if (uTypeCap[0] === 'image') { dest = shaderInfo.images; type = 'images'; }
                    else if (uTypeCap[0] === 'subpassInput') { dest = shaderInfo.subpassInputs; type = 'subpassInputs'; }
                ...
                while (tokens[idx = nextWord(tokens, idx)].data !== '}') {
                    if (dest !== shaderInfo.buffers) { // don't need to parse SSBO members
                        const info = extractInfo(tokens, idx);
                        if (mappings.isSampler(info.type)) { error('EFX2208: texture uniforms must be declared outside blocks.', tokens[idx].line); }
                ...
                param.members.reduce((acc, cur) => {
                    ...
                    switch (cur.typename) {
                        case 'mat2': baseAlignment /= 2; break;
                        case 'mat3': baseAlignment /= 3; break;
                        case 'mat4': baseAlignment /= 4; break;
                    }
            ...
            if (dest === shaderInfo.buffers || dest === shaderInfo.images) {
                ...
                if (/writeonly|readonly/.test(tokens[i - 2].data)) { beg = i - 2; }
            }
...
const miscChecks = (() => {
    ...
    return (code) => {
        ...
        if (cap) {
            if (/#extension/.test(code.slice(cap.index))) { warn('EFX2400: precision declaration should come after extensions'); }
...
const finalTypeCheck = (() => {
    ...
    const getDefineString = (defines) => defines.reduce((acc, cur) => {
        ...
        switch (cur.type) {
            ...
            case 'boolean': value = cur.default === undefined ? 1 : cur.default; break;
        }
    ...
    return (code, version, extensions, isVert) => {
        if (version < 310) { // keep std140 declaration, discard others
            code = code.replace(/layout\s*\((.*?)\)(\s*)(\w+)\s+(\w+)/g, (_, tokens, trailingSpaces, type, uType) => {
                ...
                if (type !== 'out' && type !== 'in' && type !== 'uniform') { return _; } // keep Storage Buffer bindings
                if (type === 'uniform' && uType.includes('image')) { return _; } // keep Storage Image bindings
...
const glsl300to100 = (code, blocks, defines, paramInfo, functions, cache, vert) => {
    ...
    paramInfo.forEach((i) => {
        ...
        blocks.find((u) => u.name === i.param.name).members.forEach((m) => {
...
const decorateBlockMemoryLayouts = (code, paramInfo) => {
    ...
    paramInfo.forEach((info, paramIdx) => {
        if (info.type !== 'blocks' && info.type !== 'buffers') { return; }
    ...
    paramInfo.forEach((info, paramIdx) => {
        ...
        // insert declarations
        if (info.type === 'blocks') { // UBO-specific
        ...
        } else if (info.type === 'buffers') { // SSBO-specific
...
const decorateBindings = (code, manifest, paramInfo) => {
    ...
    paramInfo.forEach((info, paramIdx) => {
        ...
        if (info.type === 'fragColors') { return; }
        ...
        if (cap) {
            ...
            if (bindingCap) {
                if (cap[1].search(/\bset\s*=/) < 0) { layoutInfo.position = cap[1].length - layoutInfo.position; } // should insert set declaration
                else { layoutInfo.position = -1; } // indicating no-op
                ...
                if (!validSubstitution && info.type === 'subpassInputs') { // input attachments need fallback bindings, skip this check
                    validSubstitution = true;
                }
                if (validSubstitution) { // auto-generated binding is guaranteed to be consecutive
    ...
    paramInfo.forEach((info, paramIdx) => {
        ...
        if (info.type === 'attributes') {
            ...
            if (name in category) { record[paramIdx].prop[dest] = category[name]; }
            else {
            ...
            if (name in category) {
                ...
                if (substitute) { substitute[dest] = oldLocation; }
    ...
    paramInfo.forEach((info, paramIdx) => {
        ...
        if (position === undefined) { // no qualifier, just insert everything
            ...
            res += code.slice(position, info.beg);
        } else if (position < -1) { // binding exists, but no set specified
            res += code.slice(idx, -position);
            res += setDeclaration;
            res += code.slice(-position, info.beg);
    ...
    manifest.samplerTextures = manifest.samplerTextures.filter((t) =>
        manifest.subpassInputs.findIndex((s) => s.binding === t.binding) < 0);
    return res;
};
...
const shaderFactory = (() => {
    ...
    const assignRate = (entry, rates) => {
        entry.forEach((i) => {
            ...
            if (rate) { i.rate = rateMapping[rate.rate]; }
        });
    };
    const tokenizerOpt = { version: '300 es' };
    const createShaderInfo = () => ({
        blocks: [], samplerTextures: [], samplers: [], textures: [], buffers: [], images: [],
        subpassInputs: [], attributes: [], varyings: [], fragColors: [], descriptors: []
    });
    const compile = (name, stage, outDefines = [], shaderInfo = createShaderInfo(), chunks = globalChunks, deprecations = globalDeprecations) => {
        ...
        shaderInfo.samplerTextures = shaderInfo.samplerTextures.filter(ele =>
            !shaderInfo.subpassInputs.find(obj => obj.name === ele.name)
        );
        ...
        shaderInfo.attributes.forEach((attr) => { remapDefine(attr, substituteMap); });
        shaderInfo.blocks.forEach((block) => { remapDefine(block, substituteMap); });
        shaderInfo.buffers.forEach((buffer) => { remapDefine(buffer, substituteMap); });
        shaderInfo.images.forEach((image) => { remapDefine(image, substituteMap); });
        shaderInfo.samplerTextures.forEach((samplerTexture) => { remapDefine(samplerTexture, substituteMap); });
        shaderInfo.samplers.forEach((sampler) => { remapDefine(sampler, substituteMap); });
        shaderInfo.textures.forEach((texture) => { remapDefine(texture, substituteMap); });
        ...
        if (stage == 'vert' || stage == 'frag') { // glsl1 only supports vert and frag
    ...
    const build = (stageNames, type, chunks = globalChunks, deprecations = globalDeprecations) => {
        ...
        shaderInfo.blocks.forEach((b) => {
            ...
            if (b.stageFlags & CSBit) { csUniformVectors += vectors; }
        }, 0);
        if (type === 'graphics') {
        ...
        shaderInfo.descriptors[0] = { rate: 0, blocks: [], samplerTextures: [], samplers: [], textures: [], buffers: [], images: [], subpassInputs: [] };
        shaderInfo.descriptors[1] = { rate: 1, blocks: [], samplerTextures: [], samplers: [], textures: [], buffers: [], images: [], subpassInputs: [] };
        shaderInfo.descriptors[2] = { rate: 2, blocks: [], samplerTextures: [], samplers: [], textures: [], buffers: [], images: [], subpassInputs: [] };
        shaderInfo.descriptors[3] = { rate: 3, blocks: [], samplerTextures: [], samplers: [], textures: [], buffers: [], images: [], subpassInputs: [] };
        ...
        if (type === 'graphics') {
            ...
            hash = mappings.murmurhash2_32_gc(
                glsl4.vert + glsl4.frag + glsl4.compute +
                glsl3.vert + glsl3.frag + glsl3.compute +
                glsl1.vert + glsl1.frag, 666);
        }
    ...
    const structuralTypeCheck = (ref, cur, path = 'effect') => {
        if (Array.isArray(ref)) {
            ...
            for (const key of Object.keys(cur)) {
                if (key.indexOf(':') !== -1) { error(`EFX1004: syntax error at '${key}', you might need to insert a space after colon`); }
            }
...
const mapPassParam = (() => {
    ...
    const mapTarget = (target, shader) => {
        ...
        if (swizzle.split('').map((c, idx) => channelMap[c] - beginning - idx).some((n) => n)) { error(`EFX3304: '${target}': random component swizzle is not supported`); }
        handleInfo[0] = cap[1];
    ...
    const mapSampler = (obj) => {
        for (const key of Object.keys(obj)) {
            if (samplerInfo[key] === undefined) { warn(`EFX3301: illegal sampler info '${key}'`); }
        }
    ...
    const mapPriority = (str) => {
        ...
        if (cap[1]) { res = mappings.RenderPriority[cap[1].toUpperCase()]; }
        if (cap[3]) { res += parseInt(cap[3]) * (cap[2] === '-' ? -1 : 1); }
    ...
    const mapDSS = (dss) => {
        ...
        if (dss.stencilWriteMaskFront !== dss.stencilWriteMaskBack) { warn('EFX3100: WebGL(2) doesn\'t support inconsistent front/back stencil write mask'); }
        if (dss.stencilReadMaskFront !== dss.stencilReadMaskBack) { warn('EFX3101: WebGL(2) doesn\'t support inconsistent front/back stencil read mask'); }
        if (dss.stencilRefFront !== dss.stencilRefBack) { warn('EFX3102: WebGL(2) doesn\'t support inconsistent front/back stencil ref'); }
...
const stageValidation = (stages) => {
    ...
    stages.forEach((stage) => { // validation: all stages must have the same pass type
        if (!passMap[stage]) {
