...
const {
  defaultConfigKey,
} = require("../dist/contributions/project/modules/default-config");
...
describe("query-engine-modules-profile 接口测试", () => {
  describe("常规返回值测试", async () => {
    it("检查消息接口的返回值是否符合要求", async () => {
      ...
      const defaultProfileNoParam = await Editor.Message.request(
        "engine",
    ...
    it("获取默认配置（使用 defaultConfigKey）", async () => {
      const defaultProfileWithKey = await Editor.Message.request(
        "engine",
        "query-engine-modules-profile",
        defaultConfigKey
      );
    ...
    it("传入一个不存在的 key 得到空值", async () => {
      const otherProfile = await Editor.Message.request(
        "engine",
        "query-engine-modules-profile",
        "otherKey"
      );
  ...
  if (Editor.Project.name === "build-example") {
    describe("获取默认的全局配置需要过滤 physics-2d-box2d-jsb", async () => {
      const moduleRes = await Editor.Message.request(
        "engine",
        "query-engine-modules-profile"
      );
      it("需要过滤 physics-2d-box2d-jsb", () => {
        expect(!moduleRes.includeModules.includes("physics-2d-box2d-jsb")).to
          .true;
      });
      console.debug(moduleRes.includeModules);
      it("physics-2d-box2d-jsb 需要替换成 physics-2d-box2d-wasm", () => {
        expect(moduleRes.includeModules.includes("physics-2d-box2d-wasm")).to
          .true;
      });
      it("moduleToFallBack 需要记录 physics-2d-box2d-wasm", () => {
        expect(
          moduleRes.moduleToFallBack["physics-2d-box2d-jsb"] ===
    ...
    describe("支持传递环境参数查询全局配置", async () => {
      ...
      it("需要过滤 gfx-webgl gfx-webgl2 gfx-webgpu 等模块", () => {
        expect(
          moduleRes.includeModules.every((module) => !module.includes("gfx"))
        ).to.true;
      });
    });
  }
...
describe("filter-engine-modules 接口测试", () => {
  it("HTML5 不支持 physics-2d-box2d-jsb 模块", async () => {
    ...
    const { includeModules: includeModules_1, moduleToFallBack } =
    ...
    expect(includeModules_1).to.include("physics-2d-box2d-wasm");
    expect(includeModules_1).to.not.include("physics-2d-box2d-jsb");
    expect(moduleToFallBack["physics-2d-box2d-jsb"]).to.equal(
      "physics-2d-box2d-wasm"
    );
  });
  ...
  it("NATIVE 不支持 gfx-webgl | gfx-webgl2 | gfx-webgpu 模块", async () => {
    ...
    expect(includeModules_1).to.not.include("gfx-webgpu");
    expect(includeModules_1).to.not.include("gfx-webgl");
    expect(includeModules_1).to.not.include("gfx-webgl2");
    expect(includeModules_1).to.include("base");
