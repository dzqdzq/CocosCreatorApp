...
describe("1.0.11", async () => {
  ...
  it("验证项目迁移完成", () => {
    ...
    expect(_clone.modules.configs.migrationsConfig.includeModules).to.be.an(
      "array"
    );
    expect(_clone.modules.configs.migrationsConfig.flags).to.be.an("object");
    expect(
      _clone.modules.configs.migrationsConfig.noDeprecatedFeatures
    ).to.be.an("object");
  });
...
describe("1.0.12", async () => {
  describe("测试关闭 2d ", async () => {
    ...
    _clone.modules.includeModules.splice(
      _clone.modules.includeModules.indexOf("2d"),
      1
    ); // 删除2d
    const {
      migrateProject: migrateProject_1,
    } = require("./../dist/migrations/1.0.11");
    await migrateProject_1(_clone);
    ...
    it("验证 2d 的迁移模块不被勾选", () => {
      expect(migrationsConfig.includeModules).to.not.include.members([
        "rich-text",
        "mask",
        "graphics",
        "affine-transform",
  ...
  describe("测试开启 2d", async () => {
    ...
    it("验证 2d 的迁移模块被勾选", () => {
      expect(migrationsConfig.includeModules).to.include.members([
        "rich-text",
        "mask",
        "graphics",
        "affine-transform",
    ...
    it("物理的选项有配置", () => {
      ["physics-2d-box2d-wasm", "physics-ammo", "physics-physx"].forEach(
        (physics) => {
  ...
  describe("测试关闭 spine", async () => {
    ...
    _clone.modules.includeModules.splice(
      _clone.modules.includeModules.indexOf("spine"),
      1
    ); // 删除2d
