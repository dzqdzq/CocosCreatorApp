...
describe("compressUuid", () => {
  ...
  it(normalUuid + subUuidSuffix, () => {
    expect(uuidTool.compressUuid(normalUuid + subUuidSuffix)).to.equal(
      normalCompressedUuid + subUuidSuffix
    );
  });
  it(normalUuid + disabledMipmapSuffix, () => {
    expect(uuidTool.compressUuid(normalUuid + disabledMipmapSuffix)).to.equal(
      normalUuid + disabledMipmapSuffix
    );
  });
  it(normalUuid + subUuidSuffix + subUuidSuffix, () => {
    expect(
      uuidTool.compressUuid(normalUuid + subUuidSuffix + subUuidSuffix)
    ).to.equal(normalCompressedUuid + subUuidSuffix + subUuidSuffix);
  });
});
...
describe("decompressUuid", () => {
  ...
  it(normalCompressedUuid + subUuidSuffix, () => {
    expect(
      uuidTool.decompressUuid(normalCompressedUuid + subUuidSuffix)
    ).to.equal(normalUuid + subUuidSuffix);
  });
  it(normalCompressedUuid + subUuidSuffix + subUuidSuffix, () => {
    expect(
      uuidTool.decompressUuid(
        normalCompressedUuid + subUuidSuffix + subUuidSuffix
      )
    ).to.equal(normalUuid + subUuidSuffix + subUuidSuffix);
  });
  it(normalCompressedUuid + mipmapSuffix, () => {
    expect(
      uuidTool.decompressUuid(normalCompressedUuid + mipmapSuffix)
    ).to.equal(normalUuid + mipmapSuffix);
  });
});
...
describe("isUuid", () => {
  ...
  it(normalUuid + subUuidSuffix + subUuidSuffix, () => {
    expect(uuidTool.isUuid(normalUuid + subUuidSuffix + subUuidSuffix)).to.be
      .true;
  });
  it(normalUuid + disabledMipmapSuffix, () => {
...
describe("getUuidFromLibPath", () => {
  it(`c:/xxx/5b/${normalUuid}/SmallCaps.ttf`, () => {
    expect(
      uuidTool.getUuidFromLibPath(`c:/xxx/5b/${normalUuid}/SmallCaps.ttf`)
    ).to.equal(normalUuid);
  });
  it(`c:/xxx/5b/${normalUuid}.png`, () => {
    expect(uuidTool.getUuidFromLibPath(`c:/xxx/5b/${normalUuid}.png`)).to.equal(
      normalUuid
    );
  });
  it(`c:/xxx/5b/${normalUuid + subUuidSuffix}.json`, () => {
    expect(
      uuidTool.getUuidFromLibPath(
        `c:/xxx/5b/${normalUuid + subUuidSuffix}.json`
      )
    ).to.equal(normalUuid + subUuidSuffix);
  });
  it(`c:/xxx/5b/${normalUuid + subUuidSuffix + subUuidSuffix}.json`, () => {
    expect(
      uuidTool.getUuidFromLibPath(
        `c:/xxx/5b/${normalUuid + subUuidSuffix + subUuidSuffix}.json`
      )
    ).to.equal(normalUuid + subUuidSuffix + subUuidSuffix);
  });
  it(`c:/xxx/5b/${normalUuid + disabledMipmapSuffix}.pkm`, () => {
    expect(
      uuidTool.getUuidFromLibPath(
        `c:/xxx/5b/${normalUuid + disabledMipmapSuffix}.pkm`
      )
    ).to.not.equal(normalUuid + disabledMipmapSuffix);
  });
});
