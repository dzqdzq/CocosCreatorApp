...
import { StatsQuery } from '@cocos/build-engine';
...
export async function compileEngine(directory: string, force?: boolean) {
    ...
    /** 由 quick compiler 导致的产物 */
    const quickCompilerDist = [join(outDir, 'editor'), join(outDir, 'preview')];
    // 发布之后不需要编译内置引擎
    // 开始第一次编译引擎
    ...
    if (isNativeScene) {
        ...
        quickCompilerDist.push(join(engineDir, 'bin/.editor'));
    }
    if (needClear) {

        console.debug('[EditorQuickCompiler]Version information lost.');
        await Promise.all(quickCompilerDist.map(item => {
            if (existsSync(item)) {
...
async function generateEngineAddon(options?: Pick<IRebuildOptions, 'debugNative'>) {
    ...
    const cmakeJsPath = join(__dirname, '../node_modules/cmake-js/bin/cmake-js');
...
async function getIsSceneNative() {
    ...
    if (Editor) {
        ...
        isNativeScene = await Editor.Profile.getConfig('scene', 'scene.native-engine') ?? false;
    }
