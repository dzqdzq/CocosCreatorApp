Object.defineProperty(exports, "__esModule", { value: true });
exports.BuildGlobalInfo = undefined;

const { join } = require("path");

exports.BuildGlobalInfo = {
  LIBRARY_NAME: "library",
  IMPORT_HEADER: "import",
  RESOURCES: "resources",
  SUBPACKAGES_HEADER: "subpackages",
  ASSETS_HEADER: "assets",
  REMOTE_HEADER: "remote",
  NATIVE_HEADER: "native",
  BUNDLE_SCRIPTS_HEADER: "bundle-scripts",
  SCRIPT_NAME: "index.js",
  CONFIG_NAME: "config.json",
  BUNDLE_ZIP_NAME: "res.zip",
  buildTemplateDir: join(Editor.Project.path, "build-templates"),
  projectTempDir: join(Editor.Project.tmpDir, "builder"),
  globalTempDir: join(Editor.App.temp, "builder"),
  debugMode: false,
  init: false,
  isCommand: false,
  buildOptionsFileName: "cocos.compile.config.json",
};
