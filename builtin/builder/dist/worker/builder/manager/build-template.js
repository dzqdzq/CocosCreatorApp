...
exports.BuildTemplate = undefined;

const { existsSync, readJSON, copy, remove } = require("fs-extra");
...
class BuildTemplate {
  ...
  async _initVersion(platform) {
    ...
    try {
      // 默认构建模板需要有版本号
      const templateVersionJson = join(
        Build.buildTemplateDir,
      ...
      if (
        ...
        console.warn(
          Editor.I18n.t("builder.tips.templateVersionWarning", {
  ...
  async copyTo(dest) {
    ...
    // 移除已经被处理的一些特殊的文件夹
    await Promise.all(
      Object.values(this.map).map((info) => {
