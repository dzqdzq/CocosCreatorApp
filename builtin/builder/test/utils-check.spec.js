...
const {
  checkStartScene,
  checkConflict,
  generateNewOutputName,
  handleOverwriteProjectSettings,
} = require("./../dist/share/common-options-validator");
const {
  filterAssetWithBundleConfig,
} = require("../dist/worker/builder/utils/bundle");
const { join } = require("path");

describe("构建工具函数的基本使用", () => {
  describe("验证模块覆盖选项", () => {
    const testIncludeModules = [
      "gfx-webgl2",
      "gfx-webgl1",
      "physics-physx",
      "physics-2d-box2d",
    ...
    it("单独配置开启", () => {
      ...
      handleOverwriteProjectSettings(options);
      expect(options.includeModules).to.have.members([
        "gfx-webgl1",
        "gfx-webgl2",
        "physics-ammo",
        "physics-2d-builtin",
  ...
  describe("构建输出目录校验工具", () => {
    ...
    const buildPathDict = {
      [buildPath]: ["web-mobile-002", "web-desktop", "web-desktop-001"],
    };
    ...
    it("生成新的构建输出目录", () => {
      const outputName = generateNewOutputName(
        buildPath,
        platform,
        buildPathDict
      );
  ...
  describe("checkAssetWithFilterConfig", () => {
    ...
    it("asset include empty 过滤无效", () => {
      const res = filterAssetWithBundleConfig(testAssets, [
