...
describe('自动图集预览生成功能测试', () => {
    ...
    describe('自动图集合图生成与预览接口测试', async () => {
        ...
        it('正常返回值', async () => {
            const allKeys = ['atlasImagePaths', 'dirty', 'unpackedImages', 'atlases'];
    ...
    describe('自动图集嵌套的预览接口测试', (async () => {
        // db://assets/sheep-subPackage/sheep.pac
        const testSubPacUuid = 'c1d5901d-542d-4316-ad85-066d763d473b';
        const result = await Editor.Message.request('builder', 'preview-pac', testSubPacUuid);
        it('参与此自动图集的 sprite 需要过滤子目录下其他图集的 sprite', async () => {
            expect(result.storeInfo.sprites.length).to.equal(4);
        });
        it('参与此自动图集的 sprite 需要包含其他子目录下非图集目录的 sprite', async () => {
            expect(result.storeInfo.sprites.every((sprite) => sprite.name.startsWith('sheep_'))).to.be.true;
        });
    }));
...
describe('查询小图对应自动图集接口', async () => {
    if (Editor.Project.name !== 'build-example') {
        it('查询 assets 下资源', async () => {
            const res = await Editor.Message.request('builder', 'request-to-build-worker', 'build-worker:query-atlas-by-sprite', '0a2f8577-9298-44ff-93d0-8a055f252cb0@f9941');
            expect(res.uuid).to.equal('474408c2-fb2d-45b7-9f90-b42ac2fae5df');
        });
    }

    it('查询 db 下的资源', async () => {
        const res = await Editor.Message.request('builder', 'request-to-build-worker', 'build-worker:query-atlas-by-sprite', '951249e0-9f16-456d-8b85-a6ca954da16b@f9941');
