...
const { checkBuildCommonOptionsByKey, getCommonOptionDefaultByKey, checkProjectSetting } = require('../dist/share/common-options-validator');
...
describe('构建通用配置验证测试', async () => {
    ...
    describe('buildPath 路径验证测试', () => {
        ...
        it('buildPath 项目路径无效', async () => {
            const result1 = await pluginManager.checkCommonOptionByKey('buildPath', `project://${Editor.App.path.replace(/\\/g, '/')}`, options);
        ...
        it('buildPath 绝对路径无效', async () => {
            const result1 = await pluginManager.checkCommonOptionByKey('buildPath', Editor.App.path, options);
    ...
    describe('startScene 验证测试', () => {
        ...
        it('内置场景模板 3b7550e2-beea-4079-ad8d-dfef104086bd 不能作为初始场景', async () => {
            const result = await checkBuildCommonOptionsByKey('startScene', '3b7550e2-beea-4079-ad8d-dfef104086bd');
            expect(result.error).to.not.equal('');
            expect(result.newValue).to.not.equal('3b7550e2-beea-4079-ad8d-dfef104086bd');
        });
    ...
    describe('checkProjectSetting', () => {
        ...
        describe('保留用户传递的有效配置', async () => {
            ...
            it('验证 includeModules 的完整性', () => {
                expect(res.includeModules).to.be.deep.equal(['base', 'physics-2d-box2d-jsb']);
            });
        ...
        describe('剔除不需要的配置', async () => {
            ...
            it('验证替换掉 web-mobile 平台的 physics-2d-box2d-jsb 模块', () => {
                expect(res.includeModules).to.be.deep.equal(['base', 'physics-2d-box2d-wasm']);
            });
        });
    });
