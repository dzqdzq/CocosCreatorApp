...
const {
  checkBuildCommonOptionsByKey,
  getCommonOptionDefaultByKey,
  checkProjectSetting,
} = require("../dist/share/common-options-validator");
...
describe("构建通用配置验证测试", async () => {
  ...
  describe("name 名称验证测试", () => {
    it("name 不能为空", async () => {
      const result = await pluginManager.checkCommonOptionByKey(
        "name",
        "",
        optionBase
      );
  ...
  describe("outputName 名称验证测试", () => {
    ...
    it("outputName 原生平台支持路径符号、空格、下划线、中划线、字母、数字", async () => {
      const result = await pluginManager.checkCommonOptionByKey(
        "outputName",
        "project://build/ios-001/output name/path_01",
        options
      );
      expect(result.error).to.equal("");
    });
  ...
  describe("buildPath 路径验证测试", () => {
    ...
    it("buildPath 项目路径无效", async () => {
      const result1 = await pluginManager.checkCommonOptionByKey(
        "buildPath",
        `project://${Editor.App.path.replace(/\\/g, "/")}`,
        options
      );
      expect(result1.error).to.not.equal("");
      const result2 = await pluginManager.checkCommonOptionByKey(
        "buildPath",
        "project://../test",
        options
      );
      expect(result2.error).to.not.equal("");
      const result3 = await pluginManager.checkCommonOptionByKey(
        "buildPath",
        "project:///build",
        options
      );
      expect(result3.error).to.not.equal("");
      const result4 = await pluginManager.checkCommonOptionByKey(
        "buildPath",
        "project://./build",
        options
      );
      expect(result4.error).to.equal("");
      const result5 = await pluginManager.checkCommonOptionByKey(
        "buildPath",
  ...
  describe("platform 验证测试", () => {
    it("不支持的构建平台 newPlatform", async () => {
      const result = await checkBuildCommonOptionsByKey(
        "platform",
        "newPlatform"
      );
      expect(result.error).to.not.equal("");
  ...
  describe("startScene 验证测试", () => {
    ...
    it("内置场景模板 3b7550e2-beea-4079-ad8d-dfef104086bd 不能作为初始场景", async () => {
      const result = await checkBuildCommonOptionsByKey(
        "startScene",
        "3b7550e2-beea-4079-ad8d-dfef104086bd"
      );
      expect(result.error).to.not.equal("");
      expect(result.newValue).to.not.equal(
        "3b7550e2-beea-4079-ad8d-dfef104086bd"
      );
    });
    it("Bundle 内的场景不能作为首场景", async () => {
      const result = await checkBuildCommonOptionsByKey(
        "startScene",
        "7f5ba957-296e-4230-824c-46d9a65ec5f1"
      );
      expect(result.error).to.not.equal("");
    });
  });
  ...
  describe("scenes 验证测试", () => {
    ...
    it("场景数据混入不存在的资源要被剔除，剔除后的场景数据为空要报错", async () => {
      const result = await checkBuildCommonOptionsByKey("scenes", [
        { uuid: "ahisbfbj", url: "ad://asset/njjdks.scene" },
      ]);
      expect(result.error).to.not.equal("");
    });
    ...
    it("场景数据混入不存在的资源要被剔除", async () => {
      const assets = await Editor.Message.request("asset-db", "query-assets", {
        ...
        pattern: "!db://internal/default_file_content/**/*",
      });
  ...
  describe("check-and-complete-options options={ configPath, debug: false }", async () => {
    ...
    res = await Editor.Message.request(
      "builder",
  ...
  describe("checkOptions", async () => {
    ...
    await Editor.Profile.setConfig(
      "android",
      "builder.options.android.packageName",
      packageName
    );
  ...
  describe("checkProjectSetting", () => {
    describe("自动补全所有项目设置", async () => {
      const res = await checkProjectSetting({
        platformType: "HTML5",
      });
      const allKeys = [
      ...
      it(`验证结果的数据齐全`, () => {
        expect(res).to.include.keys(allKeys);
      });
      it("验证 includeModules 里的数值有效", () => {
        expect(res.includeModules.length).to.be.greaterThan(0);
      });
    });

    describe("保留用户传递的有效配置", async () => {
      const res = await checkProjectSetting({
        flags: { test: false },
        includeModules: ["base", "physics-2d-box2d-jsb"],
        platform: "ios",
        platformType: "IOS",
      });
      it("验证 includeModules 的完整性", () => {
        expect(res.includeModules).to.be.deep.equal([
          "base",
          "physics-2d-box2d-jsb",
        ]);
      });
      it("验证 flags 填写的值保留并且融合内置的 flags", () => {
        expect(res.flags.test).to.be.false;
      });
    });

    describe("剔除不需要的配置", async () => {
      const res = await checkProjectSetting({
        includeModules: ["base", "physics-2d-box2d-jsb"],
        platform: "web-mobile",
        platformType: "HTML5",
      });
      it("验证替换掉 web-mobile 平台的 physics-2d-box2d-jsb 模块", () => {
        expect(res.includeModules).to.be.deep.equal([
          "base",
