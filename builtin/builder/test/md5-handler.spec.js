...
const {
  md5CacheHandler,
} = require("../dist/worker/builder/tasks/postprocess-task/md5-cache-handler");
...
const commonMd5TestFolder = join(
  Editor.Project.path,
  "test-asset",
  "test-md5-file"
);
...
describe("自动扫描根目录以及 src 下的文本，替换链接", async () => {
  ...
  before(async () => {
    ...
    Object.keys(fileNameHashMap).forEach((fileName) => {
      hashedPathMap[join(md5TestFolder, "src", fileName)] = join(
        md5TestFolder,
        "src",
        fileNameHashMap[fileName]
      );
    });
    const md5CacheHandler = new md5CacheHandler(md5TestFolder, {
      replaceOnly: ["index.html"],
      handleTemplateMd5Link: true,
      excludes: ["vconsole.min.js"],
      includes: ["src/*", "*", "src/test-src.txt"],
    });
  ...
  describe("验证 replaceOnly 的 index.html 是否正常替换 md5 且文件名称不变", async () => {
    ...
    htmlReplaceContent.forEach((fileName) => {
      it(`HTML 内的 ${fileName}路径都被正确替换`, () => {
        expect(
          code.includes(basename(hashedPathMap[join(md5TestFolder, fileName)]))
        ).to.be.true;
      });
    });

    const htmlNotReplaceContent = [
  ...
  it("验证非根目录下的文件是否正常无变化", async () => {
    expect(existsSync(join(md5TestFolder, "test-no-md5", "index.js"))).to.be
      .true;
  });
