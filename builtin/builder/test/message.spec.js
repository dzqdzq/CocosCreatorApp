...
describe("构建通用消息测试", () => {
  describe("query-worker-ready", async () => {
    const readyState = await Editor.Message.request(
      "builder",
      "query-worker-ready"
    );
  ...
  describe("query-tasks-info", async () => {
    const taskInfo = await Editor.Message.request(
      "builder",
      "query-tasks-info"
    );
  ...
  describe("query-bundle-config", async () => {
    const bundleConfigs = await Editor.Message.request(
      "builder",
      "query-bundle-config"
    );
  ...
  describe("create-build-template", async () => {
    ...
    const templateMenu =
      allMenu["i18n:menu.project"].submenu["i18n:builder.create_user_template"]
        .submenu;
    // 特殊模板配置
      ...
      try {
        it(`创建 ${templateMenu[type].label} 平台模板`, async () => {
          ...
          const templatePath = join(
            Editor.Project.path,
            "build-templates",
            type in specialTemplateConfig
              ? specialTemplateConfig[type]
              : message.params[1]
          );
