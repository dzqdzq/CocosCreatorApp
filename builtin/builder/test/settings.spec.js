    ...
    describe("生成 bundleConfigs 验证", () => {
      it("bundleConfigs 正常生成包含所有需要的字段", () => {
        ...
        for (const config of data.bundleConfigs) {
          ...
          Object.values(config.paths).every((info) => {
            info[1] !== "cc.Script";
          });
        }
      });
      ...
      it("start scene bundle 正常生成", () => {
        const startScene = data.bundleConfigs.find(
          (item) => item.name === "start-scene"
        );
      ...
      it("internal Bundle 正常生成路径记录", () => {
        ...
        const internalPaths = data.bundleConfigs.find(
        ...
        expect(internalPaths[baseDependAsset][0]).to.equal(
      ...
      if (Editor.Project.name === "build-example") {
        const { paths, uuids } = data.bundleConfigs.find(
        ...
        it("不能包含模型资源的 uuid", () => {
          ...
          expect(uuids.includes("82b73f77-d266-4b0f-bfaa-5655143c6ac3")).to.be
            .false;
        });
        it("需要包含模型子资源", () => {
          expect(paths["82b73f77-d266-4b0f-bfaa-5655143c6ac3@e979f"]).to.be
            .exist;
          expect(uuids.includes("82b73f77-d266-4b0f-bfaa-5655143c6ac3@e979f"))
            .to.be.true;
        });
        it("不能包含脚本资源的 uuid", () => {
          expect(paths["b02a8776-6b86-4f1a-8ecf-93bcc1a55bea"]).to.be.not.exist;
          expect(uuids.includes("b02a8776-6b86-4f1a-8ecf-93bcc1a55bea")).to.be
            .false;
        });
      }
      const testAtlasBundle = data.bundleConfigs.find(
        (item) => item.name === "test-atlas-build"
      );
      if (testAtlasBundle) {
