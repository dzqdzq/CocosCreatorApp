...
} = require("../dist/share/platforms-options.js");
const { platform } = require("os");

describe("测试一些重要配置的平台默认值", () => {
  const testPlatform = PLATFORMS.filter(
    (platform) => !EXTERNAL_NATIVE_PLATFORM.includes(platform)
  );

  describe("nativeBundleMode", async () => {
    const defaultWasmPlatform = [
      "windows",
      "android",
      "google-play",
      "mac",
    ...
    const defaultBothPlatform = [
      "bytedance-mini-game",
      "web-desktop",
      "web-mobile",
    ];
    for (const platform of testPlatform) {
      const defaultValue = defaultWasmPlatform.includes(platform)
        ? "wasm"
        : defaultBothPlatform.includes(platform)
        ? "both"
        : "asmjs";
      const value = await Editor.Profile.getConfig(
        platform,
        "builder.common.nativeCodeBundleMode",
        "default"
      );
      if (value) {
  ...
  describe("gfx-webgl2", () => {
    ...
    testPlatform.forEach((platform) => {
      ...
      it(`${platform} 平台默认 gfx-webgl2 为 ${defaultValue}`, async () => {
        const value = await Editor.Profile.getConfig(
          platform,
          "builder.common.overwriteProjectSettings.includeModules.gfx-webgl2",
          "default"
        );
        if (value) {
