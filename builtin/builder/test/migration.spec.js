...
describe("构建数据迁移测试", async () => {
  const rawBuilderConfig = await Editor.Profile.getConfig(
    "builder",
    "",
    "local"
  );
  ...
  describe("0 -> 1.2.1 版本数据迁移测试", async () => {
    const config = readJSONSync(
      join(migrationConfigBase, "0.0.0", "builder.json")
    );
    ...
    const {
      migrationOptions_1_2_1,
    } = require("../dist/browser/migration/version/1.2.1");

    // 1.2.1 仅支持部分平台
    for (let platform of builtinPlugins) {
      // TODO 如果将来 平台数据注入方式发生改变需要调整写法
      // HACK
      const defaultOptions = await Editor.Profile.getConfig(
        "builder",
        `${platform}.${platform}`,
        "default"
      );
      ...
      platform =
        platform === "huawei-quick-game" ? "huawei-mini-game" : platform;
      const options = await Editor.Profile.getConfig(
        "builder",
        `${platform}.${platform}`,
        "local"
      );
      ...
      const version = await Editor.Profile.getConfig(
        "builder",
        `version.${platform}`,
        "local"
      );
    ...
    const queues = await Editor.Profile.getConfig(
      "builder",
  ...
  describe("1.2.1 -> 1.2.2 版本数据迁移测试", async () => {
    const config = readJSONSync(
      join(migrationConfigBase, "1.2.1", "builder.json")
    );
    await Editor.Profile.setConfig("builder", "", config, "local");
