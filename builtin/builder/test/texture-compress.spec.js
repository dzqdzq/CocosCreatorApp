...
describe('compressCustomFormat 自定义压缩方式', () => {
    ...
    const customToolDir = join(Editor.Project.path, './temp/CompressTexture/libwebp');
    copySync(join(Editor.App.path, `../tools/libwebp_${process.platform == 'win32' ? 'win32' : 'darwin'}`), customToolDir, { overwrite: true });

    describe('自定义压缩纹理工具配置 file 路径', async () => {
        const dest = join(destDir, 'raw-path.png');
        defaultOptions.dest = dest;
        const rawPath = join(customToolDir, `./bin/cwebp${process.platform === 'win32' ? '.exe' : ''}`);
    ...
    describe('自定义压缩纹理工具配置 project 路径', async () => {
        ...
        defaultOptions.dest = dest;
        const relPath = Editor.UI.File.resolveToUrl(join(customToolDir, `./bin/cwebp${process.platform === 'win32' ? '.exe' : ''}`), 'project');
