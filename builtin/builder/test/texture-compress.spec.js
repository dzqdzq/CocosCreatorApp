...
const { compressPVR } = require('../dist/worker/builder/build-task/image/texture-compress/compress-tool');
...
describe('compressPVR 压缩 pvr 类型图片测试', () => {
    ...
    const destDir = join(Editor.Project.path, 'temp', 'CompressTexture', 'dest');
            ...
            for (let j = 0; j < qualities.length; j++) {
                ...
                it(`generator ${compressFormats[i]}_${qualities[j]}.pvr`, async function() {
                    ...
                    try {
                        ...
                        console.error(`Test ${compressFormats[i]} ${qualities[j]} failed!`);
                    }
                });
