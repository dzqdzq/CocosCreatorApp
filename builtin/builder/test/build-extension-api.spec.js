...
const { readJsonSync, existsSync, remove } = require("fs-extra");
...
describe("执行构建，验证构建扩展机制结果是否正常", async () => {
  const extensionResJSONPath = join(
    Editor.Project.tmpDir,
    "builder",
    "test-extension-api-res.json"
  );
  const buildOptions = {
    platform: "web-mobile",
  ...
  const exitCode = await Editor.Message.request(
    "builder",
    "add-task",
    buildOptions,
    true
  ...
  describe("检查构建结果记录是否正常", () => {
    let res;
    try {
      res = readJsonSync(extensionResJSONPath);
    } catch (error) {
      console.error(error);
    }
    ...
    describe("检查 Image 资源基本的接口返回值、纹理压缩的情况", () => {
      const { contain, paths, rawPaths, importPaths } =
        res.resultAPI.imageCompress;
      it("Image contain", () => {
    ...
    describe("通过 sprite 查询图集接口、图集纹理压缩的返回值", () => {
      const { contain, paths, rawPaths, importPaths } =
        res.resultAPI.spriteFrameCompress;
      it("sprite contain", () => {
        expect(contain).to.be.true;
      });
