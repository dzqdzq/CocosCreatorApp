...
describe("compressUuid", () => {
  const testMap = {
    ...
    [normalUuid + subUuidSuffix + subUuidSuffix]:
      normalCompressedUuidWithMin + subUuidSuffix + subUuidSuffix,
  };
...
describe("getUuidFromPath", () => {
  const pathList = {
    ...
    [`${packUuid}.png`]: packUuid,
    [`${packUuid + MD5Suffix}.png`]: packUuid,
  };

  Object.keys(pathList).forEach((path) => {
    it(`c:/xxx/5b/${path} -> ${pathList[path]}`, () => {
      expect(utils.getUuidFromPath(`c:/xxx/5b/${path}`)).to.equal(
        pathList[path]
      );
    });
  });
