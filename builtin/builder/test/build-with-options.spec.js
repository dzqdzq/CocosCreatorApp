...
describe("请使用构建专用测试项目 build-example 项目测试！", async () => {
  ...
  describe("测试平台构建", async () => {
    describe("测试平台构建覆盖率", async () => {
      const { order } = await Editor.Message.request(
        "builder",
        "query-platform-config"
      );
      const missingPlatforms = order.filter(
        (platform) =>
          !platform.startsWith("xr-") && !testPlatforms.includes(platform)
      );
      if (order.length > testPlatforms) {
        console.error(
          `测试平台 ${missingPlatforms.join("; ")} 有缺失，请补充测试`
        );
      }
    ...
    for (const platform of testPlatforms) {
      ...
      describe(`测试平台 ${platform}`, async () => {
        const options = await readJson(
          join(buildConfigDir, `buildConfig_${platform}.json`)
        );
        ...
        try {
          // 固定 id 避免增加过多的任务
          options.id = platform;
          info.normalBuild.exitCode = await Editor.Message.request(
            "builder",
            "command-build",
            options
          );
        ...
        if (info.normalBuild.exitCode === 36) {
          ...
          bundleOptions.logDest = join(
            logDestDir,
            platform + "-bundle-build-only.log"
          );
          info.bundleBuild = {
            exitCode: 0,
          ...
          try {
            info.bundleBuild.exitCode = await Editor.Message.request(
              "builder",
              "command-build",
              bundleOptions
            );
          ...
          console.log(
            `平台 ${platform} 仅 bundle 构建模式构建 ${
              info.exitCode === 36 ? "成功" : "失败"
            }`
          );

          info.bundleBuild.logInfo = await analyzeLogFile(
            bundleOptions.logDest
          );
  ...
  describe("测试淘宝微信引擎分离的一些目录结构生成、关键构建结果", () => {
    const engineCacheDir = join(
      Editor.App.path,
      "resources/3d/engine/bin/.cache/editor-cache"
    );
