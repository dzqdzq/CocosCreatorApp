...
describe("1.2.9 迁移字节、微信物理相关选项", () => {
  ...
  before(async () => {
    rawIncludeModules = await Editor.Profile.getProject(
      "engine",
  ...
  describe("指定物理选项开启时", async () => {
    ...
    it("wasm 选项 fallback -> wasm", () => {
      expect(taskMap.fallback.options.packages.wechatgame.wasm).to.equal(
        "wasm"
      );
    });
    ...
    it("physX 选项 use -> physX", () => {
      expect(
        taskMap.physX.options.packages["bytedance-mini-game"].physX.use
      ).to.equal("physX");
    });
  });
