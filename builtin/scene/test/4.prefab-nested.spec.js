...
const { clearTestDir, getTestDir, adaptCloseCurrentScene } = require("./utils");
const reloadTime = 1200;

const testDBHeader = `${getTestDir()}/prefab-nested`;

describe("嵌套Prefab操作测试", () => {
  before(async () => {
    console.log("=========invoke 4.prefab.before");
  });
  after(async () => {
    await clearTestDir();
    console.log("=========invoke 4.prefab.after");
  });
  ...
  describe("prepare test resources", async () => {
    ...
    it("add child node in prefab mode", async () => {
      ...
      expect(childCylinderNodeDump.__prefab__.uuid).to.equal(
        testPrefabAssetUUID
      );
