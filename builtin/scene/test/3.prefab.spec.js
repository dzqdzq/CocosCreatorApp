...
const testDBHeader = `${getTestDir()}/prefab`;
let IS_MULTI_SCENE_MODE = false;
describe("Prefab操作测试", () => {
  after(async () => {
    await clearTestDir();
    console.log("=========invoke 3.prefab.after");
  });
  before(async () => {
    IS_MULTI_SCENE_MODE = await isMultiSceneMode();
    if (IS_MULTI_SCENE_MODE) {
  ...
  const testPrefabAssetUrl2 = `${testDBHeader}/testPrefab2.prefab`;
  ...
  describe("prepare test resources", async () => {
    ...
    it("create testPrefab", async () => {
      testPrefabAssetUUID = await requestScene(
        "create-prefab",
        testNodeUuid,
        testPrefabAssetUrl
      );
    ...
    it("add child prefab in prefab mode", async () => {
      ...
      const childCylinderNodeDump = await requestScene(
        "query-node",
        childCylinderNodeUuid
      );
    ...
    it("create child prefab", async () => {
      childPrefabAssetUUID = await requestScene(
        "create-prefab",
        childNodeUUID,
        childPrefabAssetUrl
      );
      ...
      expect(!!childNodeUUID).to.true;
      const childPrefabNodeDump = await requestScene(
        "query-node",
        childNodeUUID
      );
    ...
    it("modify nested prefabInstance position", async () => {
      ...
      const positionOverride =
        propertyOverrides[propertyOverrides.length - 1].value;
    ...
    it("create mountedChildren for testPrefab in general", async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const mountedChildren =
          nodeDump.__prefab__.instance.value.mountedChildren.value[0].value;
    ...
    it("create mountedChildren in general", async () => {
      ...
      // console.log(nodeDump);
      const mountedChildren =
        nodeDump.__prefab__.instance.value.mountedChildren.value;
    ...
    it("apply prefab", async () => {
      ...
      const childConeNodeDump = await requestScene(
        "query-node",
        childPrefabUUID
      );
    ...
    it("modify prefab Root position", async () => {
      ...
      const nodeDump = await requestScene("query-node", testNodeUuid);
      const prefabFileId = nodeDump.__prefab__.fileId;
      const positionOverride = prefab.getPropertyOverrides(
        nodeDump,
        positionSerializedKey
      );
    ...
    it("create testPrefab2", async () => {
      testPrefabAssetUUID2 = await requestScene(
        "create-prefab",
        testNodeUuid,
        testPrefabAssetUrl2
      );
    ...
    it("add mountedComponent to prefabInstance", async () => {
      ...
      await requestScene("create-component", {
        uuid: lastChild.uuid,
        component: "cc.BoxCollider2D",
      });
      await requestScene("end-recording", undoId);
    ...
    it("remove component in prefabInstance", async () => {
      ...
      removedCompFileID =
        firstChildDump.__comps__[0].value.__prefab.value.fileId.value;

      await requestScene("remove-array-element", {
    ...
    it("create prefabInstance 2", async () => {
      ...
      testNodeUUID2 = await requestScene("create-node", {
        ...
        assetUuid: testPrefabAssetUUID2,
      });
    ...
    it("unlink prefab", async () => {
      ...
      const nodeDump = await requestScene("query-node", testNodeUuid);
      // console.log(nodeDump);
      const nodeDump_prefab = nodeDump.__prefab__;
      expect(!!nodeDump_prefab).to.false;
      expect(nodeDump.children.length).equal(4);
      const childPrefabUUID = nodeDump.children[1].value.uuid;
