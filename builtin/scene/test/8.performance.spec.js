...
describe("场景常用接口性能测试", async () => {
  ...
  describe("新建1000个节点的时间记录(小于10秒)", async () => {
    ...
    const time = await Editor.Metrics.trackTimeEnd("scene:create-node", {
      output: true,
    });
  ...
  describe("新建 100 个菜单节点的时间记录(小于5秒)", async () => {
    ...
    const menus = pkgOptions.contributions.menu.filter(
      (item) => item.message === "create-node"
    );
    ...
    const time = await Editor.Metrics.trackTimeEnd("scene:create-menu-node", {
      output: true,
    });
