...
const { requestScene } = require('./utils');
...
describe('场景查询测试', () => {
    ...
    after(async () => {
        //多场景模式，就要顺便把场景关一下
        const curUuid = await requestScene( 'query-current-scene');
        curUuid &&(await requestScene('multi-close-scene', curUuid));
    });

    describe('query-is-ready', async () => {
        const isReady = await requestScene( 'query-is-ready');
    ...
    describe('query-node-tree', async () => {
        // 返回当前编辑的尝尽内的第一个场景
        const dump_empty = await requestScene( 'query-node-tree');
    ...
    describe('query-node', async () => {
        const dump = await requestScene( 'query-node', uuid);
    ...
    describe('query-dirty', async () => {
        const dirty = await requestScene( 'query-dirty', uuid);
    ...
    describe('query-component-function-of-node', async () => {
        const map = await requestScene( 'query-component-function-of-node', uuid);
