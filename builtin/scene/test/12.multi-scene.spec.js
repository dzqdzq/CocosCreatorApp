...
describe("多场景编辑功能测试", () => {
  before(async () => {
    ...
    if (currentScene) {
      await requestScene("multi-close-others", currentScene);
    }
  ...
  describe("场景保存测试", async () => {
    ...
    const curNode = await requestScene(
      "query-node",
      insertNodes[lastScene.uuid]
    );
  ...
  describe("tab 移动测试", async () => {
    ...
    scenes = scenes.filter((scene) =>
      SCENE_INFO.find((info) => info.uuid === scene.uuid)
    );
    //检查现在是否是 1 3 2 的顺序
  ...
  describe("关闭场景测试", async () => {
    ...
    await requestScene(
      "multi-scene-focus",
      SCENE_INFO[SCENE_INFO.length - 1].uuid
    );
