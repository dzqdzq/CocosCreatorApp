...
describe("Prefab PrefabState 操作测试", () => {
  ...
  describe("init test resources", async () => {
    it("init scene", async () => {
      // 创建并且打开场景
      const internalScenePath = await Editor.Message.request(
        "asset-db",
        "query-path",
        internalSceneURL
      );
      ...
      for (let i = 0; i < keys.length; i++) {
        const key = keys[i];
        const tempNodeID = await requestScene("create-node", {
          name: key,
          parent: lastNodeID,
        });
        if (key === "B") {
          const tempChildNodeID = await requestScene("create-node", {
            name: `${key}-child`,
            parent: tempNodeID,
          });
        }
        if (key === "C") {
          ...
          const assetUuid = await requestScene(
            "create-prefab",
            tempNodeID,
            prefabAssetUrl
          );
  ...
  describe("prepare test resources", async () => {
    ...
    it("child nested parent is normal node", async () => {
      ...
      const A = getNodeIDByName(tree, "A");
      let tempNodeID = await requestScene("create-node", {
        name: "E",
        ASSET_UUID,
        parent: A.uuid,
      });
      tempNodeID = await requestScene("create-node", {
        name: "F",
        ASSET_UUID,
        parent: tempNodeID,
      });
