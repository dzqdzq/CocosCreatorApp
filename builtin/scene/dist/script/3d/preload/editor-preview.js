...
class EditorPreview {
  ...
  async start() {
    ...
    await this._startEditor();
  }
  async _startEngine() {
    var e = await Editor.Profile.getProject("project", "layer");
    var t = (await Editor.Profile.getProject("project", "sorting-layer")) || {};
    ...
    this.engineInfo.overrideSettings = {
      engine: {
        customLayers: e.map((t) => {
    ...
    await cce.Startup.configEngine(this.engineInfo, r, e, t);
    await cce.Startup.initDesignResolution();

    cc.game.restart = () => {
      cce.Ipc.clearQueue();
      cce.Ipc.send("preview-restart");
    };
  }
  async _startEditor() {
    await cce.Startup.initManager(
      Object.assign(this.engineInfo, { project: Editor.Project.path })
    );
    var e =
    ...
    if (e && !e.includes("3d")) {
      cce.SceneFacadeManager.changeProjectMode("2d");
    }
  }
  async _startPreview() {}
}
exports.default = new EditorPreview();
