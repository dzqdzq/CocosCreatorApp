...
class SceneGizmo extends gizmo_select_1.default {
  _controller;
  ...
  updateControllerData() {
    ...
    if (this._isInitialized && this.target) {
      if ((e = this.target.node.globals.octree) && e.enabled) {
        cc_1.geometry.AABB.fromPoints(
          this._octreeBoundingBox,
          e.minPos,
          e.maxPos
        );
        ...
        cc_1.Vec3.multiplyScalar(
          this._octreeBBSize,
          this._octreeBoundingBox.halfExtents,
          2
        );

        this._controller.updateSize(
          this._octreeBoundingBox.center,
          this._octreeBBSize
        );
