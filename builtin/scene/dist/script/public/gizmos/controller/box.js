...
const controller_shape_2 = __importDefault(
  require("../utils/controller-shape")
);
...
class BoxController extends editable_1.default {
  ...
  setColor(e) {
    if (this._wireFrameBoxNode) {
      this._color = e;
      setMeshColor(this._wireFrameBoxNode, e);
      this.setEditHandlesColor(e);
    }
  }
  setOpacity(e) {
    if (this._wireFrameBoxNode) {
      setNodeOpacity(this._wireFrameBoxNode, e);
    }
  }
  _updateEditHandle(e) {
    ...
    var i =
      (cc_1.Vec3.multiply(o, i, this._size),
      cc_1.Vec3.multiplyScalar(o, o, 0.5),
      new cc_1.Vec3(o));

    i.add(this._center);
    var o = this._editHandleScales[e];
    var e = this.getScale();
    t.setScale(o / e.x, o / e.y, o / e.z);
    cc_1.Vec3.multiply(i, i, e);
    t.setPosition(i);
  }
  initShape() {
    this.createShapeNode("BoxController");

    this._wireFrameBoxNode = controller_utils_1.default.wireframeBox(
      this._center,
      this._size,
      this._color,
      { forwardPipeline: true, depthTestForTriangles: true }
    );

    this._wireFrameBoxNode.parent = this.shape;
    this._wireFrameBoxMeshRenderer = getModel(this._wireFrameBoxNode);

    this._cubeNode = controller_utils_1.default.cube(
      1,
      1,
      1,
      cc_1.Color.GREEN,
  ...
  updateSize(e, t) {
    ...
    if (this._cubeNodeMR && this._wireFrameBoxMeshRenderer) {
      ...
      this._size !== t &&
        updateBoundingBox(
          this._cubeNodeMR,
          cc_1.Vec3.multiplyScalar(new cc_1.Vec3(), t, -0.5),
          cc_1.Vec3.multiplyScalar(new cc_1.Vec3(), t, 0.5)
        );

      this._size = t;
      i = controller_shape_1.default.calcBoxPoints(this._center, this._size);
      updatePositions(this._wireFrameBoxMeshRenderer, i);
      this._edit && this.updateEditHandles();

      updatePositions(
        this._cubeNodeMR,
        controller_shape_2.default.calcCubeData(t.x, t.y, t.z, e).positions
      );
  ...
  onMouseMove(e) {
    ...
    if (
      ...
      e.handleName === AxisName.x || e.handleName === AxisName.neg_x
        ? (this._deltaSize.x = t)
        : e.handleName === AxisName.y || e.handleName === AxisName.neg_y
        ? (this._deltaSize.y = t)
        : (e.handleName !== AxisName.z && e.handleName !== AxisName.neg_z) ||
          (this._deltaSize.z = t),
  ...
  onHoverIn(e) {
    if (this._cubeNodeMR && this._cubeNodeMR.material) {
      this._cubeNodeMR.material.setProperty(
        "selectedFaceForward",
        handleNameToFace[e.handleName] || cc_1.Vec4.ZERO,
        0
      );
