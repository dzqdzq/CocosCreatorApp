...
class CapsuleColliderComponentGizmo extends base_1.SelectGizmo {
  _controller;
  _maxScale = 1;
  _radius = 0;
  _height = 0;
    ...
    if (this._isInitialized && this.target != null) {
      if (this.target instanceof cc_1.CapsuleCollider) {
        ...
        this._controller.updateSize(
          this.target.center,
          this.target.radius,
          this.target.cylinderHeight
        );
      } else {
        this._controller.hide();
        console.error("target is not a cc.CapsuleCollider");
      }
    }
  }
  onTargetUpdate() {
...
exports.default = CapsuleColliderComponentGizmo;
