...
const preloadPath = join(
  __dirname,
  "../../../script/3d/preload/web/preload.js"
);
...
class EditorClient extends base_client_1.BaseClient {
  ...
  load(e) {
    ...
    this.webview.addEventListener("dom-ready", () => {
      ...
      if (!t) {
        ...
        e?.on("render-process-gone", (e, t) => {
          if (
            t &&
            (Editor.Message.broadcast("crash-reporter:report", {
              process: "Scene Web View",
              value: { A100000_3D_Engine: 1 },
              details: t,
              time: new Date(),
            }),
            t.reason === "crashed")
    ...
    this.ipc = new scene_web_ipc_1.default(
      this.webview,
      utils_1.IPCChannel.SceneWebSend,
      utils_1.IPCChannel.SceneWebReply
    );
  }
  unload(e) {
    if (this._loaded) {
