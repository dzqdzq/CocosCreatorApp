Object.defineProperty(exports, "__esModule", { value: true });
exports.element = undefined;

const { getMessageProtocolScene } = require("./utils");

class element extends Editor.UI.__protected__.DragObject {
  missStateShowText = "Missing Component";
  selectIconValue = "drag-component";
  constructor() {
    super();

    if (this.shadowRoot) {
      this.shadowRoot.querySelector("#custom-style").innerHTML =
        ".type ui-icon { color: var(--color-warn-fill-normal); }";
    }
  }
  async _onAreaClick() {
    var e = await Editor.Message.request(
      getMessageProtocolScene(this),
      "query-component",
      this.$root.value
    );

    if (e && e.value.node) {
      e = e.value.node.value.uuid;
      Editor.Message.broadcast("ui-kit:touch-node", e);
    }
  }
  _onSelectClick() {
    if (!this.$root.disabled && !this.$root.readonly) {
      Date.now();

      Editor.Panel.__protected__.openKit("ui-kit.searcher", {
        elem: this.$root,
        params: [
          {
            type: "component",
            value: this.$root.value,
            droppable: this.$root.droppable,
          },
        ],
        listeners: {
          confirm: (e) => {
            if (e) {
              this.$root.value = e.value;
              this.$root.dispatch("confirm");
            }
          },
          change: (e) => {
            if (e) {
              this.$root.value = e.value;
              this.$root.dispatch("change");
            }
          },
          cancel: (e) => {
            if (e) {
              this.$root.value = e.value;
              this.$root.dispatch("cancel");
            }
          },
        },
      });
    }
  }
  async _onTranslationName(e) {
    if (e) {
      e = await Editor.Message.request(
        getMessageProtocolScene(this),
        "query-component",
        e
      );
      if (e && e.value.node) {
        e = e.value.node.value.uuid;

        e = await Editor.Message.request(
          getMessageProtocolScene(this),
          "query-node",
          e
        );

        if (e && e.name) {
          return e.name.value;
        }
      }
    }
    return null;
  }
}
exports.element = element;
