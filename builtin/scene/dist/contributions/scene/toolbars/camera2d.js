...
`,SceneCamera2DVM=Vue.extend({name:"SceneCamera2DVM",data(){return{is2D:!1}},methods:{zoomReset(){$scene&&$scene.callSceneMethod("resetSceneViewZoom")}},template:vueTemplate});function ready(e){close(),panel=this,$scene=e.nextElementSibling,vm?.$destroy(),(vm=new SceneCamera2DVM).$mount(panel.$.container),Editor.Message.__protected__.addBroadcastListener("scene:ready",panel.sceneReady),Editor.Message.__protected__.addBroadcastListener("scene:dimension-changed",panel.dimensionChanged)}function close(){panel&&(Editor.Message.__protected__.removeBroadcastListener("scene:ready",panel.sceneReady),Editor.Message.__protected__.removeBroadcastListener("scene:dimension-changed",panel.dimensionChanged)),vm?.$destroy(),vm=null,panel=null,$scene=null}exports.position="right",exports.template=`
<style>
.camera2d {
    position: relative;
    width: 24px;
...
`,exports.$={container:".camera2d"},exports.methods={dimensionChanged(e){vm&&(vm.is2D=e)},async sceneReady(){var e=await Editor.Message.request("scene","query-is2D");panel&&panel.dimensionChanged(e)}},exports.default=__importStar(require("./camera2d"));