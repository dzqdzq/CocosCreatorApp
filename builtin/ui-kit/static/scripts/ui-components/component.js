...
const STYLE = `<style>${fs.readFileSync(path.join(__dirname, './../../../dist/ui-components/component.css'), 'utf8')}</style>`;
const CUSTOM_STYLE = `<style id="custom-style"></style>`;
const HTML = `${fs.readFileSync(path.join(__dirname, '../../../static/template/ui-components/component.html'), 'utf8')}`;
const instanceArray = [];

let customStyle = '';
async function nameTranslator(nodeUuid) {
    if (!nodeUuid || !typeof nodeUuid === 'string') {
        return null;
    }
    const nodeInfo = await Editor.Message.request('scene', 'query-node', nodeUuid);
    if (nodeInfo) {
        return nodeInfo.name.value;
    }
    return null;
};

async function getNodeUuid(compUuid) {
    if (!compUuid || !typeof compUuid === 'string') {
        return null;
    }
    compUuid = compUuid.trim();
    // 组件
    const info = await Editor.Message.request('scene', 'query-component', compUuid);
    if (info && info.value.node) {
        return info.value.node.value.uuid;
    }
    return null;
}
...
class Component extends Base {
            ...
            case 'value':
                {
                    ...
                    if (this._value) {
                        this.nodeUuid = await getNodeUuid(this._value);
                        name = await nameTranslator(this.nodeUuid);
                        if (name === null) {
    ...
    _onAreaClick() {
        ...
        Editor.Message.send('hierarchy', 'twinkle', this.$root.nodeUuid.trim());
    }
    ...
    _onAreaDblClick() {
        ...
        Editor.Selection.select('node', this.$root.nodeUuid.trim());
    }
}
