...
function IcreateKey(clipUuid, nodePath, propKey, frame, customData) {
  return generateAnimOperation("createKey", [
    clipUuid,
    nodePath,
    propKey,
    frame,
...
function IcopyKeysTo(clipUuid, nodePath, propKey, srcFrames, dstFrame) {
  return generateAnimOperation("copyKeysTo", [
    clipUuid,
    nodePath,
    propKey,
    srcFrames,
...
function IremoveKey(clipUuid, nodePath, propKey, frame) {
  return generateAnimOperation("removeKey", [
    clipUuid,
    nodePath,
    propKey,
    frame,
...
function ImoveKeys(clipUuid, nodePath, propKey, frames, offset) {
  return generateAnimOperation("moveKeys", [
    clipUuid,
    nodePath,
    propKey,
    frames,
...
function IspacingKeys(clipUuid, nodePath, propKey, frames, spacingFrame) {
  return generateAnimOperation("spacingKeys", [
    clipUuid,
    nodePath,
    propKey,
    frames,
...
function IchangeNodeDataPath(clipUuid, srcPath, dstPath) {
  return generateAnimOperation("changeNodeDataPath", [
    clipUuid,
    srcPath,
    dstPath,
  ]);
...
function ImodifyCurveOfKey(clipUuid, nodePath, propKey, frame, data) {
  return generateAnimOperation("modifyCurveOfKey", [
    clipUuid,
    nodePath,
    propKey,
    frame,
...
describe("Test Animation IPC", () => {
  ...
  describe("prepare test resources", async () => {
    ...
    it("add animation component", async () => {
      ...
      const nodeDump = await Editor.Message.request(
        "scene",
        "query-node",
        testNodeUuid
      );
    ...
    it("create animation clip", async () => {
      ...
      const filePath = await Editor.Message.request(
        "asset-db",
        "query-path",
        templateUrl
      );
    ...
    it("set default clip to component", async () => {
      ...
      const defaultClipUuid =
        nodeDump.__comps__[1].value.defaultClip.value.uuid;
  ...
  describe("test animation clip edit", async () => {
    ...
    const getCurEditClipDump = async () =>
      await Editor.Message.request(
        "scene",
        "query-animation-clip",
        testNodeUuid,
        animClipUuid
      );
    ...
    describe("enter Animation Mode", async () => {
      it("set edit clip", async () => {
        ...
        const curAnimInfo = await Editor.Message.request(
          "scene",
      ...
      it("change to Animation Mode", async () => {
        ...
        const sceneMode = await Editor.Message.request(
          "scene",
          "query-scene-mode"
        );
    ...
    describe("edit uniform key", async () => {
      ...
      it("get animatable properties", async () => {
        const animatableProps = await Editor.Message.request(
          "scene",
      ...
      it("create key", async () => {
        ...
        const addKey1 = await IApplyOperation(
          IcreateKey(animClipUuid, nodePath, propKey, keyframe1, customData)
        );
        customData.newValue = { r: 0, g: 255, b: 0, a: 255 };
        const addKey2 = await IApplyOperation(
          IcreateKey(animClipUuid, nodePath, propKey, keyframe2, customData)
        );
        const clipDump = await getCurEditClipDump();
      ...
      it("get value at", async () => {
        const dumpValue = await Editor.Message.request(
          "scene",
          "query-property-value-at-frame",
          animClipUuid,
          nodePath,
          propKey,
          60
      ...
      it("copy key", async () => {
        ...
        const result = await IApplyOperation(
          IcopyKeysTo(
            animClipUuid,
            nodePath,
            propKey,
      ...
      it("spacing keys", async () => {
        const result = await IApplyOperation(
          IspacingKeys(
            ...
            nodePath,
            propKey,
            [keyframe1, keyframe2, copyKeyframe1, copyKeyframe2],
            10
          )
        );
        const clipDump = await getCurEditClipDump();
      ...
      it("remove key", async () => {
        const deleteKey1 = await IApplyOperation(
          IremoveKey(animClipUuid, nodePath, propKey, copyKeyframe1)
        );
        const deleteKey2 = await IApplyOperation(
          IremoveKey(animClipUuid, nodePath, propKey, copyKeyframe2)
        );
        const clipDump = await getCurEditClipDump();
      ...
      it("change sample", async () => {
        const sample = 30;
        const result = await IApplyOperation(
          IchangeClipSample(animClipUuid, sample)
        );
        const clipDump = await getCurEditClipDump();
      ...
      it("clear keys", async () => {
        const result = await IApplyOperation(
          IclearKeys(animClipUuid, nodePath, propKey)
        );
        const clipDump = await getCurEditClipDump();
        console.log("remove key:", clipDump);
        const keyframes = clipDump.curves[curveIndex].keyframes;
      ...
      it("remove material prop", async () => {
        const result = await IApplyOperation(
          IremoveProp(animClipUuid, nodePath, propKey)
        );
        const clipDump = await getCurEditClipDump();
        console.log("remove key:", clipDump);
        const clipDump_curves = clipDump.curves;
        const checkcurves = clipDump_curves.length === 0;
    ...
    describe("edit node keys", async () => {
      ...
      it("create key", async () => {
        const addKey1 = await IApplyOperation(
          IcreateKey(animClipUuid, nodePath, propKey, keyframe1)
        );
        const addKey2 = await IApplyOperation(
          IcreateKey(animClipUuid, nodePath, propKey, keyframe2)
        );
        const clipDump = await getCurEditClipDump();
      ...
      const InterpMode_CUBIC = InterpMode.CUBIC;
      const curveData = {
        inTangent: modifyInTangent,
        outTangent: modifyOutTangent,
        interpMode: InterpMode_CUBIC,
      };
      it("modify curve", async () => {
        const addKey = await IApplyOperation(
          IcreateKey(animClipUuid, nodePath, propKey, endKey)
        );
        const curveResult = await IApplyOperation(
          ImodifyCurveOfKey(
            animClipUuid,
            nodePath,
            propKey + ".x",
      ...
      it("copy node curve to subNode", async () => {
        ...
        await IApplyOperation(
          IcopyNode(
            animClipUuid,
            { curvesDump: srcClipDump.curves },
      ...
      it("remove subNode data", async () => {
        const result = await IApplyOperation(
          IremoveNode(animClipUuid, subNodePath)
        );
        const clipDump = await getCurEditClipDump();
        console.log("dump after remove subNode data:", clipDump);
        const clipDump_curves = clipDump.curves;
        const checkCurves = clipDump_curves.length === 4;
        expect(result && checkCurves).to.true;
      });
      // key
      // - testAnimNode[0, 30, 45]
      ...
      it("copy prop curve to subNode", async () => {
        ...
        await IApplyOperation(
          IcopyProp(
            animClipUuid,
            { curvesDump: [srcClipDump.curves[1]] },
            { nodePath: subNodePath, propKeys: [propKey + ".x"] }
          )
        );
        const clipDump = await getCurEditClipDump();
        ...
        const [, curveKeyframe] = clipDump.curves[subCurveIndex + 1].keyframes;
        expect(curveKeyframe.inTangent).to.approximately(
          modifyInTangent,
          0.001
        );
        expect(curveKeyframe.outTangent).to.approximately(
          modifyOutTangent,
          0.001
        );
        expect(curveKeyframe.interpMode).to.equal(InterpMode_CUBIC);
      });
      // key
      // - testAnimNode[0, 30, 45]
      //   - subNode[0, 30, 45]
      ...
      it("remove key", async () => {
        const deleteKey1 = await IApplyOperation(
          IremoveKey(animClipUuid, nodePath, propKey, 0)
        );
        const clipDump = await getCurEditClipDump();
        console.log("remove key:", clipDump);
        const keyframes = clipDump.curves[curveIndex].keyframes;
      ...
      it("copy prop curve key to subNode prop", async () => {
        ...
        await IApplyOperation(
          IcopyKey(
            animClipUuid,
            { curvesDump: [srcClipDump.curves[0]] },
            { nodePath: subNodePath, propKeys: [propKey], startFrame }
          )
        );
        const clipDump = await getCurEditClipDump();
        ...
        // position curve has one combined curve and three part curve
        expect(clipDump.curves.length === 8 && checkFrames && checkValues).to
          .true;
      });
      // key
      // - testAnimNode[30, 45]
      ...
      it("clear subNode keys", async () => {
        const result = await IApplyOperation(
          IclearKeys(animClipUuid, subNodePath, propKey)
        );
        const clipDump = await getCurEditClipDump();
        const keyframes = clipDump.curves[subCurveIndex].keyframes;
        const checkFrames = keyframes.length === 0;
      ...
      it(`create key for subNode ${prop}`, async () => {
        const addKey1 = await IApplyOperation(
          IcreateKey(animClipUuid, subNodePath, propKey, keyframe1)
        );
        const addKey2 = await IApplyOperation(
          IcreateKey(animClipUuid, subNodePath, propKey, keyframe2)
        );
        const clipDump = await getCurEditClipDump();
      ...
      it("change node data path", async () => {
        IApplyOperation(
          IchangeNodeDataPath(animClipUuid, nodePath, subNodePath)
        );
        const clipDump = await getCurEditClipDump();
      ...
      it("change node data path back", async () => {
        IApplyOperation(
          IchangeNodeDataPath(animClipUuid, subNodePath, nodePath)
        );
        const clipDump = await getCurEditClipDump();
      ...
      it("remove node clip", async () => {
        const result = await IApplyOperation(
          IremoveNode(animClipUuid, nodePath)
        );
        const clipDump = await getCurEditClipDump();
        console.log(clipDump);
        const clipDump_curves = clipDump.curves;
    ...
    describe("edit events", async () => {
      ...
      it("add event", async () => {
        const result1 = await IApplyOperation(
          IaddEvent(animClipUuid, eventFrame1, eventFuncName1, [])
        );
        const result2 = await IApplyOperation(
          IaddEvent(animClipUuid, eventFrame2, eventFuncName2, [])
        );
        const clipDump = await getCurEditClipDump();
        const checkEvents =
          clipDump.events.length === 2 &&
      ...
      it("copy event", async () => {
        ...
        const result = await IApplyOperation(
          IcopyEvent(
            animClipUuid,
            { eventsDump: srcClipDump.events },
            { startFrame: dstFrame }
          )
        );
        const clipDump = await getCurEditClipDump();
        const checkEvents =
          clipDump.events.length === 4 &&
      ...
      it("move events", async () => {
        const result = await IApplyOperation(
          ImoveEvents(animClipUuid, [copyEventFrame1, copyEventFrame2], offset)
        );
      ...
      it("update event", async () => {
        ...
        const result = await IApplyOperation(
          IupdateEvent(animClipUuid, copyEventFrame1, [
            { func: updateFuncName, params: [] },
          ])
        );
        const clipDump = await getCurEditClipDump();
        const checkEvents =
      ...
      it("delete event", async () => {
        const result = await IApplyOperation(
          IdeleteEvent(animClipUuid, copyEventFrame2)
        );
        const clipDump = await getCurEditClipDump();
        const checkEvents =
          clipDump.events.length === 3 &&
