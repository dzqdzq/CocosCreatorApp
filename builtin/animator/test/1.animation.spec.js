...
describe('Test Animation IPC', () => {
    ...
    describe('prepare test resources', async () => {
        ...
        it('set default clip to component', async () => {
            ...
            const nodeDump = await Editor.Message.request('scene', 'query-node', testNodeUuid);
            let defaultClipUuid = nodeDump.__comps__[1].value.defaultClip.value.uuid;
    ...
    describe('test animation clip edit', async () => {
        ...
        let getCurEditClipDump = async function() {
        ...
        describe('enter Animation Mode', async () => {
            it('set edit clip', async () => {
                ...
                let curAnimInfo = await Editor.Message.request('scene', 'query-current-animation-info');
                expect(curAnimInfo.clipid).to.equal(animClipUuid);
            });

            it('change to Animation Mode', async () => {
                Irecord(testNodeUuid, true);
                let sceneMode = await Editor.Message.request('scene', 'query-scene-mode');
        ...
        describe('edit uniform key', async () => {
            ...
            it('get animatable properties', async () => {
                let animatableProps = await Editor.Message.request('scene', 'query-animation-properties', testNodeUuid);
            ...
            it(`create ${uniformProp} prop`, async () => {
                ...
                let clipDump = await getCurEditClipDump();
            ...
            it('create key', async () => {
                ...
                let checkFrames = (keyframes.length === 2) &&
            ...
            it('get value at', async () => {
                let dumpValue = await Editor.Message.request('scene', 'query-property-value-at-frame', animClipUuid, nodePath, propKey, 60);
            ...
            it('copy key', async () => {
                ...
                let keyframes = clipDump.curves[curveIndex].keyframes;
                let checkFrames = (keyframes.length === 4) &&
            ...
            it('clear keys', async () => {
                ...
                console.log('remove key:', clipDump);
                let keyframes = clipDump.curves[curveIndex].keyframes;
                let checkFrames = (keyframes.length === 0);
                expect(result && checkFrames).to.true;
            });
            ...
            it('remove material prop', async () => {
                ...
                let clipDump = await getCurEditClipDump();
                console.log('remove key:', clipDump);
                let curves = clipDump.curves;
                let checkcurves = (curves.length === 0);
        ...
        describe('edit node keys', async () => {
            ...
            it('get animatable properties', async () => {
                let animatableProps = await Editor.Message.request('scene',
            ...
            it('remove key', async () => {
                ...
                let keyframes = clipDump.curves[curveIndex + 1].keyframes;
                let checkFrames = (keyframes.length === 2) &&
                                  (keyframes[0].frame === keyframe1) &&
                                  (keyframes[1].frame === keyframe2);
                expect(deleteKey1 && deleteKey2 && checkFrames).to.true;
            ...
            const endKey = 45;
            const modifyKey = 30;
            const curveData = { inTangent: 0.5, outTangent: 0.7, interpMode: InterpMode.CUBIC };
            it('modify curve', async () => {
                ...
                let checkCurve = (keyframes[1].inTangent === 0.5);
            ...
            it(`copy node curve to subNode`, async () => {
                let srcClipDump = await getCurEditClipDump();
                ...
                let keyframes = clipDump.curves[subCurveIndex].keyframes;
                let checkFrames = (keyframes.length === 3);
                // position curve has one combined curve and three part curve
            ...
            it('remove subNode data', async () => {
                ...
                let curves = clipDump.curves;
                let checkCurves = (curves.length === 4);
                expect(result && checkCurves).to.true;
            });
            // key
            // - testAnimNode[0, 30, 45]
            ...
            it(`copy prop curve to subNode`, async () => {
                let srcClipDump = await getCurEditClipDump();
                await IApplyOperation(IcopyProp(animClipUuid, { curvesDump: [srcClipDump.curves[0]] }, { nodePath: subNodePath, propKeys: [propKey] }));
                let clipDump = await getCurEditClipDump();
            ...
            it(`copy prop curve key to subNode prop`, async () => {
                ...
                let keyframes = clipDump.curves[subCurveIndex].keyframes;
                let checkFrames = (keyframes.length === 5) &&
                                    keyframes[1].frame === 5 &&
                                    keyframes[2].frame === 20;
                let checkValues = (keyframes[1].dump.value.y === 5);
                // position curve has one combined curve and three part curve
        ...
        describe('edit events', async () => {
            let eventFrame1 = 5;
            let eventFrame2 = 30;
            let eventFuncName1 = 'Hello1';
            let eventFuncName2 = 'Hello2';
            it('add event', async () => {
                ...
                let clipDump = await getCurEditClipDump();
                let checkEvents = clipDump.events.length === 2 &&
            ...
            it('copy event', async () => {
                ...
                let clipDump = await getCurEditClipDump();
                let checkEvents = clipDump.events.length === 4 &&
            ...
            it('delete event', async () => {
                ...
                let clipDump = await getCurEditClipDump();
                let checkEvents = clipDump.events.length === 3 &&
