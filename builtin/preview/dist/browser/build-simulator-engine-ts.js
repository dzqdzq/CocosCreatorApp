...
const { emptyDirSync, writeFile } = require("fs-extra");
...
async function buildSimulatorEngineTS(isOnPackingProcess) {
  ...
  if (isOnPackingProcess) {
    // 打包环境只能用写死的内置引擎路径
    engineDir = join(__dirname, "../../../../../resources/3d/engine");
  } else {
    engineDir = (await Editor.Message.request("engine", "query-engine-info"))
      .typescript.path;
    ...
    const engineModules =
      (await Editor.Message.request("engine", "query-engine-modules-profile"))
        ?.includeModules ?? [];
    const foundSpineVersions = engineModules.filter((feature) =>
      feature.startsWith("spine-")
    );
    if (foundSpineVersions.length > 1) {
      throw new Error(
        `Should only select one spine version, found: ${foundSpineVersions.join(
          ", "
        )}`
      );
  ...
  const staticDir = join(__dirname, "../../static/simulator").replace(
    "app.asar",
    "app.asar.unpacked"
  );
  ...
  let allFeatures = excludeFeatures(statsQuery.getFeatures(), [
    "gfx-webgpu",
    "vendor-google",
  ]);
  ...
  await writeFile(
    join(staticDir, "import-map.json"),
  ...
  await (0, build_polyfills_1.default)({
    debug: false,
    sourceMap: false,
    file: join(staticDir, "polyfills.bundle.js"),
    coreJs: {
  ...
  // 构建 system.bundle.js
  await build({
    out: join(staticDir, "system.bundle.js"),
    minify: false,
    sourceMap: false,
