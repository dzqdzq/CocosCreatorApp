...
async function getNativeEnginePath() {
  return (await Editor.Message.request("engine", "query-engine-info")).native
    .path;
}
...
async function getSimulatorPreference() {
  const showDebugPanel = await Editor.Profile.getConfig(
    "preview",
    "preview.simulator_debugger"
  );
  let waitForConnect = false;
  if (showDebugPanel) {
    waitForConnect = await Editor.Profile.getConfig(
      "preview",
      "preview.wait_for_connect"
    );
  }
  const orientation = await Editor.Profile.getConfig(
    "preview",
    "preview.simulator_orientation"
  );
  const device = await Editor.Message.request("device", "query");
  const resolutionIndex = await Editor.Profile.getConfig(
    "preview",
  ...
  if (!resolution) {
    ...
    resolution = await Editor.Profile.getProject(
      "project",
...
async function generateSimulatorConfig() {
  ...
  const simulatorResourcesMac = join(
    nativeEnginePath,
    "simulator/Release/SimulatorApp-Mac.app/Contents/Resources"
  );
  const simulatorWritablePath = isDarwin
    ? simulatorResourcesMac
    : join(process.env.LOCALAPPDATA, "SimulatorApp-Win32/debugruntime");
  // 偏好设置
  const preference = await getSimulatorPreference();
  // 写入 JSON
  ensureDirSync(simulatorWritablePath);
  const simulatorConfigPath = join(simulatorWritablePath, "config.json");
  ...
  await writeFile(
    simulatorConfigPath,
    JSON.stringify(simulatorConfigData, undefined, 2),
    "utf8"
  );
}
