...
var ResolutionPolicy = {
  ResolutionExactFit: 0,
  ResolutionNoBorder: 1,
  ResolutionShowAll: 2,
  ResolutionFixedHeight: 3,
  ResolutionFixedWidth: 4,

  // reverse mapping
  0: "ResolutionExactFit",

  1: "ResolutionNoBorder",
  2: "ResolutionShowAll",
  3: "ResolutionFixedHeight",
  4: "ResolutionFixedWidth",
};
...
async function generatePreviewData() {
  ...
  const previewScene = await Editor.Profile.getConfig(
    "preview",
    "general.start_scene"
  );
  if (previewScene === "current_scene") {
    previewSceneJson = await Editor.Message.request(
      "scene",
      "query-scene-json"
    );
  } else {
    const previewScenePath = await Editor.Message.request(
      "asset-db",
      "query-path",
      previewScene
    );
    previewSceneJson = await readFile(previewScenePath, "utf8");
  }
  // 模拟器预览不显示插屏，加快启动速度
  data.settings.splashScreen.totalTime = 0;
  const globalInternalLibrary = await Editor.Message.request(
    "asset-db",
  ...
  if (globalInternalLibrary) {
    ...
    data.settings.assets.server =
      (await preview_manager_1.browserPreviewManager.queryPreviewUrl()) +
      "/simulator/";
...
async function writeSettingFile(dstDir) {
  ...
  ensureDirSync(join(dstDir, "src"));
  await writeFile(
    join(dstDir, "src/settings.json"),
    JSON.stringify(data.settings, undefined, 2)
  );
  for (let i = 0; i < data.bundleConfigs.length; ++i) {
    const config = data.bundleConfigs[i];
    const bundleDir = join(dstDir, "assets", config.name);
    ...
    await writeFile(
      join(bundleDir, "cc.config.json"),
      JSON.stringify(config, undefined, 2)
    );
    // TODO: 目前的实现跟 web 预览一样，一次性加载所有脚本
    await writeFile(
      join(bundleDir, "index.js"),
      await generateBundleIndex(config.name),
      "utf8"
    );
  }
}
async function generateBundleIndex(bundleName) {
  ...
  return await ejs_1.default.renderFile(
    join(__dirname, "../../static/simulator/bundleIndex.ejs"),
    {
      bundleName,
      bundleEntry,
    }
...
async function runSimulator(onCompleted) {
  ...
  const jsEnginePath = await getJsEnginePath();
  const nativeEnginePath = await getNativeEnginePath();
  const simulatorRoot = join(
    nativeEnginePath,
    isDarwin ? "simulator/Release/SimulatorApp-Mac.app" : "simulator/Release"
  );
  const simulatorResources = isDarwin
    ? join(simulatorRoot, "Contents/Resources")
    : simulatorRoot;
  const executableSimulator = isDarwin
    ? join(simulatorRoot, "Contents/MacOS/SimulatorApp-Mac")
    : join(simulatorRoot, "SimulatorApp-Win32.exe");
  ensureDirSync(join(simulatorResources, "jsb-adapter"));
  ensureDirSync(join(simulatorResources, "src/cocos-js"));
  // 清空缓存
  await emptyDir(join(simulatorResources, "assets"));
  const autoCleanCache = await Editor.Profile.getConfig(
    "preview",
    "preview.auto_clean_cache"
  );
  if (autoCleanCache) {
    await emptyDir(join(simulatorResources, "gamecaches"));
  }
  // 根据偏好设置的模块配置，生成 cc.js 到 static/simulator/cocos-js 目录
  // TODO: 使用 QUICK_COMPILE 编译 engine
  const ccModuleFile = join(simulatorResources, "src/cocos-js/cc.js");
  const cceCodeQualityFile = join(
    simulatorResources,
    "src/builtin/cce.code-quality.cr.js"
  );
  const cceEnvFile = join(simulatorResources, "src/builtin/cce.env.js");
  ensureDirSync(dirname(ccModuleFile));
  ensureDirSync(dirname(cceCodeQualityFile));
  ...
  const ccEnvConstants =
    statsQuery.constantManager.genCCEnvConstants(ccEnvOptions);
  const features =
    (
  ...
  const { code } = await ccbuild_1.buildEngine.transform(
    statsQuery.evaluateIndexModuleSource(
      featureUnits,
      (moduleName) => moduleName
    ),
    "system"
  );
  const builtinModuleProvider =
    await builtin_module_provider_1.BuiltinModuleProvider.create({
      format: "systemjs",
    });
  await Promise.all([
  ...
  await writeFile(ccModuleFile, code, "utf8");
  await writeFile(cceEnvFile, builtinModuleProvider.modules["cc/env"], "utf8");
  // 拷贝文件
  const toCopy = [
    // 拷贝 jsb-adapter
    {
      src: join(jsEnginePath, "bin/adapter/native/web-adapter.js"),
      dest: join(simulatorResources, "jsb-adapter/web-adapter.js"),
      isDir: false,
    },
    {
      src: join(jsEnginePath, "bin/adapter/native/engine-adapter.js"),
      dest: join(simulatorResources, "jsb-adapter/engine-adapter.js"),
      isDir: false,
    },
    // 拷贝 engine, import-map.json
    {
      src: join(jsEnginePath, "bin/native-preview"),
      dest: join(simulatorResources, "src/cocos-js"),
      isDir: true,
    },
    {
      src: join(__dirname, "../../static/simulator/import-map.json"),
      dest: join(simulatorResources, "src/import-map.json"),
      isDir: false,
    },
    {
      src: join(__dirname, "../../static/simulator/system.bundle.js"),
      dest: join(simulatorResources, "src/system.bundle.js"),
      isDir: false,
    },
    {
      src: join(__dirname, "../../static/simulator/polyfills.bundle.js"),
      dest: join(simulatorResources, "src/polyfills.bundle.js"),
      isDir: false,
    },
    {
      src: join(Editor.Project.tmpDir, "asset-db/effect/effect.bin"),
      dest: join(simulatorResources, "src/effect.bin"),
      isDir: false,
    },
  ];
  toCopy.forEach((item) => {
    if (pathExistsSync(item.src)) {
      if (item.isDir) {
        copyDirSync(item.src, item.dest);
      } else {
  ...
  // 写入初始场景数据
  const data = await generatePreviewData();
  let previewSceneJsonPath = join(simulatorResources, "preview-scene.json");
  previewSceneJsonPath = formatPath(previewSceneJsonPath);
  await writeFile(previewSceneJsonPath, data.previewSceneJson, "utf8");
  // 生成 main.js
  ...
  const mainJsSource = await ejs_1.default.renderFile(
    join(__dirname, "../../static/simulator/main.ejs"),
    {
      libraryPath: formatPath(join(Editor.Project.path, "library")),
      waitForConnect: preference.waitForConnect,
      projectPath: formatPath(Editor.Project.path),
      previewIp,
      previewPort,
  ...
  await writeFile(join(simulatorResources, "main.js"), mainJsSource, "utf8");
  // 生成 application.js
  const hasPhysicsAmmo = features.includes("physics-ammo");
  const projectData = await Editor.Profile.getProject(
    "project",
  ...
  const appJsSource = await ejs_1.default.renderFile(
    join(__dirname, "../../static/simulator/application.ejs"),
    {
      hasPhysicsAmmo,
  ...
  await writeFile(
    join(simulatorResources, "src/application.js"),
    appJsSource,
    "utf8"
  );
  // 根据偏好设置，更新模拟器配置文件
  await generateSimulatorConfig();
  // 运行模拟器
  // environment
  // TODO: 初始化环境变量
  // env = {
  ...
  simulatorProcess = spawn(executableSimulator, args);
  // 打开模拟器调试器
