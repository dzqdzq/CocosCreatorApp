...
describe("预览消息通知测试", () => {
  ...
  describe("generate-settings in web", async () => {
    ...
    const dataKeys = ["settings", "script2library", "bundleConfigs"];
    ...
    if (Editor.Project.name === "build-example") {
      it("生成 指定的 jsList (build-example 仓库)", () => {
        expect(
          data.settings.plugins.jsList.includes(
            "assets/test-js-list/plugin-not-web.js"
          )
        ).to.be.false;
      });
    }
    if (Editor.Project.name === "test-cases") {
      it("生成 指定的 jsList (case 3d 仓库)", () => {
        expect(data.settings.plugins.jsList.includes("assets/test-plugin.js"))
          .to.be.true;
      });
    }
  });
  ...
  describe("generate-settings in windows", async () => {
    ...
    if (Editor.Project.name === "build-example") {
      it("生成 指定的 jsList (build-example 仓库)", () => {
        expect(
          data.settings.plugins.jsList.includes(
            "assets/test-js-list/plugin-not-native.js"
          )
        ).to.be.false;
      });
    }
