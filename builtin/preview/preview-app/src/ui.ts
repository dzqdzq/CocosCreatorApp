...
export class Ui {
    ...
    get debugMode() {
        return parseInt(this._optsDebugMode.value, 10);
    }
    ...
    public showLoading(flag?: boolean) {
        ...
        if (!this._splash) {
            ...
            if (this._splash.style.width < this._splash.style.height) {
                ...
                let logoDisplayH = width * LOGO_SIZE / LOGO_IMG_L_W * LOGO_IMG_L_H;
    ...
    public bindEngine(cc: any) {
        ...
        this._optsDebugMode.addEventListener('change', (event) => {
            ...
            // @ts-ignore
            window.cc.debug._resetDebugSetting(parseInt(value, 10));
     ...
     * 更新屏幕分辨率
     */
    private _updateResolution(cc: any) {
        const { width, height } = this._getEmulatedScreenSize();
        if (!this._gameContainer || this._gameContainer.style) {
            this._gameContainer = this._queryChecked<HTMLElement>('#Cocos3dGameContainer');
        }
