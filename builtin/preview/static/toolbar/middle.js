...
exports.ready = function () {
  ...
  var i = new Vue({
    ...
    data: {
      ...
      platformsIcon: {
        browser: { prefix: "sphere", suffix: "" },
        gameView: { prefix: "scene", suffix: "experiment" },
        simulator: { prefix: "mini-game", suffix: "" },
      },
      gameView: { isPlay: false, isPaused: false, isStep: false },
    ...
    watch: {
      async currPlatform(e) {
        await Editor.Profile.setConfig(
          "scene",
          "console.extend.clearOnPlay.show",
          e === "gameView",
          "global"
        );
    ...
    mounted() {
      this.curSelectScene = { uuid: "current_scene" };
      ...
      Editor.Profile.getConfig(
        ...
        console.debug("[Preview] change platform: " + e);

        if (!this.platformsIcon[e]) {
          e = "browser";
        }

        this.currPlatform = e;
        Editor.Message.send("preview", "change-platform", e);
      });
    },
    destroyed() {
    ...
    methods: {
      async dbReadyCallback() {
        if (await Editor.Message.request("asset-db", "query-ready")) {
          this._refreshScenes();
        }
      },
      previewStopCallback() {
        this.gameView = { isPlay: false, isPaused: false, isStep: false };
      },
      handleOnIPChange(e) {
        this.previewIp = e;
        this.updateAddress();
      },
      handleOnPortChange(e) {
        this.previewPort = e;
        this.updateAddress();
      },
      updateAddress() {
      ...
      async updateIp() {
        ...
        this.previewIp = await Editor.Message.request(
          "preview",
          "get-preview-ip"
        );
      ...
      _onSettingsChanged(i) {
        if (i.detail) {
          let e = { uuid: "current_scene" };
          var r;

          if (
            i.detail !== e.uuid &&
            (r = this.scenes.find((e) => e.uuid === i.detail))
          ) {
            e = r;
          }

          this.curSelectScene = e;
      ...
      _onLocation(e) {
        if (e.detail.value === "current_scene") {
          Editor.Message.request("scene", "query-current-scene").then((e) => {
        ...
        if (!this.isPlaying) {
          ...
          try {
            ...
            Editor.Metrics.trackEvent({
              ...
              category: "view",
              value: {
                A100000: this.currPlatform,
                A100001: Editor.Project.uuid,
              },
            });
          }
        }
      ...
      async gameViewPause() {
        if (
          !this.isPlaying &&
          this.gameView.isPlay &&
          (!this.gameView.isStep || !this.gameView.isPaused)
        ) {
      ...
      async changePlatform(e) {
        if (this.changePlatform !== e) {
          this.currPlatform === "gameView" &&
            (this.gameView.isPlay || this.isPlaying) &&
            e !== "gameView" &&
            (await this.play(),
      ...
      async _refreshScenes() {
        ...
        if (e) {
          ...
          var i = e.find((e) => e.uuid === r);

          if (i) {
            this.curSelectScene = {
              name: this.splitName(i.name),
              uuid: i.uuid,
              url: i.url,
            };
          } else if (r && r !== "current_scene") {
            ...
            console.warn(
              `The start scene: ${r} for preview does not exist, will reset to the default start scene`
            );
          }
          ...
          this.scenes = e
            .map((e) => ({
              uuid: e.uuid,
              name: this.splitName(basename(e.source)),
              url: e.url,
            }))
            .sort((e, i) => collator.compare(e.name, i.name));
        }
      },
      splitName(e) {
        return e
          ? ((e = e.split(".")).splice(e.length - 1, 1),
            e.length > 1 ? e.join(".") : e[0])
          : "";
      },
    },
  });
  ...
  this.documentClickEvent = () => {
    if (e.vm) {
      e.vm.hidePlatformList();
      e.vm.hideQR();
    }
  };

  document.addEventListener("click", e.documentClickEvent);
};

exports.close = function () {
  var e = this;
  document.removeEventListener("click", e.documentClickEvent);
  e.vm?.$destroy();
  e.vm = null;
  window.xxx = null;
};
