    ...
    describe("query-db-info ：查询数据库信息", async () => {
      it("传入错误的参数", async () => {
        ...
        for (const test of queue) {
          try {
            console.debug("start query-db-info with: ", test.args);
  ...
  describe("数据库操作相关接口", () => {
    describe("暂停恢复数据库相关", () => {
      ...
      describe("pause", async () => {
        const pauseSuccess = await Editor.Message.request(
          "asset-db",
          "pause",
          "test"
        );
      ...
      describe("resume", async () => {
        const resumeSuccess = await Editor.Message.request(
          "asset-db",
          "resume"
        );
        it("is busy = false", async () => {
          const isBusy = await Editor.Message.request("asset-db", "is-busy");
          expect(isBusy).to.be.false;
        });
        it("正常执行并收到操作的消息回调", () => {
    ...
    function onDBReady(dbName, dbInfo) {
      dbMessageRes.ready = {
        dbName,
        dbInfo,
      };
    }
    function onDBClose(dbName, dbInfo) {
      dbMessageRes.close = {
        dbName,
        dbInfo,
      };
    ...
    if (Editor.Project.name === "build-example") {
      before(() => {
        Editor.Message.addBroadcastListener("asset-db:db-ready", onDBReady);
        Editor.Message.addBroadcastListener("asset-db:db-close", onDBClose);
      });
      describe("start db", async () => {
        ...
        it("正常收到 db ready 的广播以及返回值", () => {
          expect(dbMessageRes.ready.dbName).equal("test");
          expect(dbMessageRes.ready.dbInfo.name).to.equal("test");
        });
      });
      ...
      describe("stop db", async () => {
        ...
        it("正常收到 db close 的广播以及返回值", () => {
          expect(dbMessageRes.close.dbName).equal("test");
          expect(dbMessageRes.close.dbInfo.name).to.equal("test");
        });
      });
      after(() => {
        Editor.Message.removeBroadcastListener("asset-db:ready", onDBReady);
        Editor.Message.removeBroadcastListener("asset-db:close", onDBClose);
      });
    }
      ...
      describe("刷新数据库", async () => {
        ...
        return new Promise((resolve, reject) => {
          ...
          async function start() {
            const ready = await Editor.Message.request(
              "asset-db",
              "query-ready"
            );
            if (ready) {
