...
describe("测试 db 的查询接口", async () => {
  ...
  describe("query-db-infos", async () => {
    const res = await Editor.Message.request("asset-db", "query-db-infos");
    it("返回结果为数组", () => {
      expect(Array.isArray(res)).is.true;
    });
    it("结果包含 internal 数据库信息", () => {
      const internalInfo = res.find((item) => item.name === "internal");
      expect(internalInfo).to.have.all.keys(dbInfoKeys);
    });
  });
  ...
  describe("query-create-list", async () => {
    ...
    it("艺术字菜单数据正常", () => {
      ...
      const value = {
        ...
        template:
          "db://internal/default_file_content/label-atlas/default.labelatlas",
      };
    ...
    describe("给出的菜单都可以正常创建资源", () => {
      ...
      function testCreate(info) {
        ...
        const label = info.label.startsWith("i18n:")
          ? Editor.I18n.t(info.label.replace("i18n:", ""))
          : info.label;
        describe(`测试创建 ${label || info.label}(${
          info.fullFileName
        })`, async () => {
          ...
          try {
            ...
            it(`创建文件使用的 importer(${assetInfo.importer}) 类型与预期的 ${info.handler}符合`, () => {
              expect(assetInfo.importer).equal(info.handler);
            });
            it(`创建文件的资源后缀与预期 ${extname(
              info.fullFileName
            )} 一致`, () => {
              expect(extname(assetInfo.file)).equal(extname(info.fullFileName));
            });
          } catch (error) {
            console.error(error);
          }
  ...
  describe("query-icon-config-map", async () => {
    ...
    describe("给出的所有图标信息正常", () => {
      ...
      const hasAllKeys = Object.values(result).every((info) =>
        keys.every((key) => key in info)
      );
      expect(hasAllKeys).to.be.true;
    });
    it("查询到的 gltf 图标信息正常", () => {
      const result_gltf = result.gltf;
      expect(result_gltf).not.null;
      const value = {
        thumbnail: false,
        type: "icon",
        value: "gltf",
      };
      expect(result_gltf).to.deep.equal(value);
    });
  });

  describe("query-asset-thumbnail", async () => {
    it("查询 sprite 资源的缩略图信息", async () => {
      const thumbnailInfo = await Editor.Message.request(
        "asset-db",
    ...
    // 此处依赖了场景的接口，需要等待场景启动
    // if (await Editor.Message.request('scene', 'query-is-ready')) {
    ...
    it("查询 prefab 资源的缩略图信息", async () => {
      const thumbnailInfo = await Editor.Message.request(
        "asset-db",
        "query-asset-thumbnail",
        "90bdd2a9-2838-4888-b66c-e94c8b7a5169"
      );
  ...
  describe("query-path", async () => {
    it("查询 assets 数据库", async () => {
      const path = await Editor.Message.request(
        "asset-db",
        "query-path",
        "db://assets"
      );
    ...
    it("查询 assets 数据库里测试生成的临时资源", async () => {
      const path = await Editor.Message.request(
        "asset-db",
        "query-path",
        `db://assets/${name}`
  ...
  describe("query-url", async () => {
    ...
    it("查询 assets 数据库", async () => {
      const url = await Editor.Message.request(
        "asset-db",
        "query-url",
        assetsPath
      );
  ...
  describe("query-uuid", async () => {
    it("查询 assets 数据库", async () => {
      const id = await Editor.Message.request(
        "asset-db",
        "query-uuid",
        "db://assets"
      );
    ...
    it("查询 assets 数据库里不存在的资源2", async () => {
      const id = await Editor.Message.request(
        "asset-db",
        "query-uuid",
        "db://internal/default_file_content/abc.xxx"
      );
      expect(id).is.equal("");
    });
  ...
  describe("query-asset-info 消息接口测试", async () => {
    const values = {
      displayName: "string",
      file: "string",
      imported: "boolean",
    ...
    it("dataKeys: 查询 depends 信息", async () => {
      const info = await Editor.Message.request(
        "asset-db",
        "query-asset-info",
    ...
    it("dataKeys: 查询 mtime 信息", async () => {
      const info = await Editor.Message.request(
        "asset-db",
        "query-asset-info",
        "d032ac98-05e1-4090-88bb-eb640dcb5fc1",
        ["mtime"]
      );
  ...
  describe("query-asset-meta", async () => {
    const values = {
      ver: "string",
      importer: "string",
      imported: "boolean",
  ...
  describe("query-assets", async () => {
    ...
    const allInternal = await Editor.Message.request(
      "asset-db",
      "query-assets",
      { pattern: "db://internal" }
    );
    ...
    it("查询 internal 目录下 ccType = cc.SceneAsset 资源", async () => {
      const allScenes = await Editor.Message.request(
        "asset-db",
        "query-assets",
        { ccType: "cc.SceneAsset", pattern: "db://internal/**/*" }
      );
      expect(allScenes.length).equal(3);
    });
    it("查询 internal 目录下 extname = mp4资源", async () => {
      const allMP4 = await Editor.Message.request("asset-db", "query-assets", {
        extname: ".mp4",
        pattern: "db://internal/**/*",
      });
    ...
    it("通过 pattern 精准查询 internal 下指定地址的 spriteFrame 资源", async () => {
      const sprites = await Editor.Message.request("asset-db", "query-assets", {
        pattern: "db://internal/default_ui/default_editbox_bg.png/spriteFrame",
      });
      expect(sprites.length).equal(1);
    });
    if (Editor.Project.name === "build-example") {
      it("查询 assets 目录下 userData.isPlugin = true 的插件脚本资源", async () => {
        const allPlugins = await Editor.Message.request(
          "asset-db",
          "query-assets",
          { userData: { isPlugin: true }, pattern: "db://assets/**/*" }
        );
        expect(allPlugins.length).equal(3);
      });
    }
  });

  describe("query-asset-mtime", async () => {
    const mtime = await Editor.Message.request(
      "asset-db",
      "query-asset-mtime",
      internalSpriteImageUuid
    );
  ...
  describe("query-asset-data", async () => {
    ...
    it("查询 data 数据是否完整", () => {
      expect(data).to.have.all.keys(["value", "url", "versionCode"]);
    });
    it("data 数据需要包含依赖信息", () => {
  ...
  if (Editor.Project.name === "build-example") {
    ...
    describe("query-asset-users", () => {
      it("脚本 uuid, asset -> 使用此脚本 uuid 的资源列表", async () => {
        const assetUuids = await Editor.Message.request(
          "asset-db",
          "query-asset-users",
          scriptRequired
        );
      ...
      it("image 资源 uuid, asset -> 使用此资源 uuid 的资源列表", async () => {
        ...
        expect(assetUuids).to.include(sceneUuid);
        expect(assetUuids).to.include(prefabUuid);
        expect(assetUuids).to.not.include(spriteUuid);
      });
    ...
    describe("query-asset-dependencies", () => {
      ...
      it("场景 uuid, script -> 场景使用的脚本列表", async () => {
        const uuids = await Editor.Message.request(
          "asset-db",
          "query-asset-dependencies",
          sceneUuid,
          "script"
        ...
        expect(uuids).to.include("f14e1127-f4ff-418d-a54a-5d7daaa942c8");
      });
  ...
  describe("query-asset-userData-config", async () => {
    const config = await Editor.Message.request(
      "asset-db",
      "query-asset-userData-config",
      internalSpriteImageUuid
    );
  ...
  describe("query-asset-config-map", async () => {
    const configMap = await Editor.Message.request(
      "asset-db",
      "query-asset-config-map"
    );
    expect(configMap).to.exist;
    expect(configMap.image).to.include.keys([
      "description",
      "displayName",
      "iconInfo",
      "userDataConfig",
  ...
  describe("batch-message-handler", async () => {
    ...
    const urls = await Editor.Message.request(
      "asset-db",
