...
describe('测试 db 的查询接口', async function() {
    ...
    describe('query-create-list', async () => {
        ...
        describe('给出的菜单都可以正常创建资源', () => {
            ...
            function testCreate(info) {
                ...
                const label = info.label.startsWith('i18n:') ? Editor.I18n.t(info.label.replace('i18n:', '')) : info.label;
                describe(`测试创建 ${label || info.label}(${info.fullFileName})`, async () => {
    ...
    describe('query-assets', async function() {
        ...
        it('通过 pattern 精准查询 internal 下指定地址的 spriteFrame 资源', async () => {
            const sprites = await Editor.Message.request('asset-db', 'query-assets', { pattern: 'db://internal/default_ui/default_editbox_bg.png/spriteFrame'});
    ...
    if (Editor.Project.name === 'build-example') {
        ...
        describe('query-asset-users', function() {
            it('脚本 uuid, asset -> 使用此脚本 uuid 的资源列表', async () => {
                const assetUuids = await Editor.Message.request('asset-db', 'query-asset-users', scriptRequired);
            ...
            it('image 资源 uuid, asset -> 使用此资源 uuid 的资源列表', async () => {
                const assetUuids = await Editor.Message.request('asset-db', 'query-asset-users', imageUuid);
                expect(assetUuids).to.include(sceneUuid);
                expect(assetUuids).to.include(prefabUuid);
    ...
    describe('query-asset-config-map', async () => {
        ...
        expect(configMap.image).to.include.keys(['description', 'displayName', 'iconInfo', 'userDataConfig', 'from']);
    });
