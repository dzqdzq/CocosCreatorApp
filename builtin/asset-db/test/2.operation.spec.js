...
const invalidParams = [
  ...
  {
    name: "readonly url",
    value: "db://internal/primitives.fbx/lambert1.material",
  },
];

describe("测试 db 的操作接口", async () => {
  ...
  describe("copy-asset", () => {
    ...
    it("复制普通资源", async () => {
      ...
      const content = readFileSync(
        join(databasePath, `${name}.normal2`),
        "utf8"
      );
      expect(content).is.equal("test");
    });
    ...
    invalidParams.forEach((item) => {
      it(`第一个参数传入参数错误 - ${item.name}`, async () => {
        let err = false;
        try {
          const info = await Editor.Message.request(
            "asset-db",
            "copy-asset",
            item.value,
            join(databasePath, `${name}.normal10`)
          );
        } catch (error) {
          err = true;
        }
      ...
      it(`第二个参数传入参数错误 - ${item.name}`, async () => {
        let err = false;
        try {
          const info = await Editor.Message.request(
            "asset-db",
  ...
  describe("move-asset", () => {
    ...
    it("移动普通资源", async () => {
      ...
      const content = readFileSync(
        join(databasePath, `${name}.normal3`),
        "utf8"
      );
      expect(content).is.equal("test");
    });
    ...
    it("普通的移动重命名资源", async () => {
      ...
      const content = readFileSync(
        join(databasePath, `${testName2}.normal3`),
        "utf8"
      );
      expect(content).is.equal("test");
    });

    it("大小写差异的重命名资源", async () => {
      ...
      const testName1Uuid = await Editor.Message.request(
        "asset-db",
        "query-uuid",
        join(databasePath, `${testName1}.normal3`)
      );
      expect(!!testName1Uuid).is.equal(false);

      const testName2Uuid = await Editor.Message.request(
        "asset-db",
        "query-uuid",
        join(databasePath, `${testName2}.normal3`)
      );
      ...
      const metaExist = existsSync(
        join(databasePath, `${testName2}.normal3.meta`)
      );
  ...
  describe("delete-asset", () => {
    describe("删除文件夹", async () => {
      ...
      it("删除文件夹后源文件 meta 应该不存在", () => {
        const metaExists = existsSync(
          join(databasePath, `${name}.directory3.meta`)
        );
    ...
    it("使用 url 删除普通资源", async () => {
      await Editor.Message.request(
        "asset-db",
        "delete-asset",
        `db://assets/__asset-db-test__/${name}a.normal3`,
        "test"
      );
      ...
      const metaExists = existsSync(
        join(databasePath, `${name}a.normal3.meta`)
      );
      expect(metaExists).is.equal(false);
    });

    it("使用 uuid 删除普通资源", async () => {
      const testName = `${name}_delete.normal`;
      const asset = await Editor.Message.request(
        "asset-db",
        "create-asset",
        `db://assets/__asset-db-test__/${testName}`,
        "test"
      );
  ...
  describe("save-asset", () => {
    ...
    invalidParams.forEach((item) => {
      it(`传入参数错误 - ${item.name}`, async () => {
        let err = false;
        try {
          const bool = await Editor.Message.request(
            "asset-db",
            "save-asset",
            item.value
          );
  ...
  describe("reimport-asset", () => {
    it("普通资源 uuid 的 reimport", async () => {
      ...
      const assetMeta = await Editor.Message.request(
        "asset-db",
        "query-asset-meta",
        uuid
      );
    ...
    it("子资源 url 的 reimport", async () => {
      ...
      const subAssetUuid = await Editor.Message.request(
        "asset-db",
        "query-uuid",
        subAssetUrl
      );
  ...
  describe("rename-asset", () => {
    ...
    it("大小写差异的重命名资源", async () => {
      ...
      const testName1Uuid = await Editor.Message.request(
        "asset-db",
        "query-uuid",
        join(databasePath, `${testName1}.normal2`)
      );
      expect(!!testName1Uuid).is.equal(false);
      ...
      const testName2Uuid = await Editor.Message.request(
        "asset-db",
        "query-uuid",
        join(databasePath, `${testName2}.normal2`)
      );
      expect(!!testName2Uuid).is.equal(true);

      const metaExist = existsSync(
        join(databasePath, `${testName2}.normal2.meta`)
      );
      expect(metaExist).is.equal(true);
    ...
    it("重命名资源但实际为移动文件", async () => {
      ...
      const exists2 = existsSync(
        join(databasePath, `move/${testName2}.normal2`)
      );
      expect(exists2).is.equal(true);
  ...
  describe("new-asset", async () => {
    ...
    describe("创建普通资源 overwrite", async () => {
  ...
  describe("open-asset", () => {
    ...
    if (Editor.Project.name === "build-example") {
      describe("打开 ts 脚本", async () => {
        const hasScriptEditor = await Editor.Profile.getConfig(
          "program",
          "scriptEditor.path"
        );
        ...
        it("未配置脚本编辑器", async () => {
          if (hasScriptEditor) {
            await Editor.Profile.removeConfig("program", "scriptEditor.path");
          }
    ...
    it("打开图片", async () => {
      const res = await Editor.Message.request(
        "asset-db",
        "open-asset",
        "db://internal/default_ui/default_btn_pressed.png"
      );
      expect(res).equal(true);
    });
    ...
    it("不允许打开内置模板的 prefab", async () => {
      const res = await Editor.Message.request(
        "asset-db",
        "open-asset",
        "db://internal/default_file_content/prefab/default.prefab"
      );
      expect(res).equal(false);
    });
  });
  ...
  describe("batch-message-handler", async () => {
    ...
    const testResult = [
      "db://internal/Default-Particle.png",
      "db://internal/primitives.fbx",
    ];
  ...
  describe("generate-available-url", () => {
    ...
    it("db://internal/default_ui 作为目标路径自动更名", async () => {
      ...
      const validUrl = await Editor.Message.request(
        "asset-db",
  ...
  describe("update-default-user-data", () => {
    ...
    if (existsSync(defaultMetaPath)) {
      describe("获取用户本地导入配置数据，验证接口查询结果适合正常", async () => {
        ...
        const assetConfigMap = await Editor.Message.request(
          "asset-db",
          "query-asset-config-map"
        );
        Object.keys(info).forEach((assetHandlerName) => {
          if (
            !assetConfigMap[assetHandlerName] ||
            !Object.keys(info[assetHandlerName])
          ) {
            return;
          }
          Object.keys(info[assetHandlerName]).forEach((type) => {
            ...
            it(`验证 ${assetHandlerName} ${type} 的默认值 ${userDataConfig[type].default} 是否与用户文件内容 ${info[assetHandlerName][type]}一致 `, () => {
              expect(userDataConfig[type].default).to.equal(
                info[assetHandlerName][type]
              );
            });
          });
        });
    ...
    describe(`更新 image 的默认导入类型为 ${testImageType}`, async () => {
      ...
      it(`验证拷贝的 image 的导入类型为 ${testImageType}`, async () => {
        ...
        const testImage = await Editor.Message.request(
          "asset-db",
          "query-path",
          "db://internal/default_ui/default_btn_disabled.png"
        );
    ...
    describe(`更新 sprite-frame 的 trim type 类型为 ${testTrimType}`, async () => {
      ...
      it("验证实际写入文件的内容修改正常", () => {
        expect(readJSONSync(defaultMetaPath)["sprite-frame"].trimType).equal(
          testTrimType
        );
      });
    });
