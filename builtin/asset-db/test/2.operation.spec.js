...
const { existsSync, statSync, readJSONSync, writeJSONSync, readFileSync, removeSync, copy } = require('fs-extra');
...
describe('测试 db 的操作接口', async function() {
    ...
    describe('update-default-user-data', () => {
        it('更新 image 的默认导入类型为 texture', async () => {
            await Editor.Message.request('asset-db', 'update-default-user-data', 'image', 'type', 'texture');
            ...
            expect(assetMeta.userData.type).equal('texture');
        });
    });
