...
import { GFXPrimitiveMode } from 'cc';
import { PPGeometry, PPGeometryTypedArray, getNormalizer } from '../../../source/importer-3d/importers/utils/pp-geometry';
...
function average(arr: number[], by: number) {
...
expect.extend({
    toBeExactArrayBufferView<T extends ArrayBufferView>(received: T, expected: T, epsilon = 1e-5) {
        ...
        return received.length === expected.length &&
            received.every((v: number, i: number) => Math.abs(v - expected[i]) < epsilon) ? ({
                pass: true,
...
test('pp-geometry:reduce-joint-influences', () => {
    ...
    const merge = (jointsWeights: Array<{
        ...
        const ppGeometry = new PPGeometry(nVertices, GFXPrimitiveMode.POINT_LIST);
