...
function RSAEncrypt(r) {
  var e = require("crypto");
  return e.publicEncrypt(
    { key: rsakey, padding: e.constants.RSA_PKCS1_PADDING },
    Buffer.from(r)
  );
}
const encrypt = {
  ...
  encryptPostData(r) {
    ...
    let c = 0;
    for (var n = (r = Buffer.from(r)).length; 0 < n - c; ) {
      t =
        ((e =
          n - c > MAX_ENCRYPT_BLOCK
            ? RSAEncrypt(r.slice(c, c + MAX_ENCRYPT_BLOCK))
            : RSAEncrypt(r.slice(c, c + n - c))),
        Buffer.concat([t, e], e.length + t.length));

      c += MAX_ENCRYPT_BLOCK;
    }
    var l = t.toString("base64");
    return this.urlsafe_b64encode(l);
  },
};
