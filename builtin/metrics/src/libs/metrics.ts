...
import { googleG4MetricsObserver } from '../observers/google-metrics-observer-v4';
...
export class Metrics {
    ...
    public async init() {
        ...
        this._metricsDebugMode = !!(await Editor.Profile.getConfig('metrics', 'analytics-debug'));

        // 初始化统计日志
        await logMgr.init(outputMetricLog, this._metricsDebugMode);

        const disableGa = !!(await Editor.Profile.getConfig('metrics', 'disable-analytics-ga'));
        if (!disableGa) {
            this.addMetricsObserver(require('../observers/google-metrics-observer'));
        }
        this.addMetricsObserver(googleG4MetricsObserver);
        const cocosMetricsObserver = require('../observers/cocos-metrics-observer');
        try {
            // 获取配置内定义的内置 appID ，由此来支持外部修改统计的 appID ，主要在发布不同类型编辑器时使用，比如车机编辑器
            // 由于 metric 插件最先启动，无法通过配置系统默认值获取
            const customAppID = require('../../package.json').contributions.profile.editor.appID.default;
            customAppID && (cocosMetricsObserver._analyticsID = customAppID);
        } catch (error) {
            console.error(error);
        }
        this.addMetricsObserver(cocosMetricsObserver);
    ...
    public _trackEventWithTimer(index: number) {
        ...
        try {
            ...
            if (!this._timer) {
                // debug 模式下 1 分钟发送一次统计，否则是 5 分钟发送一次统计
                const ms = this._metricsDebugMode ? 60 * 1000 : 5 * 1000 * 60;
