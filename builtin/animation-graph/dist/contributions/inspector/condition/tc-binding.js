...
const variable_selection_1 = __importDefault(require("../variable-selection"));
...
const tcBinding = {
  components: { "variable-selection": variable_selection_1.default },
  props: ["binding", "queryData", "typeFilter", "transitionSourceType"],
  template,
  methods: {
    showBindingTypeMenu() {
      var i = this.typeFilter;
      const { tcBindingTypeInfos, TCBindingValueType } = this.queryData;

      const n =
        typeof i == "string"
          ? [TCBindingValueType[i]]
          : i.map((i) => TCBindingValueType[i]);

      i = tcBindingTypeInfos.filter((i) =>
        i.provisions.some((i) => n.includes(i))
      );
      Editor.Menu.popup({
        menu: i
          .filter(
            (i) =>
              !i.transitionSourceFilter ||
              i.transitionSourceFilter.includes(this.transitionSourceType)
          )
          .map((i) => ({
            label: `i18n:ENGINE.classes.${i.id}.menu`,

            click: () => {
              this.$emit("binding-type-changed", i.id);
            },
          })),
      });
    },
