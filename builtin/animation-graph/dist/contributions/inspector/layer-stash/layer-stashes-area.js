...
const layerStashesArea = {
  props: ["stashes", "layerId"],
  template,
  methods: {
    t(...e) {
    ...
    addLayerStash() {
      Editor.Message.send("scene", "execute-scene-script", {
        ...
        method: "addLayerStash",
        args: [this.layerId],
      });
    },
    async removeLayerStash(e) {
      var a = this.stashes[e].referenceCount;
      if (
        a > 0 &&
        (
          await Editor.Dialog.info(
            Editor.I18n.t(
              "animation-graph.layer.layer_stashes_area.remove_stash_button.confirm_dialog.message",
              { referenceCount: a }
            ),
            {
              title: Editor.I18n.t(
                "animation-graph.layer.layer_stashes_area.remove_stash_button.confirm_dialog.title"
              ),
              buttons: [
                Editor.I18n.t(
                  "animation-graph.layer.layer_stashes_area.remove_stash_button.confirm_dialog.yes"
                ),
                Editor.I18n.t(
                  "animation-graph.layer.layer_stashes_area.remove_stash_button.confirm_dialog.cancel"
                ),
              ],
              default: 1,
              cancel: 1,
      ...
      Editor.Message.send("scene", "execute-scene-script", {
        name: "animation-graph",
        method: "removeLayerStash",
        args: [this.layerId, e],
      });
    },
    renameLayerStash(e, a) {
      var a = a.target;

      if (a && e in this.stashes) {
        if ((a = a.value.trim()) && !(a in this.stashes)) {
          Editor.Message.send("scene", "execute-scene-script", {
            name: "animation-graph",
            method: "renameLayerStash",
            args: [this.layerId, e, a],
          });
        }
      }
    },
    renameLayerStashCancel() {},
    editLayerStash(e) {
      Editor.Message.send("scene", "execute-scene-script", {
        name: "animation-graph",
        method: "editLayerStash",
        args: [this.layerId, e],
      });
    },
  },
...
exports.default = layerStashesArea;
