...
describe('项目设置消息接口测试', () => {
    ...
    describe('get-setting：查询设置信息', () => {
        it('查询预览的分辨率宽度设置,应与 profile 取得的值一致', async () => {
            let width = await Editor.Message.request('project', 'get-setting', 'preview.design_width');
    ...
    describe('set-setting && save-setting：写入/保存项目配置', () => {
        it('设置预览的分辨率宽度为600', () => {
            ...
            // 设置有异步问题
            process.nextTick(() => {
                let language = await Editor.Profile.getConfig('project', 'preview.design_width');
