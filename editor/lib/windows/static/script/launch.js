...
exports.header = function () {
    ...
    const projectProfile = Editor.Profile.load('project://project.json');
    let vm = new Vue({
        el: '#toolbar',
        data: {
            transform: 0,
            ...
            scene: projectProfile.get('start-scene') || 'current',
            sceneList: [],
        ...
        watch: {
            ...
            scene () {
                projectProfile.set('start-scene', this.scene);
                projectProfile.save();
            }
        ...
        methods: {
            ...
            initSceneList() {
                Editor.assetdb.queryAssets(null, 'scene', (err, results) => {
                    this.sceneList = results.map(function (result) {
                        return {
                            value: result.uuid,
                            text: Path.basename(result.url, Path.extname(result.url))
                        };
                    });
                    this.sceneList.unshift({
                        value: 'current',
                        text: Editor.T('PROJECT_SETTINGS.preview.current_scene')
                    });
                });
            }
        },
    ...
    projectProfile.on('changed', () => {
        vm.scene = projectProfile.get('start-scene');
    });

    ipcRenderer.on('asset-db:assets-moved', function ( event, results ) {
        let scenes = results.map(asset => {
            if (asset.type === "scene") {
                return asset;
            }
        }).filter(Boolean);
        if (scenes.length > 0) {
            vm.sceneList = vm.sceneList.map(scene => {
                let item = scenes.find(asset => scene.value === asset.uuid);
                if (item) {
                    return {
                        value: item.uuid,
                        text: Path.basename(item.url, Path.extname(item.url))
                    }
                }
                return scene;
            });
    ...
    ipcRenderer.on('asset-db:assets-deleted', function ( event, results ) {
        let scenes = results.map(asset => {
            if (asset.type === "scene") {
                return asset;
            }
        ...
        if (scenes.length > 0) {
            vm.sceneList = vm.sceneList.filter(scene => {
                return scenes.find((item) => scene.value !== item.uuid);
            }).filter(Boolean);
        }
    });

    ipcRenderer.on('asset-db:assets-created', function ( event, results ) {
        let scenes = results.map(asset => {
            if (asset.type === "scene") {
                return asset;
            }
        ...
        if (scenes.length > 0) {
            const sceneList = scenes.map((scene) => {
                return {
                    value: scene.uuid,
                    text: Path.basename(scene.url, Path.extname(scene.url))
                };
            });
            vm.sceneList = vm.sceneList.concat(sceneList);
        }
    });
