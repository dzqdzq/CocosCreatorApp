...
class NativeWindowManager {
    ...
    editorHandle: any;
    // 分别记录主窗口和预览窗口大小
    mainWindowRect: IRectLike = { x: 0, y: 0, w: 0, h: 0 };
    previewWindowRect: IRectLike = { x: 0, y: 0, w: 0, h: 0 };
    ...
    init(editorHandle: any, w: number, h: number) {
        this.editorHandle = editorHandle;
        this.parentWindow = this.manager.init(editorHandle, w, h);
        if (!this.parentWindow) {
            console.error('editorNative init fail');
        }
    }

    setMainWindowRect(rect: Partial<IRectLike>) {
    ...
    setPreviewWindowRect(rect: Partial<IRectLike>) {
    ...
    async createWindow(w: number, h: number, name = '', isPreview = false): Promise<SceneWindow | null> {
        ...
        const { handler } = await this.request(isPreview, 'createWindow', w, h, name, this.editorHandle.toString('base64'));
    ...
    resizeWindow(name: string, rect: Partial<IRectLike>) {
        if (name === 'MainWindow') {
